...
| --- | --- | --- |
| P1 | The selected CCPU USE16 instruction-IP update masks the delta instead of the final sum; 0xfffe plus 4 yields 0x10002 instead of 0x0002. [Source proof](../etc/evidence/m0-t413-s1-accessor-layout-audit.md). | Admit a complete original instruction-boundary recovery profile covering branch directions, operand sizes, page crossings, stack return addresses and exception boundaries. Explicitly excluded from the video-accessor profile; not repaired by its closure. |

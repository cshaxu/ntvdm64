...
| --- | --- | --- |
| 1 | Eliminate private MVDM/OpenNT-host components and plan project-component reorganization — sequentially remove `mvdm-softpc-patch`, `mvdm-overlay`, and `opennt-host-overlay` by restoring each body to its proper original mirror or named adapter boundary, then audit `app`/`broker`/`session`/`adapter-*` relationships and publish a separate reorganization plan. | [Proposal](../proposals/proposal-retire-mvdm-softpc-patch-001.md) |
...
| 3 | Selective NTVDMx64 source-correction recovery — prove and repair remaining CPU/device/ABI, rendering and DPMI lifecycle defects; account for existing equivalent fixes and reject unsupported optimization/hack imports. | [Proposal](../proposals/proposal-ntvdmx64-source-corrections-001.md) |

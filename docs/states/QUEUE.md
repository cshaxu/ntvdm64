...
| --- | --- | --- |
| 1 | CCPU40 C-VID and event-path research and restoration — resolve U01 using original direct-interface and event-policy evidence, prove the complete profile closure, then replace unsupported local binding/calibration approximations without changing CPU instruction semantics. | [Proposal](../proposals/proposal-ccpu40-cvid-event-path-restoration-001.md) |
| 2 | Cross-process broker closure — restore D01-D07/D10 as one original BaseSrv/BaseClient lifecycle with three-program composition and finite IPC. Prove selected coordination rows without acquiring guest, COMMAND, SoftPC or WOW provider behavior. | [Proposal](../proposals/proposal-cross-process-broker-closure-001.md) |

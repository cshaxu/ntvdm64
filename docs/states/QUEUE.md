...
| 2 | COMMAND standard-stream and pipe recovery — preserve original COMMAND ownership while making host-provided `stdin`/`stdout`/`stderr` attachments independent of Console membership, and prove direct plus nested COMMAND file redirection and pipes. | [Proposal](../proposals/proposal-command-standard-stream-pipe-recovery-001.md) |
...
| 4 | WOW16 `WRITE.EXE` workload recovery — use the preceding contract ledger to recover the one immediate original owner that still prevents selected Win16 programs from starting, running and returning inside an existing `ntvdm.exe` worker. This is not a single-process product or a second broker architecture. | [Proposal](../proposals/proposal-wow16-single-process-lifecycle-recovery-001.md) |

...
tests, its selected real runtime workload, and package closure.  A separate
single-process compatibility-matrix candidate is not a substitute for these
per-owner exit criteria.  It may not restart repository-wide BFS, rename
the shared ledgers into a new gate, or defer its owner slice merely because an
unrelated package remains unclassified. Cross-package interface and perimeter
...
Each candidate below turns its own recovered original-owner contract into
proved single-process behavior with its selected workload; static package
closure and a reached BOP are not evidence that COMMAND, a DOS child, a DOS
...
[Broker proposal's closure record](../proposals/proposal-cross-process-broker-closure-001.md).
The owner-promoted [C-VID accessor proposal](../proposals/proposal-ccpu40-cvid-accessor-layout-restoration-001.md)
has now been admitted in Status and is absent from the remaining candidates.

On 2026-09-12 the owner approved the revised candidate sequence: physical-memory
mapping, DPMI/XMS, RTL,
DOS file services, Console/resources, CCPU40 C-VID/event recovery, broker with original BaseSrv/entry,
...
| --- | --- | --- |
| 1 | WOW32 complete runtime recovery — inherit all prior USER/WOW32 research, retained partial implementation and immutable WOW16/WRITE acceptance; finish source-backed client objects, task/window/message lifecycles, resources, services and full production validation. | [Proposal](../proposals/proposal-wow32-complete-runtime-recovery-001.md) |
| 2 | VDMREDIR/WOW32 DLL compilation-boundary reorganization — after the active VDMREDIR package accepts, make both independently loaded DLL targets explicit while keeping original sources in the canonical MVDM mirror and worker-local state solely in `ntvdm.exe`. | [Proposal](../proposals/proposal-vdmredir-wow32-dll-component-boundary-reorganization-001.md) |
| 3 | OpenNT error-dialog response and termination semantics recovery — restore and verify the source-defined Abort/Retry/Ignore result paths, fatal worker completion tail and bounded WOW hard-error task termination before further runtime packaging work. | [Proposal](../proposals/proposal-error-dialog-termination-semantics-restoration-001.md) |
| 4 | Product experience repair — fix evidenced launch/use/exit defects, starting with a Console that persists after dragging a program onto run16; preserve inherited shells, interactive tasks and independent workers. | [Proposal](../proposals/proposal-product-experience-repair-001.md) |
| 5 | MVDM whole-closure completeness audit and recovery — reconcile all original MVDM parts with imported and actually used code; repair missing/cropped/unconnected originals, justify serious or unavoidable exclusions, and reduce mirror diffs and autonomous implementations without losing verified capabilities. | [Proposal](../proposals/proposal-mvdm-whole-closure-recovery-001.md) |
| 6 | Project-wide original-source implementation minimization review — audit all autonomous implementations, mirror diffs, overlays and bindings for functional duplication of original OpenNT or existing mvdm/opennt-host implementations; consolidate to original owners and remove unnecessary local code while preserving verified capabilities. Known register/FLAGS cases are examples, not the scope boundary. | [Proposal](../proposals/proposal-original-source-implementation-minimization-review-001.md) |
| 7 | CCPU40/V86 guest-contract consistency audit — before further package recovery, establish whether the current CCPU40 host actually supplies the original DOS, DOSX/DPMI and WOW16 guest contracts; produce a bidirectional original-owner/provider ledger and attribute the first proven WRITE blocker without speculative repair. | [Proposal](../proposals/proposal-ccpu40-v86-guest-contract-audit-001.md) |
| 8 | SoftPC `kvm-window` graphics/full-screen presentation — subject to source-policy admission, introduce the sibling SoftPC component as a minimal `ntvdm.exe` worker-local presentation adapter for graphics modes and optional full-screen operation, while retaining the native Console text route and existing multi-worker architecture. | [Proposal](../proposals/proposal-kvm-window-graphics-presentation-001.md) |

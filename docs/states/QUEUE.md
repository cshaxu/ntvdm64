...
| --- | --- | --- |
| 1 | BDM Terminal Task Manager — a product-owned native Console TUI that queries BaseSrv's authenticated task snapshot for this product's DOS/Win16/WOW16 tasks and can request confirmed, broker-authorized termination of one selected task. It shows an explicit empty/unavailable state when no tasks or broker exist, and never enumerates or controls unrelated Windows processes. | [Proposal](../proposals/proposal-bdm-terminal-task-manager-001.md) |
| 2 | MVDM runtime package completion — recover every current-product original package unit as a complete manifest, interface and lifecycle closure before claiming it connected; retire duplicate/limited bindings where an original owner can compose. Each S closes exactly one original package unit. | [Proposal](../proposals/proposal-mvdm-runtime-package-completion-001.md) |
| 3 | Single-process WOW16 application lifecycle recovery — after complete prerequisite package closures, prove the selected WRITE workload's original start/run/return within one x86 worker. No substitute guest/UI failure policy, private USER/CSRSS shell or transport-based workaround. | [Proposal](../proposals/proposal-wow16-single-process-lifecycle-recovery-001.md) |

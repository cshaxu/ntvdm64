...
| --- | --- | --- |
| 1 | Retire `mvdm-softpc-patch` — move each still-selected SoftPC declaration or correction to its original `mvdm` owner, using `mvdm-overlay` only for irreducible material providers; remove the component, its build edges and stale component architecture references. | [Proposal](../proposals/proposal-retire-mvdm-softpc-patch-001.md) |

...
| --- | --- |
| 1 | x64 production retirement and x86 source restoration — retire native-x64 product compile/link/staging/runtime targets. The sole current product and acceptance executable remains `ntvdm32.exe`; preserve x86 execution under host WOW64 and do not create a second worker route. Do not repair, preserve or create x64-only source differences merely for native-x64 closure. The sole exception is a demonstrated architecture-neutral mapping-manager correctness defect; its shared session contract remains architecture-neutral. |
...
| 3 | Cross-process broker closure — only after candidate 2's applicable single-process owner contract closes, recover required BaseSrv-style registration, command-queue, notification and disconnect-cleanup semantics through public IPC, and prove the selected brokered lifecycle rows. It cannot own guest, COMMAND, SoftPC or WOW behavior. |
| 4 | Multi-process matrix and product release — prove the brokered multi-process x86-worker profile, the cross-owner DOS/Win16 compatibility matrix, portable media layout and release packaging. Host x64 Windows is an x86-worker acceptance platform, not a native-x64 worker requirement. |

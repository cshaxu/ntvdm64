...
| Reference Baseline | T410 S1 retired the isolated app window at the formal source/build level; its runtime container exited `0x40` before guest input, as recorded in the S1 evidence. S2 P1 separated pointer count from the text cursor; P2 restores returned-input prepend and NOWAIT/NOREMOVE; P3 restores reserved Alt+Enter pair consumption without recreating a display action. The noninteractive Console limitation is recorded in the indexed S2 evidence. This S owns the input boundary only; D25 and D34 remain later S slices. |

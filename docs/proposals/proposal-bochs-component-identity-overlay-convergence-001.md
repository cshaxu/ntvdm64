...
| S7 | Re-audit the S4/S6 retained diff. Move duplicated default-off observer gate/macro scaffolding to the private overlay, and re-test whether the non-product keyboard creation body can join its existing private teardown body. Keep every CPU decision point, checked-RAM interface and true-subset product-shell omission that must remain in the adopted source. | Per-file baseline measurements show the reduced retained diff; the mirror contains only minimal marked calls/declarations/subsets, the overlay remains private, and focused observer plus two-round headless-8042 fixtures pass without behavior expansion. |
...
[docs/history/m0-t265-closure-20260824.md](../../artifacts/documentation-archive/20260910/strict-topology/history/raw/m0-t265-closure-20260824.md).

...
1. **S1 — source/mechanics audit (closed):** `int5c.inc` and `vdmredir.h`
   ...
   [S1 mechanics map](../../artifacts/documentation-archive/20260910/etc/evidence/m0-t253-s1-selector-blind-async-delivery-mechanics-map-001.md).
2. **S2 — minimal mechanical seam (closed):** the fixed no-payload physical
   IRQ request uses the existing mantle PIC lifecycle, with accepted, masked,
   inactive and out-of-range tests. No BOP/DOS/OpenNT naming enters
   bx-core/bx-mantle; no Bochs exception is selected. See
   [S2 closure](../../artifacts/documentation-archive/20260910/etc/evidence/m0-t253-s2-physical-irq-mechanical-closure-001.md).

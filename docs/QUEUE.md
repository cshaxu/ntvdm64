...
| --- | --- | --- | --- |
| 1 | Engine composition of the finite declared-target sequence | Recompose only the current v5 CLI/adapter objects into the retained native-machine derivative, then observe the unchanged finite sequence once under a bounded budget. | T181 local closure, exact object/link manifest, byte-identical retained machine inputs, and a one-run observation plan that cannot claim normal DOS result transport. |

...
| 4 | Original WOW owner packages | Recover `wow32` and `fax` after their monitor, COMMAND, Redirector and adapter-wow rows close. | [Wave 8](etc/operations/mvdm-package-recovery-wave-plan-001.md#wave-8-wow). |
| 5 | Original VDD and debugger packages | Recover `vdd`, `bde`, `dbg`, `vdmdbg` and `vdmexts` after required monitor and machine callback rows close. | [Wave 9](etc/operations/mvdm-package-recovery-wave-plan-001.md#wave-9-vdd-and-debugger-products). |
| 6 | MVDM platform-ABI canonical union | Replace edition-named parallel ABI source roots with one selected OpenNT public/private declaration tree under `mvdm-platform-abi`; record every merged, one-sided and conflicting header selection with source/hash/rejection evidence, then update all include/build manifests. | [Platform-ABI canonical-union proposal](etc/operations/proposal-mvdm-platform-abi-canonical-union-001.md). |
| 7 | Dual-architecture product composition and integrated verification | Link all admitted original package closures, audit the tracker and run declared host/guest integration profiles. | [Wave 10](etc/operations/mvdm-package-recovery-wave-plan-001.md#wave-10-product-composition-and-integrated-verification). |

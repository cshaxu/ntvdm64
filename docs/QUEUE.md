...
| --- | --- | --- | --- |
| 1 | OpenNT guest SFT opaque-handle publication | Recover the guest JFN/SFT ↔ shared opaque host-token boundary that Redirector async pipes require, without restoring a raw host `HANDLE` in guest RAM. | [SFT token proposal](etc/operations/proposal-opennt-guest-sft-opaque-handle-publication-001.md): `MapNtHandle` source/lifetime ledger and token-boundary regression. |
| 2 | Selector-blind guest async-completion delivery | Recover the mechanical interrupt-delivery contract needed by original `int5c.asm` ANR completion, without giving bx-vdm CPU/PIC/IVT ownership. | [Async delivery proposal](etc/operations/proposal-selector-blind-guest-async-completion-delivery-001.md): original call-frame and native-machine seam audit. |

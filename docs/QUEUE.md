...
| --- | --- | --- | --- |
| 1 | Canonical original first-degree function audit | Resolve every T298 external interface spelling from the two original OpenNT source trees only. Record every original definition/variant, original declaration boundary and leaf status; output direct outgoing calls only as frozen second-degree candidates. No current mirror, adapter or `src.old` source is an input. | T298 canonical external-interface and raw-call-resolution ledgers; original OpenNT/OpenNT-4.5 source indexes. |

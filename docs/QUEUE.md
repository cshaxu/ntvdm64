...
| 7 | NTDOS/COMMAND guest EXEC and parent-return completion | Recover guest-owned PSP/arena/JFN/environment, child execution and ordinary `54:0B` parent return without placing DOS semantics in the host worker. | [Guest EXEC/parent-return proposal](etc/operations/proposal-ntdos-command-guest-exec-parent-return-001.md): original guest source map, owner recovery and declared parent-return profile. |
| 8 | OpenNT COMMAND cmdExec32 full-capability closure | Audit every source-reachable `cmdExec32` capability, integrate completed owner packages, and close one declared end-to-end COMMAND execution profile. | [cmdExec32 full-closure proposal](etc/operations/proposal-opennt-command-cmdexec32-full-capability-closure-001.md): prerequisite ledger, source-first integration and complete-profile regression. |

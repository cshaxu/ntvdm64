...
| --- | --- | --- | --- |
| 1 | OpenNT Redirector owner-package recovery | Recover the complete `57:xx` family and its declared DEM/COMMAND remote-stream branches using original `VDMREDIR` ownership and explicit public-Win32 compatibility decisions. It may not recreate a second COMMAND child broker. | [Redirector proposal](etc/operations/proposal-opennt-redirector-owner-package-completion-001.md): package map, compatibility ledger and full-family regression. |

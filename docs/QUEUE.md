...
| --- | --- | --- | --- |
| 1 | Source-built guest low-memory runtime provenance | Attribute the runtime mutation and far indirect transfer from the original NTIO low-memory aperture reached after guest EXEC. First audit a selector-blind physical-write observation seam; it may not assign BOP/DOS/PSP semantics to bx-core or bx-mantle. | [Guest-runtime provenance proposal](etc/operations/proposal-source-built-guest-low-memory-runtime-provenance-001.md): source/ABI map, existing-seam assessment, smallest observation only if required, and checkpoint reconciliation. |

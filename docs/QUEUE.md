...
| --- | --- | --- | --- |
| 1 | OpenNT MVDM package-mirror rebootstrap and adapter-contract recovery | Rebuild `src/` as exactly thirteen components: canonical `opennt-mvdm-host`, `opennt-platform-abi`, complete load-only DOS/WOW16 mirrors, Bochs core/adapters, VDM-monitor adapter, multi-instance-safe session, cross-process broker and app. Establish the shared x86/x64 32-bit compatibility object space and one-session-per-process profile while selecting one package-scope OpenNT/OpenNT-4.5 union. The CLI remains `ntvdm.exe` direct COM/EXE/BAT/PIF launch. | [Rebootstrap proposal](etc/operations/proposal-opennt-mvdm-package-mirror-rebootstrap-001.md): union ledger, thirteen-root map, width/mapping contract, broker boundary, guest-image isolation, `src.old` quarantine and whole-tree audit. |

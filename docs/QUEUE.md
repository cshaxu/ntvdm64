...
| --- | --- | --- | --- |
| 1 | Source-built guest far-target publication and low-memory control-flow provenance | The completed writer-provenance package attributes the first `0x0A87` mutation to original NTDOS runtime. Map the original guest source/layout that publishes and consumes the resulting far target `5859:5F04`, then classify the current zero-RAM target without assigning BOP/DOS/PSP semantics to bx-core or bx-mantle. | The closure supplies the source/runtime fact; admission requires a source/image control-flow map and proof that any further observation is the smallest selector-blind mechanical seam. |

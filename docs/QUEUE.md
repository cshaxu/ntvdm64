...
| --- | --- | --- | --- |
| 1 | OpenNT MVDM package-mirror rebootstrap and adapter-contract recovery | Rebuild `src/` as exactly twelve components: canonical `opennt-mvdm-host`, `opennt-platform-abi`, separate complete load-only DOS/WOW16 guest mirrors, Bochs core/adapters, VDM-monitor adapter, session and app. Select one package-scoped union from complete OpenNT and OpenNT-4.5 MVDM trees; restore only audited app/session/Bochs foundations from `src.old`; recover same-shaped SoftPC, Win32 and VDM-monitor families. The CLI remains `ntvdm.exe` direct COM/EXE/BAT/PIF launch. | [Rebootstrap proposal](etc/operations/proposal-opennt-mvdm-package-mirror-rebootstrap-001.md): dual-baseline union ledger, twelve-root package map, guest-image boundary, `src.old` quarantine, adapter precedence and whole-tree audit. |

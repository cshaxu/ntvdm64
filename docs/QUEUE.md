...
| --- | --- | --- | --- |
| 1 | Guest vector/stack predecessor attribution | Classify the reached vector chain into guest code, native Bochs mechanics, or an unavailable historical host-composition edge before proposing any repair. | Retained stack/vector and current-observation logs, exact source/vector/IVT correlation, and a negative proof that adapter BOP providers and optional devices cannot own the transition. |
| 2 | Native reset/lifecycle composition decision | Reconsider a Bochs-native lifecycle correction only if a future independent audit disproves the current deferred POST/IVT evidence. | A new contradictory owner audit; the current POST/IVT evidence rejects admission. |

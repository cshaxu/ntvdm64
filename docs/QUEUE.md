...
| 2 | Selector-blind guest async-completion delivery | Recover the mechanical interrupt-delivery contract needed by original `int5c.asm` ANR completion, without giving bx-vdm CPU/PIC/IVT ownership. | Recorded async-boundary evidence plus a new machine-owner source/ABI proposal; only admissible after its selector-blind request/result contract is mapped. |

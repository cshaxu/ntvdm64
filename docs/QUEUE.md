...
| 3 | OpenNT historical-interface adapter convergence | Consolidate the same-shaped historical Win32, SoftPC/CCPU/SAS and VDM-monitor facades below `adapter-opennt`; preserve their original interface families as internal topology, keep `adapter-bop` and `adapter-bochs` separate, and prove the resulting x86/x64 build ownership before further MVDM recovery. | [OpenNT adapter convergence proposal](etc/operations/proposal-opennt-interface-adapter-convergence-001.md). |

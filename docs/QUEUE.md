...
| --- | --- |
| 0 | COMMAND native-child (`cmdExec32`) lifecycle — prove reached original `54:08/0A/0B` public-CreateProcess/re-entry/return behavior. |
| 1 | XMS, DPMI and DOS-extender execution — prove original XMS/A20/UMB/INT15 and DPMI/DPMI32 runtime paths. |
| 2 | Redirector and host-resource execution — prove original local resources and bounded public network bindings. |
| 3 | WOW16 bootstrap, callback and Win16 NE lifecycle — prove selected WOWEXEC/Win16 startup and return paths. |
| 4 | WOW16 service, UI and host integration matrix — recover selected WOW32 provider subpackages and public bindings. |
| 5 | Cross-family execution and control-broker closure — reconcile COMMAND/DOS/native-child/WOW lifecycle, events and cleanup. |
| 6 | SoftPC graphics workload integration — after the closed original-font/exact-client presentation package, run an already-proven DOS graphics workload through original video and app presentation. |
| 7 | Runtime matrix, x64 compatibility and product release — prove DOS/Win16 matrices, finish x64 profile work and package the product. |

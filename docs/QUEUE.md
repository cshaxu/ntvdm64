...
| --- | --- | --- | --- |
| 1 | Bochs intrusion convergence audit | Read-only reconciliation of every external-intrusion-register entry with actual source, generated build rules and final maps; classify each as retain, move to adapter, retain default-off, or remove. Freeze new Bochs semantic intrusions until the audit closes. | The current register, imported-source diff, generated derivative makefiles/maps and a per-entry disposition with evidence. |
| 2 | Holistic BOP host-service and machine-composition runtime closure | Resume the complete BOP route only after the intrusion audit: adapter-owned selector/service routing and provider disposition, plus adapter-owned machine-composition selection for admitted historical handler islands. | Closed intrusion audit; source/ABI ownership map; one bounded observation with no selector semantics in Bochs and no claim beyond observed execution. |

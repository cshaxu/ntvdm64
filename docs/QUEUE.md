...
| --- | --- | --- | --- |
| 1 | OpenNT COMMAND launch, PIF and child-lifecycle recovery | Retain target-first CLI admission, then restore original PIF startup/configuration, COMMAND child/redirection lifecycle and explicit WOW/diagnostic boundaries without reimplementing COMMAND in the CLI. | [COMMAND launch/PIF proposal](etc/operations/proposal-cli-run-entry-and-opennt-pif-handoff-001.md): source-first recovery, sequential whole-package delivery and format matrix. |

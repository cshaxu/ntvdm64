...
| --- | --- | --- | --- |
| 1 | Holistic BOP host-service and machine-composition runtime closure | Continue the complete BOP route only after **all repair S tasks in the current Bochs/adapter boundary-repair package close**. The preceding audit is necessary but insufficient: the repaired diff, register reconciliation, minimal rebuild, and boundary regressions must also be closed. The package resumes adapter-owned selector/service routing and provider disposition, plus adapter-owned machine-composition selection for admitted historical handler islands. It is the next T package, not a deferred S of the active repair package. | Closed complete boundary-repair package (not merely its audit); source/ABI ownership map; one bounded observation with no selector semantics in Bochs and no claim beyond observed execution. |

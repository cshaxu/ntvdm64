...
| --- | --- | --- | --- |
| 1 | No-external-dependency OpenNT BOP closure | In one T, revalidate and close every tracker item whose declared local lifecycle has no external machine, legacy/private-API, or unresolved external owner dependency. It includes no/low-dependency leafs **and** BOP/dependency chains whose prerequisites are entirely internal to the frozen slice. The current Td freeze is 75 BOP + 30 dependency rows, all presently local-pass; admission is a no-bypass recovery audit, not a pretense that they remain unimplemented. | [No-external-dependency proposal](etc/operations/proposal-bop-low-dependency-source-closure-001.md): eligibility/DAG freeze, source-first owner grouping and whole-slice regression. |

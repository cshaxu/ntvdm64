...
| --- | --- | --- | --- |
| 1 | Selector-blind protected virtual-access and guest-linear mapping substrate | Recover the remaining generic prerequisite identified by the closed context substrate: a fault-contained copied protected virtual-range action and a declared guest-linear mapping identity. No BOP/DOS/OpenNT semantics may enter bx-core or bx-mantle; any external-code patch first requires an exception record. | The closed protected-substrate decision and `BOP-DEPENDENCY-117` establish the prerequisite for the entire DPMI package. |

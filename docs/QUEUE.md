...
| --- | --- |
| 1 | Explicit interactive COMMAND profile — `ntvdm.exe command.com` submits one original-shaped child `COMMAND.COM` target record through the resident control shell. Recover and prove the original non-first child path: copyright header, `DoReEnter → Do16BitPrompt → DOS CON`, guest-owned keyboard, Ctrl+C, `exit`, parent return and product session end. `ntvdm.exe` without a target remains non-interactive. Character mode uses one Console; a presentation window is entered only for Alt+Enter, original graphics/fullscreen selection, or an admitted original PIF disposition. No host command parser, host prompt reader, guest-media change or cross-process BaseSrv broker is permitted. See `proposal-explicit-interactive-command-profile-001.md`. |

...
| --- | --- | --- | --- |
| 1 | Seven-component source-layout reorganization | Inventory and move the present transitional source tree into the declared `bx-*`, `opennt-*`, `adapter-*`, and `app` owners without altering product behavior. | [Component-reorganization proposal](etc/operations/proposal-seven-component-source-layout-reorganization-001.md): owner manifest, `git mv` plan, README registers and per-module build closure. |

...
| --- | --- | --- | --- |
| 1 | Public Win32/CRT source-shaped binding and facade recovery | Consume every final public Win32/CRT leaf result from the **corrected** first-degree audit. For each unique original interface identity, retain the original declaration/call shape and prove one modern disposition: direct public binding, same-shaped `adapter-mvdm-host-out/win32` facade, or explicit source-shaped unavailable result. No private product API is silently substituted or reimplemented. | Corrected zero/first-degree ledgers, final public-leaf resolution ledger, declaration/ABI evidence and a complete call-site-to-interface mapping matrix. |

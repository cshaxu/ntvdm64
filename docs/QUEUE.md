...
| --- | --- | --- | --- |
| 1 | Selector-blind protected-mode substrate recovery | Audit and recover only the generic machine operations needed by an original DPMI source mirror: protected selector:offset checked access, atomic protected context/IRET transition, and guest-linear mapping identity. No BOP/DOS/OpenNT semantics may enter bx-core or bx-mantle; any external-code patch first requires an exception record. | [Substrate proposal](etc/operations/proposal-selector-blind-protected-mode-substrate-001.md) and `BOP-DEPENDENCY-117` establish the prerequisite for the entire DPMI package. |
| 2 | OpenNT DPMI protected-mode owner-package recovery | Recover one declared DPMI profile as a package only after candidate 1 proves its protected-mode, LDT/IDT, exception/IRET and memory prerequisites. | [DPMI proposal](etc/operations/proposal-opennt-dpmi-owner-package-completion-001.md): whole-profile source/ABI/failure closure. |

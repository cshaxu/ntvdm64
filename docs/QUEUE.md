...
| --- | --- | --- | --- |
| 1 | Holistic BOP host-service and machine-composition runtime closure | Continue the complete BOP route only after the **current Bochs/adapter boundary repair package closes**: adapter-owned selector/service routing and provider disposition, plus adapter-owned machine-composition selection for admitted historical handler islands. This is the next T package, not a deferred S of the active repair package. | Closed boundary-repair package; source/ABI ownership map; one bounded observation with no selector semantics in Bochs and no claim beyond observed execution. |

...
tests and package closure.  It may not restart repository-wide BFS, rename
...
| --- | --- |
| 1 | Complete adapter ownership migration — move every non-machine semantic currently mixed into `adapter-mvdm-host-out/softpc` to one architecture-approved final owner, with a full owner ledger and no dual implementation or residual forwarding path. This is a behavior-preserving prerequisite for DPMI32/WOW32 recovery. See [the migration proposal](etc/operations/proposal-adapter-ownership-migration-001.md). |
| 2 | Complete DPMI32 and Win16/WOW32 provider admission and bootstrap — retain the complete DPMI32 owner cohort and bring all 77 active original WOW32 bodies, 105 declaration carriers, original tables/resources and one original-shaped provider image into the formal graph; recover `53:00`–`53:18` and `51h → W32Init → W32Dispatch`. Runtime evidence remains subdomain-by-subdomain; a private system call stays an explicit boundary rather than omitting its original caller. |
| 3 | Single-process NTVDM compatibility matrix — run selected DOS, extender, Redirector and COMMAND-native-child workloads against completed original-owner contracts; add Win16 rows only after the WOW bootstrap and service packages complete. Reopen only the immediate original owner proven by a failure. |
| 4 | WOW16 service, UI and host integration activation — prove the admitted WOW32 provider's selected USER, GDI, shell, multimedia and network call forms through public APIs while retaining exact USER/CSRSS/Win32k private-boundary failures. |
| 5 | Cross-process broker closure — recover only required BaseSrv-style registration, command-queue, notification and disconnect-cleanup semantics through public IPC. It cannot own guest, COMMAND, SoftPC or WOW behavior. |
| 6 | Multi-process matrix, x64 compatibility and product release — prove the brokered multi-process profile, paired x86/x64 behavior where runnable, portable media layout and release packaging. |

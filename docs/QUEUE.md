...
| --- | --- | --- | --- |
| top | Project code filename identity normalization | Remove `ntdos64`, `ntvdm64`, `bx`, and `bochs` branding/backend prefixes from project-authored and mechanical-adaptation code filenames; keep all six source mirrors and their overlays on their upstream-relative filenames. | [Filename-normalization proposal](etc/operations/proposal-project-code-filename-normalization-001.md): scoped manifest, `git mv` history, reference repair and formal graph closure. |
| 1 | Bochs component identity and mirror-overlay convergence | Rename `bx-mantle`/`bx-core` to `adapter-bochs`/`bochs-core`; enforce the private-overlay rule; restore upstream Bochs material to its mirror and move oversized divergences in Bochs and OpenNT host mirrors into dedicated overlays. | [Bochs convergence proposal](etc/operations/proposal-bochs-component-identity-overlay-convergence-001.md): owner, provenance, mirror-measurement and formal-build closure. |

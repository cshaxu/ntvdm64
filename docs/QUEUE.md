...
| 2 | Canonical OpenNT source supply | Select the package-scope OpenNT/OpenNT-4.5 union and establish `opennt-platform-abi`, complete load-only DOS/WOW16 mirrors and the canonical non-guest/non-tool `opennt-mvdm-host` supply. | [Source-supply proposal](etc/operations/proposal-rebootstrap-opennt-source-supply-001.md): provenance, mirror and host/guest isolation closure. |

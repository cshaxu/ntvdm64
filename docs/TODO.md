...
| P1 | Admit and complete the deferred Overlay mutation-profile BOP/provider/session/CLI work after the Direct/Readonly matrix closes; retain and revalidate its existing source and evidence rather than replacing it. Virtual is retired from the product profile contract: retain its current code/evidence unless it demonstrably blocks Direct, Readonly or Overlay progress; then permit only a minimal, evidence-backed removal, and add no new Virtual behavior. | A separately admitted Overlay owner-package plan with an updated OpenNT source/ABI/failure map, current-state audit of retained Overlay code, and Direct/Readonly/Overlay regression matrix. |

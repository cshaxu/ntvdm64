...
| P1 | Resume the frozen Overlay/Virtual mutation-profile BOP/provider/session/CLI work after the Direct/Readonly T221 matrix closes; retain and revalidate all existing Overlay/Virtual source and evidence rather than replacing it. | A separately admitted owner-package plan with an updated OpenNT source/ABI/failure map, current-state audit of retained Overlay/Virtual code, and full profile regression matrix. |

...
completed initialization, processed the generated AUTOEXEC batch, reached
`Do16BitPrompt`, or consumed a key.  The next bounded owner is therefore the
original second-`COMMAND.COM` initialization and AUTOEXEC execution sequence,
not a keyboard repair.

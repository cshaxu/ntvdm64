...
| `redir_session` and imported `cmdredir.c` | COMMAND/Redirector boundary | HOST-03, HOST-08 | Local completion state only | Retain boundary. A later Redirector package must preserve the original loader/protocol shape through a modern source-shaped facade; this row does not imply `VDMREDIR` availability today. |
...
3. `host_simulate`, `sas_*`, CCPU macros and `NtVdmControl` spellings are
   ...
   specific `MACH-*` source-shaped typed facade or the explicitly deferred
   remainder stated in the catalog.

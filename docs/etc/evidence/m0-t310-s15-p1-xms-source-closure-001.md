...
They now compile as one formal MSVC `/MT` static archive on both x86 and x64.
The formal graph also compiles the selected same-shaped
...
pointers, which cannot be preserved on x64 or at the guest-memory boundary.
...
behavior evidence without selecting the excluded direct-pointer backend.

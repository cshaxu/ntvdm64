...
/* Host context seam: the historical PEB ConsoleHandle lookup is not a stable
 * modern-process ABI. The default-disabled config gate leaves this unresolved. */
extern ULONG ntdos64_legacy_console_identity(VOID);

/* Default-disabled checkpoints in the original config() call order. */
#ifdef NTDOS64_HOST_MAIN_CONFIG_TRACE
extern void ntdos64_host_applinit_trace_checkpoint(int stage);
#define NTDOS64_HOST_MAIN_CONFIG_TRACE_STAGE(stage) \
    ntdos64_host_applinit_trace_checkpoint(stage)
#else
#define NTDOS64_HOST_MAIN_CONFIG_TRACE_STAGE(stage) ((void)0)
#endif
...
GLOBAL VOID config( VOID )
{
...
#ifdef NTDOS64_HOST_MAIN_CONFIG_TRACE
    ULONG config_trace_console_identity;
    DWORD config_trace_thread_id;
    DWORD config_trace_process_id;
#endif

    NTDOS64_HOST_MAIN_CONFIG_TRACE_STAGE(701);
    NTDOS64_HOST_MAIN_CONFIG_TRACE_STAGE(7011);
        ...
        */
        Title[0] = '\0';
        NTDOS64_HOST_MAIN_CONFIG_TRACE_STAGE(711);
        if (!VDMForWOW)  {
            NTDOS64_HOST_MAIN_CONFIG_TRACE_STAGE(712);
            if (!DosSessionId && !GetConsoleTitle(Title, MAX_PATH))
                Title[0] = '\0';
            NTDOS64_HOST_MAIN_CONFIG_TRACE_STAGE(713);
#ifdef NTDOS64_HOST_MAIN_CONFIG_TRACE
            config_trace_console_identity = ntdos64_legacy_console_identity();
            NTDOS64_HOST_MAIN_CONFIG_TRACE_STAGE(7131);
            config_trace_thread_id = GetCurrentThreadId();
            NTDOS64_HOST_MAIN_CONFIG_TRACE_STAGE(7132);
            config_trace_process_id = GetCurrentProcessId();
            NTDOS64_HOST_MAIN_CONFIG_TRACE_STAGE(7133);
            sprintf(UniqueTitle, "ntvdm-%lx.%lx.%lx",
                   config_trace_process_id, config_trace_thread_id,
                   config_trace_console_identity);
#else
            sprintf(UniqueTitle, "ntvdm-%lx.%lx.%lx",
                   GetCurrentProcessId(), GetCurrentThreadId(),
                   ntdos64_legacy_console_identity());
#endif
            NTDOS64_HOST_MAIN_CONFIG_TRACE_STAGE(714);
            SetConsoleTitle(UniqueTitle);
            NTDOS64_HOST_MAIN_CONFIG_TRACE_STAGE(715);
            }
        else {
            strcpy(Title, "Hidden Console of WOW VDM");
            NTDOS64_HOST_MAIN_CONFIG_TRACE_STAGE(716);
            }
        NTDOS64_HOST_MAIN_CONFIG_TRACE_STAGE(702);
	...
	NTDOS64_HOST_MAIN_CONFIG_TRACE_STAGE(703);
	if (GetNextVDMCommand(&GetPIF)) {
	    NTDOS64_HOST_MAIN_CONFIG_TRACE_STAGE(704);
	...
	NTDOS64_HOST_MAIN_CONFIG_TRACE_STAGE(705);
	    }
	...
	NTDOS64_HOST_MAIN_CONFIG_TRACE_STAGE(706);

        sas_term ();
	NTDOS64_HOST_MAIN_CONFIG_TRACE_STAGE(707);
	...
	NTDOS64_HOST_MAIN_CONFIG_TRACE_STAGE(708);

...
extern void ntdos64_reset_trace_checkpoint(int stage);
#define NTDOS64_RESET_TRACE_STAGE(stage) ntdos64_reset_trace_checkpoint(stage)
#else
#define NTDOS64_RESET_TRACE_STAGE(stage) ((void)0)
#endif
 ...
 */
	...
	NTDOS64_RESET_TRACE_STAGE(4);
	sas_fills (0, '\0', 640L * 1024L);
	NTDOS64_RESET_TRACE_STAGE(5);

	/* Now set up the interrupt vector table. */
	setup_ivt();
	NTDOS64_RESET_TRACE_STAGE(6);
	...
	cmos_init();
	cmos_post();
	NTDOS64_RESET_TRACE_STAGE(8);

	ppi_init();
	ppi_get_switches(&low_switches,&high_switches);
	NTDOS64_RESET_TRACE_STAGE(9);
...
/* lim stuff has been moved to config.c */
	...
	disk_post();
	NTDOS64_RESET_TRACE_STAGE(11);

#ifdef	NTVDM
	/* On NT do this after everything else is done */
        host_reset();
	NTDOS64_RESET_TRACE_STAGE(12);
        q_event_init();
#endif

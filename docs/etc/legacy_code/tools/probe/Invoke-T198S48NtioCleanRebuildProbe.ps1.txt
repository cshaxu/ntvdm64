...
$coreProbe = Join-Path $repository 'tools\probe\Invoke-T197S6MinimalMachineLinkProbe.ps1'
$legacyResponse = Join-Path $repository 'artifacts\build\t198-s25-native-command-r1\link.rsp'
$compositionManifest = Join-Path $repository 'tools\build\t198-s50-bx-vdm-composition-manifest.json'
...
$includes = @('src', 'refs\bochs', 'refs\bochs\instrument\stubs', 'src\bx-core', 'src\bx-core\cpu', 'src\bx-mantle', 'src\bx-vdm', 'src\cli') | ForEach-Object { '/I "' + (Join-Path $repository $_) + '"' }

...
$headerPath = Join-Path $repositoryRoot 'src/adapter-bochs/finite_run.h'
$sourcePath = Join-Path $repositoryRoot 'src/adapter-bochs/finite_run.cc'
...
foreach ($pattern in @(
        'bx_machine_minimal_machine_c machine;', 'machine\.initialize\(0x100000, 0x100000\)',
...
Write-Output 'bx-ntvdm-finite-run-boundary: selector-blind finite machine invariants verified'

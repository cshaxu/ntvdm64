...
$machine = Get-Content -LiteralPath $portSpaceSource -Raw
...
foreach ($pattern in @('bx_devices_c::bx_devices_c\(\)', 'bx_devices_c::~bx_devices_c\(\)',
    ...
    if ($machine -notmatch $pattern) {
        throw "Missing BX-MACHINE-065 original member in machine: $pattern"
    }
    if ($devices -match $pattern) {
        throw "BX-MACHINE-065 member remains in full device source: $pattern"
    }
}

...
foreach ($required in @('BX_NTVDM_PORT_ACTION_V1_READ8','BX_NTVDM_PORT_ACTION_V1_WRITE8','bx_devices.inp','bx_devices.outp','bx_ntvdm_port_action_v1_lifecycle_active')) {
  if ($header.IndexOf($required,[StringComparison]::OrdinalIgnoreCase) -lt 0 -and $source.IndexOf($required,[StringComparison]::OrdinalIgnoreCase) -lt 0) { throw "Missing BX-MACHINE-083 invariant: $required" }
}
$code=[regex]::Replace($source,'(?s)/\*.*?\*/|//.*?(\r?\n|$)','')
foreach ($forbidden in @('BOP','OpenNT','DOS','WOW','DEM','selector','callback','plugin','firmware','void *','BX_CPU')) {
  if ($code.IndexOf($forbidden,[StringComparison]::OrdinalIgnoreCase) -ge 0) { throw "Forbidden BX-MACHINE-083 dependency: $forbidden" }
}

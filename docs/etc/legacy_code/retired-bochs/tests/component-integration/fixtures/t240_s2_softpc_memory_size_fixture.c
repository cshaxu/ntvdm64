...
int main(void)
{
    ...
    initialize(&event);
    if (!runtime_softpc_memory_size_generic_ud_recognizes(&event) ||
    ...
    if (!runtime_softpc_memory_size_generic_ud_dispatch(&event, &outcome) ||
    ...
    if (runtime_softpc_memory_size_generic_ud_recognizes(&event)) return 5;

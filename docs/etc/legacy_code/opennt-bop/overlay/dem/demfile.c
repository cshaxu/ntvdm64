...
VOID VDDAssociateNtHandle (PVOID pSFT, HANDLE hFile, WORD wAccess)
{
PDOSSFT pDosSFT = (PDOSSFT)pSFT;

    pDosSFT->SFT_Mode = wAccess&0x7f; // take out no_inherit bit
    pDosSFT->SFT_Attr = 0;                     // Not used.
    pDosSFT->SFT_Flags = (wAccess&0x80) ? 0x1000 : 0; // copy no_inherit bit.
    pDosSFT->SFT_Devptr = (ULONG) -1;
    pDosSFT->SFT_NTHandle = (ULONG) hFile;

...
| BX-CORE-093 | 2026-08-23: T253 S3 proves OpenNT async named-pipe completion requires a host worker to request the original IRQ14, but an arbitrary host thread may not mutate the native PIC/CPU directly. Standing owner authority permits the smallest generic mechanical ingress. | Adopted `src/bx-core/cpu/event.cc`; project-owned `src/bx-mantle` fixed physical-IRQ queue. | At an existing CPU async-event boundary only, core drains at most one mantle-owned queued physical IRQ and calls the already native PIC request path. Core receives only a line `0..15`; the queue is synchronization-safe and contains no guest or host pointer. | Registered before patch. No BOP, selector, DOS, OpenNT, Redirector, vector-purpose, callback, guest address, handle, worker, host API, provider or CLI term may enter bx-core. No direct vector injection, IVT access, CPU/register/flag mutation, altered PIC priority/mask behavior, device configuration or default behavior is allowed. The unarmed queue leaves the original event path unchanged. Require cross-thread enqueue, disabled/no-request, masked/deferred, normal PIC acknowledgement, queue-overflow and lifecycle-reset tests. Reject if more than fixed physical IRQ ingress is needed. |

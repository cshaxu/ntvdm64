...
| BX-TRACE-060 | 2026-08-12: T188 S1--S4 proves the generic adapter ingress already receives `54:0C`, static v5 inputs succeed, and only private accumulated session state remains unclassified | `src/bochs/cpu/exception.cc`; focused state diagnostic | Retained default-off generic adapter lifecycle diagnostic after pass-through | Still selector-blind and outside BOP identity observation; S5 retains it pending S6 source/map verification. |
| BX-TRACE-061 | 2026-08-12: T189/S1 reduces the real `54:0C` decline to a finite adapter compound-chain uncertainty | `src/bochs/cpu/exception.cc`; existing copied diagnostic state | Retained default-off generic first-failure category after pass-through | Still selector-blind and outside BOP identity observation; S5 retains it pending S6 source/map verification. |

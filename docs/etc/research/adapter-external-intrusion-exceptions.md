...
`src/bochs/` and `src/opennt/`. An entry is required before a patch is made.
...
| BX-ABI-054 | 2026-08-12: T118 source-proves BOP `02h` consumes three bytes then enters guest IRET continuation | `src/bochs/cpu/exception.cc`; named native derivative build script | Default-off application of a component-owned generic handled result: resume at copied fault RIP plus three, with no CPU/flags delta | T119 implemented and source-built this exact path. The T119 r3 map resolves the original `unexpected_int` and `inb`/`outb`/`c_sas_store` wrappers in the machine component; boundary tests pass. Runtime remains T120-only. Only the separately admitted machine component may produce handled, and Bochs remains selector-blind. |
| BX-TRACE-055 | 2026-08-12: T130 reaches the admitted machine-composition path but its boolean result cannot distinguish decline from failed native mechanics | `src/bochs/cpu/exception.cc`; `src/bx-ntvdm-machine-composition/bx_ntvdm_machine_bop_v1.c`; seam/component tests | Replace the internal boolean with the pre-existing fixed component result enum and append a generic non-decline outcome number; resume remains exclusively the existing handled value | Registered before implementation. Bochs receives no selector, marker, BOP, OpenNT, DOS, adapter or device detail; the component still owns classification and the original handler remains unchanged. |
| BX-TRACE-056 | 2026-08-12: T134 proves a repeated real-mode prefetch-limit fault but not the instruction that advanced into it | `src/bochs/cpu/cpu.cc`; a separately named derivative shim/object | Default-off, fault-only record at the original `EIP > CS.limit` branch of real mode: current CS, `PREV_RIP`, current EIP and the already-read limit | Registered before implementation. It must not read guest memory, decode an instruction/BOP/OpenNT term, branch on guest address, change CPU/exception/memory/device state, alter fault delivery, add a CPU loop, or become a product feature. It records only existing CPU fields to distinguish the immediately preceding instruction position from the detected invalid fetch. |
...
| BX-TRACE-046 | 2026-08-11: r22 records a post-close stack fault, but the generic BOP log omits the existing fault position needed to correlate the close call with source | `src/bochs/cpu/exception.cc`; BOP listener boundary test | Append the already passed generic `fault_rip` to every observed BOP log | Registered before implementation. The listener remains selector-independent and void; it reads no guest memory beyond existing diagnostics, takes no adapter result, and changes no CPU, memory, device, guest or host state. Review/remove after the reached BOP continuations are source-correlated. |
...
| BX-TRACE-048 | 2026-08-11: r30 proves the read ABI correction reaches `61A7`, but the next ordinary `LeaveDos` stack transfer remains unobserved; native debugger/GDB/instrumentation are disabled and debugger is incompatible with chained handlers | `src/bochs/cpu/data_xfer16.cc`; focused static boundary test | Default-off, real-mode-only logs within the existing `MOV_SwEw` and `MOV_GwEwM` handlers when their destination is SS or 16-bit SP. Each record contains only current CS:RIP and old/new register/selector values. | Registered before implementation. It must not branch on BOP/OpenNT/DOS/adapter state, inspect guest memory beyond the original MOV operand fetch, alter result/exception/device/CPU state, replace the CPU loop, enable debugger/instrumentation, or become a product feature. Remove after the `LeaveDos` transition is classified. |
| BX-TRACE-049 | 2026-08-11: r34 source correlation establishes that the terminal state enters `Redisp` through original NTDOS stack switching, but the low-memory vector that reaches `0000:019D` is unproven | `src/bochs/cpu/exception.cc`; focused static boundary test | Default-off, real-mode-only log in the original `real_mode_int` path after its existing IVT word reads. It records vector, pre-delivery CS:IP/SS:SP, and the already-read IVT target CS:IP. | Registered before implementation. It must not add IVT/RAM reads, branch on vector/BOP/OpenNT/DOS/adapter state, alter CPU/exception/memory/device state, change interrupt delivery, enable debugger/instrumentation, or become a product feature. Remove after the terminal vector/target is source-correlated. |
| BX-TRACE-050 | 2026-08-11: r35 proves `INT 1` reaches NTIO `intret`, which merely IRETs a TF-bearing frame; the origin frame remains unknown | `src/bochs/cpu/ctrl_xfer16.cc` | Default-off, real-mode-only log after original `IRET16` pops IP, CS and FLAGS, only if that already-popped FLAGS word contains TF. It records pre-IRET CS:IP/SS:SP and the three popped words. | Registered before implementation. It must not add stack/RAM reads, alter the popped values, flags, stack pointer, branch target, exception/device state, branch on BOP/OpenNT/DOS/adapter state, or become a product feature. Remove after the first TF-bearing frame is source-correlated. |
| BX-TRACE-051 | 2026-08-11: r36 source correlation proves the first TF frame follows original guest pseudo-ROM INT10, but does not expose the saved machine INT10 far-jump target | `src/bochs/cpu/ctrl_xfer32.cc` | Default-off, real-mode-only log in original immediate far-jump (`JMP_Ap`) after its existing operand decode and before its original CS:EIP assignment. It records current CS:IP and decoded target CS:IP. | Registered before implementation. It must not add RAM/stack reads, alter operands, target, CPU flags/state, exception/device/IVT/ROM behavior, branch on BOP/OpenNT/DOS/adapter state, or become a product feature. Remove after `host_int10` target provenance is classified. |
...
| BX-ABI-058 | 2026-08-12: T176 S21 proves original SoftPC machine BOP `06h` (`illegal_op_int`) needs only copied real-mode SS:ESP, checked ordinary-RAM byte reads/writes, and its original report-then-continue host outcome before it calls the already admitted `unexpected_int` island | `src/bochs/cpu/exception.cc`; `src/bx-ntvdm-machine-composition/`; named native derivative build script | Extend the existing default-off, selector-blind mechanics context with copied execution-mode/SS/ESP values, a checked ordinary-RAM byte read callback and an append-only bounded report callback. The machine component alone selects `06h`, invokes unchanged `illegal_op_int`, supplies its exact CCPU/SAS compatibility imports during that synchronous call, and returns only the pre-existing generic result enum. | Registered before implementation. Bochs must not contain selector/BOP/OpenNT/DOS/SoftPC/CCPU semantics, access a raw CPU pointer, alter exception delivery or resume logic, or provide host UI. The report is diagnostic-only; failed read/write or non-real-mode requests decline/fail without resuming. All selectors except separately admitted `02h` and `06h` remain declined; adapter ingress and host-service providers remain uninvolved. |
| BX-TRACE-059 | 2026-08-12: T186 records a repeated boot-file BOP but omits DX, which is required to evaluate its already-source-defined `DS:DX` destination | `src/bochs/cpu/exception.cc`; existing BOP-listener static test | Append the already copied low 16 bits of EDX to the existing default-off BOP catalog listener line. | Registered before implementation. No new listener, callback, ABI, CPU/memory read, selector/service branch, exception result, guest mutation, build recipe or runtime observation is authorized. The listener keeps its existing default-off macro and only formats a field already present in its copied CPU snapshot. |
| BX-TRACE-060 | 2026-08-12: T188 S1--S4 proves the generic adapter ingress already receives `54:0C`, static v5 inputs succeed, and only private accumulated session state remains unclassified | `src/bochs/cpu/exception.cc`; one focused static boundary test; named one-object derivative | Behind a new default-off macro within the existing generic adapter interceptor, copy and append only the adapter-owned fixed-width lifecycle snapshot after the unchanged runtime dispatch returns `PASS_THROUGH`. | Registered before implementation. It may not inspect instruction bytes, selector/service, guest memory, payload, pointer or session layout; it may not affect dispatch/result/resume/exception state. One fresh watchdog-bounded observation is separately required; no retry. |
| BX-TRACE-061 | 2026-08-12: T189/S1 reduces the real `54:0C` decline to a finite adapter compound-chain uncertainty | `src/bochs/cpu/exception.cc`; existing copied diagnostic state and focused static test | Append one adapter-owned first-failure category to the existing BX-TRACE-060 lifecycle line after unchanged pass-through. | Registered before implementation. Bochs must not inspect selector/service/bytes, guest state/memory, payload, path, session layout or result; it only formats the copied category and cannot alter dispatch, resume or exception flow. |

...
| --- | --- | --- | --- | --- |
| BX-MANTLE-066 | 2026-08-13: T197 S6 r9 whole-core link proves that `pc_system.cc` combines CPU-reset/time mechanics with GUI/full-device teardown, while finite SIM fixes triple-fault reset false | `src/bx-mantle/pc_system.cc`; new `src/bx-mantle/minimal_product_shell.cc` | Move `bx_pc_system_c::Reset` and `exit` out of the common PC-time object; retain CPU-only reset mechanics and reject hardware reset requiring unadmitted devices; provide finite no-GUI parameter text/exit symbols only | Approved for implementation; no GUI, device init/reset/exit, plugin, BOP, DOS, OpenNT or adapter behavior may enter. |
| BX-MANTLE-065 | 2026-08-13: T197 S6 whole-CPU5 link probe r8 reduces native core closure to the empty-port object lifetime and its original no-device dispatch methods | `src/bochs/iodev/devices.cc`; `src/bx-mantle/minimal_port_space.cc` | Move exact `bx_devices_c` constructor, destructor, `init_stubs`, `inp` and `outp` definitions into the mantle empty-port source | Approved for implementation; no device init/reset/exit, plugin loading, BOP, DOS, OpenNT or adapter behavior may move. |
...
| BX-MANTLE-063 | 2026-08-13: T197 S6 symbol audit proves the minimal machine needs `bx_devices` only for its already extracted empty-port members | `src/bochs/iodev/devices.cc`; `src/bx-mantle/minimal_port_space.cc` | Move the single `bx_devices_c bx_devices` global definition to the mantle port-space object | Implementing; no device initializer, plugin, timer or handler behavior moves. |
| BX-MANTLE-062 | 2026-08-12: owner directs `bx-core`/`bx-mantle` physical extraction, with mixed links decided individually; confirms CPU5/Pentium MMX bare-machine contract | `src/bochs/gui/siminterface.h`; original `paramtree.*` and `logio.cc` moved to `src/bx-mantle/`; `bx_ntvdm_minimal_sim.*` | Mantle owns a finite original typed parameter tree for CPU5/Pentium MMX; SIM product behavior remains excluded | Implementing: no configuration parser, GUI/plugin/device path or product build claim is authorized by this entry. |

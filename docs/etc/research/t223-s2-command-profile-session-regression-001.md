# T223 S2 COMMAND Direct/Readonly Profile-Session Regression

## Question

Can the selected original COMMAND bootstrap/environment/config component
(`54:01/02/04/05/0C/0D/0F`) traverse independent bound Direct and Readonly sessions without a
...
1. Assemble independent fresh `bx_ntvdm_boot_namespace_composition_v1` sessions using
   admitted Direct and Readonly mutation profiles. Each profile registers COMMAND
   session-context and host-global classes generically; it does not name a selector
   or service.
2. Assert both classes resolve to their selected policy (`DIRECT_HOST` or
   `REJECT_READONLY`) before BOP ingress.
3. Route each normal COMMAND session through `54:02`, `54:0F` (insufficient
...
4. Build and run with the declared MSVC x64 `/MT` closure:
   ...
   powershell.exe -ExecutionPolicy Bypass -File tools/probe/Invoke-T198S16BootNamespaceVdmX64Probe.ps1 `
     ...
     -BuildRoot O:\repos.hobby\ntdos64\build\M0-T223-S2\011-command-bootstrap-profile-session `
     -Fixture command-bootstrap-profile-session
   ```
...
- The Direct/Readonly profile-session fixture linked and ran with `linkExitCode=0` and
  `runExitCode=0`.
...
use independent COMMAND composition sessions under Direct and Readonly, and each
profile is resolved before routing rather than silently selecting another profile.

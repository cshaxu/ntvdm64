...
| --- | --- | --- |
| X01 | Can the real desktop and required native controls have source-correct guest identity, pointer relocation and retirement? First prove every rebased heap `S = C + D` relation is non-wrapping and rejects before mutation while `TEB.pDeskInfo` remains client form; then observe original guest desktop/handle reads, not a dummy object success. | S2 |

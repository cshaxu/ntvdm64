...
| A03 | In progress | Exact original owner and four-rung disposition for every external edge; finite import/expansion/removal list with hashes and line counts. | The [autonomous binding inventory](../evidence/m0-t422-s1-autonomous-binding-inventory.md) fixes the 21-file/2,977-line target-local baseline and per-file receiver. Complete function-level private/public-operation dispositions and removal tests before closure. |
| A04 | In progress | Complete producer/consumer/update matrix for all guest-visible fields and object relationships. | The [direct-data owner matrix](../evidence/m0-t422-s1-direct-data-owner-matrix.md) settles current producers, zero-producer fields and publication order. Expand it to every required object relationship and complete the feasibility evidence before closure. |

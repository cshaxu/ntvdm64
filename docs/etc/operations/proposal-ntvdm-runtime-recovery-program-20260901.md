...
2. **COMMAND built-in and batch lifecycle:** selected `/C` built-in/batch
   behavior and COMMAND return; no external DOS EXEC claim.
3. **DOS `.COM`/MZ `.EXE` execution and parent return:** original `$Exec`,
...
5. **SoftPC conventional-device and DOS workload matrix:** CPU40, memory,
   timer/PIC/PIT, keyboard, filesystem and video contracts reached by DOS.
6. **XMS, DPMI and DOS-extender execution:** original XMS/A20/UMB/INT15 and
   DPMI/DPMI32 runtime paths with mapping-manager identity boundaries.
7. **Redirector and host-resource execution:** local VDMREDIR first, then
...
10. **Cross-family execution and control-broker closure:** COMMAND/DOS/native
    child/WOW lifecycle, event and cleanup; public IPC only when reached.
11. **SoftPC graphics presentation and workload integration:** selected DOS
    graphics workload through original video and app presentation.
12. **Runtime matrix, x64 compatibility and product release:** selected DOS and

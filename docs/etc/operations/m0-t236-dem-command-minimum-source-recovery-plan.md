...
| S2 | COMMAND historical worker and host-broker recovery | Active: P1 compiled the imported worker synchronously and is retained as source-admission evidence. P2 recovers its detached-worker topology through a fixed `PENDING` continuation, opaque host-handle IDs and controlled selector-blind machine suspension/resume. | One `bx-vdm` session worker is the CCPU-equivalent composition; Bochs remains the SoftPC-equivalent machine engine. Private broker protocol, multi-session and raw host handles remain excluded. |

...
| S3 | Recover the local `AL=0` DOS-binary load/child/exit path by enabling only source-proven machine or existing DEM prerequisites. | Original `$Exec`, `$CREATE_PROCESS_DATA_BLOCK`, `$Exit/$Abort` execute in guest ownership; error exits retain source behavior. |

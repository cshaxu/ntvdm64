...
| Admission And Approval | The owner-approved T225 objective and ongoing instruction to continue select the S22-transferred machine prerequisite. [S22 closure](history/m0-t225-s22-closure-20260817.md) proves the reached original INT 06 boundary and excludes adapter-service repair. On 2026-08-17 the owner explicitly approves this and future analogous formal module-boundary extensions required to preserve OpenNT source-first ownership; `opennt-host` is admitted here as the fifth formal static library, not as a bx-vdm substitute. |
...
| Files And ABI Surface | `src/bx-mantle` native PIC/machine-mechanics and lifecycle surfaces; `src/bx-vdm` machine-plane selector/result integration only; `src/opennt/local/compat/host` fixed-width source-derived INT 06 provider; formal five-library manifest/generator, focused tests, evidence and Status. The cross-layer ABI carries copied scalar records/statuses only; no raw callback, CCPU/SAS object, host pointer, or CRT-owned value. |
...
| Verification | First complete the four-rung source/ABI/failure map and direct versus deferred PIC disposition. Then compile the admitted formal five-library Ninja closure under MSVC x64 `/MT`, run focused positive/negative mechanics tests, and make one bounded Direct/Readonly native observation only after the whole machine package is linked. |
...
| Stop Conditions | Missing original handler detail, PIC lifecycle requiring unclassified Bochs product dependencies, a necessary new device beyond PIC, a cross-ABI object proposal, a required guest-byte mutation beyond the original saved-frame/BDA contract, or any attempt to route `06h` through a general host-service plane. Record the issue; continue independent source/ABI mapping; do not invent a substitute. |

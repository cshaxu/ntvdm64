...
| Expected Markers | The selected product retains the original `ES=049F`, `BX=0010 -> required-size` retry. The next source-defined process/PSP-version or stack/control disposition explains the later `CS:03F4 IP:20F7` stop without modifying guest bytes or suppressing the retry. |

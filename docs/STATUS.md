...
> **T231 S10 progress:** [COMMAND v2 route admission](etc/evidence/t231-s10-command-v2-route-admission-001.md) records adapter-only `54:00..10` ownership, original `cmddisp.c` table admission, the rebuilt local Direct matrix, the native engine's bounded copied launch-input bind for original `54:01`, and physical deletion of the full COMMAND-v1 implementation/fixture/composition closure (including the former CMDINFO codec).  The fresh formal-r5 graph admits no `bop-v1` input and dry-runs 378 edges; its exact MSVC commands also recompile all fifteen current COMMAND mirror/v2/shim units under `/W4 /WX /MT`. The original S4 registry-success branch was replayed successfully under its scoped temporary HKCU override. Actual formal Ninja execution and bounded native observation remain open; this is not T231 closure.

...
> **Current effective packet: M0 T225 S24.** Its governing brief appears immediately below.

**Active: M0 T225 S24 -- machine/BIOS selector owner-package audit.**
...
| --- | --- |
| Identifier Mode | M0 T225 S24, Ordinary Mode. |
| Admission And Approval | The owner-approved T225 objective, directive to avoid trace-led singleton BOP work, and continuing instruction admit a whole machine/BIOS owner-package audit. [S23 closure](history/m0-t225-s23-closure-20260817.md) proves `06h` and observes `02h`; it explicitly transfers no further selector implementation. |
| Objective | Establish a complete source/ABI/failure and current-route ledger for the reachable OpenNT/SoftPC machine and BIOS selector family, classify every member as native Bochs mechanic, source-derived historical machine handler, explicit unavailable/deferred, or collision, and select at most one coherent next implementation package. |
| Non-goals | No new selector implementation, BOP/provider expansion, device enablement, firmware change, host capability, guest-image mutation, x86/prebuilt object link, or trace-led singleton patch. No claim that an inventory itself executes guest programs. |
| Reference Baseline | S22 continuity closure; S23 INT 06 closure; original SoftPC BIOS/handler tables and callers; current bx-core exception bridge, bx-mantle minimal machine/default port space, bx-vdm machine composition, formal Ninja manifest, and retained historical machine inventory evidence. |
| Files And ABI Surface | Read-only source and formal graph inventories; new source/ABI/failure ledger and route map below `docs/etc/research/`; Status/history/evidence only unless an independently admitted source-boundary test is required to prove no live route. No production ABI change is authorized. |
...
| Verification | Reconcile original selector tables, handler source and known callers against formal linked sources and current route selection. Check every selector identity, its owner, four-rung disposition, required Bochs capability and Direct/Readonly relevance. Run only static/source and graph checks; use an existing native trace solely as corroborating reachability after the ledger is complete. |
| Expected Markers | A total selector ledger with no unclassified reachable identity; explicit `02h`/`06h` distinction; selector-blind bx-core/bx-mantle proof; source-first disposition and stop conditions for every missing native prerequisite; one owner-scoped successor recommendation rather than a per-trace backlog. |
| Asset Needs | Original OpenNT/SoftPC source and table manifests, adopted Bochs 2.6 source identity, existing source-built primary inputs and S23 observation, formal MSVC x64 `/MT` Ninja manifest, and disposable `build/M0-T225-S24/<run>` roots if a graph check requires one. |
| Reporting Requirements | Record source paths, table identities, caller evidence, current bind route, owner, source-recovery rung, ABI/failure contract, Bochs dependency, Direct/Readonly disposition, collision/unavailable reason, and any stale workaround. State all limits and next-package admission facts. |
| Stop Conditions | A selector requires an unclassified Bochs device/firmware/product-shell dependency; source tables conflict; a proposed route crosses the x64 ABI boundary; a trace result is used as implementation authority; or an audit finding requires behavior change. Record it and continue independent classification; do not implement or invent a substitute. |
| Exit Criteria | Every reachable machine/BIOS selector is classified with a reviewable source/ABI/failure/current-route ledger; `02h` and `06h` have distinct owner dispositions; no singleton implementation is admitted; and the next coherent owner package is explicitly recommended or a first non-composable prerequisite is transferred. |
| Original Owner Request | “以新架构和三种模式为基础，全面实现跑通BOP表格的指令。” |
| Similar-Issue Sweep | All machine/BIOS selectors, `02h`/`06h` identities, original SoftPC handler tables, ROM/IVT/BDA callers, saved real-mode fault frames, PIC/default port space, old machine-composition island, formal manifest source ownership, and Direct/Readonly relevance. |

> **T225 S23 closure:** [the closure record](history/m0-t225-s23-closure-20260817.md) completes the admitted INT 06 machine-composition package and transfers complete machine/BIOS selector classification—not a trace-led `02h` patch—to S24.
> **T225 S24 P1 current-route ledger:** [the complete historical matrix and reachable-route reconciliation](etc/research/t225-s24-machine-bios-current-route-ledger-001.md) retains T131's 349-form classification, identifies nine reachable startup identities, and selects one normalization package rather than a singleton `02h` repair.

...
> **T225 S20 P4 live-route inventory:** [the inventory](etc/research/t225-s20-command-live-route-inventory-001.md) confirms the formal CLI/native graph reaches COMMAND only through the bound package session; exact branches in dapter_runtime remain historical-only. Direct host-child terminal handling and Readonly unavailability remain package-owned; no host-child re-entry queue is admitted. One bounded native package validation is now the next gate.

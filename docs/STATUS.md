...
> **T228 S1 P13 result:** [the COMMAND environment-size boundary result](etc/evidence/t228-s1-command-environment-size-boundary-result-001.md) proves that a controlled opaque greater-than-4-KiB host environment reproduces the ordinary-host failure after the source-shaped 54:0F retry, whereas the minimal environment completes the child/return lifecycle. It transfers the repair boundary to the complete original guest COMMAND environment-reallocation / NTDOS MCB / transient-placement domain; it selects no adapter BOP or Bochs repair.

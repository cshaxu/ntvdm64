...
| Admission And Approval | Owner rejected closure without actual original COMMAND execution and prohibits modifying `COMMAND.COM`. S2 restored the original `BX` capacity result. S3's corrected selected-source observations prove the original `0010 -> required-size` retry completes (`0161` and `015F` under their recorded inherited environments); the earlier `0270`/`EndInit` overwrite claim came from stale experimental objects and is withdrawn. |
| Objective | Establish the earliest original DOS arena/transient or CPU40 execution precondition required after `COMMAND`'s own successful `SVC_GETINITENVIRONMENT` retry and valid HMA resident transfer, while retaining the original host capacity contract. |
...
| Expected Markers | The selected product retains the original `ES=049F`, `BX=0010 -> required-size` retry. Its original `LodCom_Entry` remains a valid HMA target and A20 is enabled. The next source-defined DOS arena/transient/CPU40 disposition explains the later loss of command-loop continuity without modifying guest bytes or suppressing the retry. |
...
| Verification | Full 20-callback plus metadata contract tests, Base hook storage/consumption lifetime test, normal/failed registration tests, formal selected-source link if a complete form is reached. |
| Expected Markers | No partial output table can report success; every callback has retained original or explicit unavailable behavior; no guest pointer/HANDLE cast is introduced. |
...
**Closure:** [T371 history record](history/m0-t371-xms-dpmi-dos-extender-execution-closure-20260902.md).

**S1:** [original XMS, DPMI and DOS-extender execution topology](etc/evidence/m0-t371-s1-original-xms-dpmi-execution-topology-001.md).
...
**S4:** [fixed XMS/DPMI integration observation](etc/evidence/m0-t371-s4-fixed-xms-dpmi-integration-observation-001.md).
...
| Field | Record |
| --- | --- |
| Identifier Mode | M0 T368 S4; ordinary mode (single-person dual-role implementation; closed). |
...
| Objective | Reuse the one identity-proven fixed-container observation using staged byte-identical `LOADFIX.COM` and classify its first source-owned continuation after the original COMMAND record route. |
...
| Verification | Product/media identity comparison against the one existing non-debug fixed-container observation; classify only its first source-defined result. |
...
| Reporting Requirements | Record the reused command, product/media identity and first source-owned result; do not create trace-selected repair work. |
...
| Exit Criteria | Exactly one valid fixed observation is retained and classified against the S1 source topology. Met by the identity-proven T367 observation. |
| Original Owner Request | “单人双角色模式执行构建NTVDM64的队列任务。执行过程中，注意要保持镜像组件的最小修改复通，保持overlay最小，自主逻辑尽量放入adapter-*。” |
...
**S4 evidence:** [identical fixed-observation disposition](etc/evidence/m0-t368-s4-identical-fixed-observation-disposition-001.md).
...
| Verification | Focused original-owner proof covers exact child command/arguments, exit 37, `INCREMENT/DECREMENT_REENTER_COUNT`, Base VDM retry, invalid input, cancellation, cleanup and parent-standard-handle isolation; formal source membership remains verified.  After that link, one fixed non-debug observation may replace only `ntvdm.exe` in the already identity-verified package root and retain the exact declared `C:\\Windows\\System32\\cmd.exe /c exit 37` request. |
...
| Field | Record |
| --- | --- |
| Closure | [T379 history record](history/m0-t379-original-vdmredir-runtime-product-activation-closure-20260902.md) |
| Result | The original `VDMREDIR.dll` is a formal sibling companion of the staged original-identity parent `ntvdm.exe`; the single frozen original `REDIR.EXE` run opened the guest workload and exited `0`. |
| Limit | The run did not reach `MS_bop_7`/`57:xx`; no dynamic-load, export-call, named-pipe or general Redirector-runtime claim is made. |
| Transfer | The ordered next candidate remains COMMAND native-child runtime activation. |
...
**S1--S5 evidence:** [admission](etc/evidence/m0-t379-s1-original-vdmredir-product-admission-001.md), [DLL/parent](etc/evidence/m0-t379-s2-original-vdmredir-dll-parent-loader-closure-001.md), [local lifecycle](etc/evidence/m0-t379-s3-local-resource-lifecycle-closure-001.md), [formal matrix](etc/evidence/m0-t379-s4-formal-vdmredir-activation-matrix-001.md), [frozen observation](etc/evidence/m0-t379-s5-frozen-original-redir-observation-001.md).

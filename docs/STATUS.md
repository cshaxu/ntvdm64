...
> **T228 S1 P6 result:** [the far-call base-pointer provenance result](etc/evidence/t228-s1-far-call-base-pointer-provenance-result-001.md) maps the observed SS:[BP+974Ah] pointer to the source-built COMMAND resident image and proves that it correctly selected the dynamically allocated TRANGROUP segment 5A5F; its destination was already zero at the transition. The defect is therefore transferred to the complete original COMMAND transient allocation/move/reload package, never to a BOP leaf or selector-specific Bochs repair; see the [owner-package map](etc/operations/t228-s1-command-transient-owner-package-map-001.md).
> **T228 S1 P7 result:** [the reverse-string mechanics result](etc/evidence/t228-s1-command-transient-reverse-string-mechanics-result-001.md) adds the original COMMAND `STD; REP MOVSB; CLD` form to the complete real/V86 CPU compatibility fixture and passes it under the formal CPU5 graph. It excludes a generic bx-core string-mechanics repair, but does not claim COMMAND transient lifecycle closure.
...
> **T228 S1 P13 result:** [the COMMAND environment-size boundary result](etc/evidence/t228-s1-command-environment-size-boundary-result-001.md) proves that a controlled opaque greater-than-4-KiB host environment reproduces the ordinary-host failure after the source-shaped 54:0F retry, whereas the minimal environment completes the child/return lifecycle. It transfers the repair boundary to the original COMMAND guest-image/lifecycle domain, including environment reallocation, NTDOS MCB interaction and transient placement; it selects no adapter BOP or Bochs repair.
### T228 S1 P14 -- Environment-size first-fault boundary comparison

| Field | Required record |
| --- | --- |
| Identifier Mode | M0 T228 S1 P14, Ordinary Mode. |
| Admission And Approval | P13 establishes a size-sensitive original COMMAND lifecycle boundary; existing P2 history and the separately registered S28 first-fault observer already provide selector-blind, default-off mechanics. This P reuses them without changing product source. |
| Objective | Compare the first non-UD fault reached from the hash-locked T228 inputs under the retained minimal host environment and the P13 controlled opaque greater-than-4-KiB environment, in order to determine whether the size branch reaches a distinct mechanical fault boundary. |
| Non-goals | No source-label attribution from heuristic disassembly, BOP/service observation or interpretation, guest-RAM/provenance capture, CPU semantics change, guest image change, provider change, or repair selection. |
| Reference Baseline | P13 COMMAND environment-size boundary; T225 S28 registered first-fault observer; P1 scalar-only history boundary; T145 source-map availability limitation. |
| Files And ABI Surface | Existing history-diagnostic Ninja graph and native CLI option only; disposable build-root output plus source/ABI/failure/result evidence. No source or ordinary CLI ABI change. |
| Applicable Rules | Active T228 packet; source-recovery audit; immutable-input; architecture/coding; formal-build; existing intrusive-exception registration. |
| Verification | One controlled child environment per declared size form; immutable Direct run with --observe-first-fault, bounded budget and paired startup paths; compare copied typed fault record, typed segment-access record, lifecycle ledger and terminal. |
| Expected Markers | Minimal form either reaches normal lifecycle completion without a first fault, or exposes a typed record; large form exposes exactly one typed first fault/controlled terminal. The result distinguishes equality from difference but does not attribute an original source instruction. |
| Asset Needs | Current manifest-recorded history diagnostic executable, P13 input identity, startup fixtures, and no new external asset. |
| Reporting Requirements | Record executable/config/input identities, exact child-only environment forms, result for each form, mechanical equality/difference, source-map limitation, and rejected repair alternatives. |
| Stop Conditions | Any need for opcode/source-name filtering, guest-byte capture, extra BOP route, new Bochs intrusion, non-immutable input, or product-source change. |
| Exit Criteria | Both declared forms have reproducible typed outcomes; the result either narrows the complete owner-domain map or records that it remains unchanged without selecting a leaf repair. |
| Original Owner Request | Diagnose and repair only a demonstrated complete owner domain after code-complete BOP work, not trace-selected BOP leaves. |
...
| Field | Required record |
| --- | --- |
| Identifier Mode | M0 T228 S1 P19, Ordinary Mode. |
| Admission And Approval | P17/P18 leave one complete source-proven owner domain: original COMMAND `EndInit` and the NTDOS MCB allocation/deallocation lifecycle that determines transient placement. Owner-directed T228 permits whole-owner diagnosis and repair, never trace-led BOP work. |
| Objective | Produce the mandatory four-rung recovery ledger and a complete source/ABI/failure map for `SETBLOCK`, largest-block `ALLOC`, `DEALLOC`, `LodCom` relocation, checksum/reload, and `TJmp`; identify the smallest selector-blind observation required to distinguish their outcomes. |
| Non-goals | No BOP implementation, adapter EXEC/MCB/PSP behavior, guest-image patch, environment workaround, host child, synthetic transient bytes, address/opcode filter, or Bochs semantic repair. |
| Reference Baseline | P8 owner correction; P12 generic mechanics pass; P15/P16 exact source maps; P17/P18 bounded controlled-large result; hash-locked original COMMAND and NTDOS images. |
| Files And ABI Surface | Status, one source/ABI/failure map, and only existing source/evidence records. No runtime ABI or build-manifest change. |
| Applicable Rules | Active T228 packet; execution source-recovery audit; immutable guest input; architecture/coding boundaries; Bochs intrusion register. |
| Verification | Review the complete original source flow and current composition; inspect existing generic observation seams; account for all four recovery rungs and each failure branch; governance verification and `git diff --check`. |
| Expected Markers | A precise owner boundary, accepted/rejected recovery rung, and an observation contract that records ordinary CPU/DOS mechanics without recognizing BOP, DOS service semantics, or a guest address. |
| Asset Needs | Repository-local OpenNT `rdata.asm`, `command2.asm`, `alloc.asm`, `msinit.asm`, retained exact maps, and existing bx-core/bx-mantle diagnostic seams only. |
| Reporting Requirements | Cite source paths, data/control contracts, failure paths, existing/current gaps, recovery-rung disposition, and the reason no repair can yet be selected. |
| Stop Conditions | A need to interpret a selector/service, inspect unbounded guest memory, alter the guest image, emulate MCB/INT 21 behavior in adapter, add an address/opcode condition, or use a new external source. |
| Exit Criteria | The whole owner package and every source-recovery rung are classified; any follow-on observer is defined at a selector-blind typed boundary with no repair claim. |
| Original Owner Request | Continue holistic source-first implementation toward code-complete BOP and guest integration, avoiding trace-driven leaf fixes. |
| Similar-Issue Sweep | `EndInit`, `LodCom/LodCom1`, `ChkSum/LoadCom/ReadCom/HavCom/TJmp`, `$ALLOC/$SETBLOCK/$DEALLOC`, MCB initialization, current startup placement, existing generic interrupt/run observation, and Direct/Readonly boundary. |
> **T228 S1 P19 result:** [the source/ABI/failure map](etc/research/t228-s1-endinit-ntdos-mcb-transient-source-abi-failure-map-001.md) completes the original guest owner and recovery-rung audit. It rejects an adapter MCB/transient implementation and finds no reusable generic interrupt observer: the current startup path is BOP-specific and upstream `INT 21/4C` logging is function-specific. Any further observation therefore requires a separately admitted selector-blind bx-core/bx-mantle mechanical profile; no repair is selected.
### T228 S1 P20 -- selector-blind real/V86 software-interrupt observation profile

| Field | Required record |
| --- | --- |
| Identifier Mode | M0 T228 S1 P20, Ordinary Mode. |
| Admission And Approval | P19 proves a complete guest COMMAND/NTDOS lifecycle question and rejects both adapter recovery and the existing BOP-/function-specific interrupt paths. Standing owner authorization permits a registered Bochs mechanical exception when no adapter seam can observe the needed fact. |
| Objective | Add a default-off, finite, selector-blind machine record for every executed real/V86 software interrupt, carrying only copied pre-delivery CPU facts; prove disabled and enabled behavior with a complete profile fixture. |
| Non-goals | No INT 21 or function recognition, BOP/OpenNT/DOS/SoftPC term in bx-core, guest-memory read, opcode/address filter, register/CPU mutation, exception/result change, device/firmware feature, adapter callback, CLI capability, or native repair claim. |
| Reference Baseline | P19 source/ABI/failure map; existing BX-CORE-071 fixed-width history pattern; `soft_int.cc:INT_Ib`; `startup_machine_interrupt_v1` BOP-only rejection. |
| Files And ABI Surface | Registered bx-core `soft_int.cc` hook; project-owned bx-mantle fixed-width record/ring/query; diagnostic build switch and focused fixture only. The default formal manifest remains behaviorally unarmed. |
| Applicable Rules | Active T228; source-recovery audit; Bochs exception register; CPU compatibility completion gate; architecture/coding and immutable-input rules. |
| Verification | Disabled/default fixture, enabled real-mode multi-vector record/order/wrap fixture, protected-mode negative fixture, source scan for prohibited vocabulary, MSVC x64 `/MT` diagnostic closure, retained default graph validation. |
| Expected Markers | Enabled real/V86 fixture records every tested software-interrupt vector with pre-delivery copied state; disabled or protected-mode execution records none and preserves ordinary delivery. |
| Asset Needs | Existing CPU5 formal graph, minimal machine fixture seam, adopted `soft_int.cc`, and no new guest/host input. |
| Reporting Requirements | Exact source hook, record layout, lifecycle/reset behavior, all profile groups, negative scope, registry row, build identity, and explicit no-repair conclusion. |
| Stop Conditions | Any selector/service/function interpretation, guest byte access, address/opcode predicate, output-dependent behavior, BOP/provider dependency, altered interrupt delivery, or need for an unregistered external change. |
| Exit Criteria | The complete default-off observation profile passes focused disabled/enabled/real/protected/wrap tests, formal default build remains unarmed, and the result admits at most one later bounded integration observation. |
...
> **T225 S27 P1 source/ABI/failure map:** [the map](etc/research/t225-s27-command-parent-return-source-abi-failure-map-001.md) retains original COMMAND 54:0B ownership, proves the existing exhausted-plan record is orphaned from the opaque engine boundary, and admits only a fixed-width parent-completion seam plus family regression.
...
> **T225 S29 closure:** [the closure record](history/m0-t225-s29-closure-20260817.md) proves the fault is post-initial-NTIO and compatible with formal NTDOS loading, but retains guest-state/core ambiguity because the mechanical access tuple is absent. It admits only S30's default-off selector-blind access diagnostic.
> **T225 S30 closure:** [the closure record](history/m0-t225-s30-closure-20260817.md) completes the default-off tuple diagnostic and proves a profile-neutral DS:FFFF direct-limit word-read; it admits only S31's original-source attribution, not a repair.
...
> **T225 S25 P3 family/native result:** [the package result](etc/evidence/t225-s25-startup-machine-family-native-validation-result-001.md) proves all eight admitted startup-machine identities through the bound composition surface and one byte-identical Direct/Readonly native observation. It admits no trace-led follow-up work.
> **T225 S23 P1 source/package map:** [the map](etc/research/t225-s23-int06-native-pic-source-map-001.md) separates the original INT 06 handler pair from the native Bochs PIC candidate, rejects empty port-space as a semantic substitute, and fixes bx-core/bx-mantle/bx-vdm ownership before implementation.
> **T225 S23 P2 PIC lifecycle/link audit:** [the audit](etc/research/t225-s23-native-pic-lifecycle-link-closure-001.md) source-builds the original PIC under the formal x64 configuration, rejects only its plugin wrapper, and identifies the original two port-unregistration methods as the finite missing mantle lifecycle prerequisite. It admits P3's native PIC lifecycle only; no selector or OpenNT handler behavior has changed.
> **T225 S23 P3 native PIC result:** [the result](etc/evidence/t225-s23-native-pic-lifecycle-result-001.md) proves the fixed original 8259 lifecycle, port mask/IRQ/IAC/EOI behavior, stub restoration, and repeated cleanup under formal x64 `/MT` Ninja. It remains profile-neutral mechanics only; no INT 06 or BOP behavior is claimed.
> **T225 S23 P4 x64 rehost admission:** [the source/ABI decision](etc/research/t225-s23-int06-x64-rehost-admission-001.md) rejects the legacy callback/x86-object composition, fixes the original frame/PIC/BDA operation order, and requires deliberate `opennt-host` module admission before implementation. No INT 06 route has changed.
> **T225 S23 P5 opennt-host provider:** [the admission and fixture result](etc/research/t225-s23-opennt-host-int06-provider-admission-001.md) adds the source-derived fixed INT 06 transcript as a formal MSVC x64 `/MT` static library and verifies both original branches. It has no route or native machine execution yet.
> **T225 S23 P6 byte-port action:** [the native result](etc/evidence/t225-s23-byte-port-action-result-001.md) registers and verifies selector-blind typed one-byte native PIC port execution under the formal graph, including inactive rejection and two clean lifecycles. It does not add an INT 06 route.
...
> **T225 S23 P7 RAM preflight:** [the native result](etc/evidence/t225-s23-ram-preflight-result-001.md) adds a selector-blind no-copy ordinary-RAM write-access preflight and proves accepted/rejected checks leave guest bytes unchanged. It admits no INT 06 route.
> **T225 S23 P8 native INT 06 transaction:** [the result](etc/evidence/t225-s23-int06-native-transaction-result-001.md) proves the source-derived provider composes with actual minimal-machine RAM/PIC mechanics and checked resume. Native Direct/Readonly observation remains separate.
> **T225 S23 P9 native Direct/Readonly observation:** [the result](etc/evidence/t225-s23-direct-readonly-native-int06-observation-001.md) proves live accepted `06h` execution on identical immutable guest inputs in both modes; observed `02h` remains a machine-package audit fact, not an implementation admission.
...
> **T225 S20 P2 sensitive source/ABI map:** [the subfamily record](etc/research/t225-s20-command-sensitive-subfamily-source-map-001.md) confirms that `54:04` projects the shared Direct host-session CWD rather than a virtual root, confines real handles/child processes to Direct behind opaque session seams, retains Readonly failures, and preserves the OpenNT dynamic environment paragraph rule. The next delivery is one existing-composition family fixture.
> **T225 S20 P3 Direct/Readonly package regression:** [the result](etc/evidence/t225-s20-command-direct-readonly-package-regression-result-001.md) builds the full MSVC x64 /MT Ninja closure and passes a single package-session fixture across all callable 54:00..10 slots in both supported modes. It records source-defined DEM setup order and the dynamic double-NUL environment layout; it is family-route evidence, not COMMAND-package closure or native execution.
> **T225 S20 P4 live-route inventory:** [the inventory](etc/research/t225-s20-command-live-route-inventory-001.md) confirms the formal CLI/native graph reaches COMMAND only through the bound package session; exact branches in `adapter_runtime` remain historical-only. Direct host-child terminal handling and Readonly unavailability remain package-owned; no host-child re-entry queue is admitted. One bounded native package validation is now the next gate.
> **T225 S21 P1 source/ABI/current-route map:** [the guest package record](etc/research/t225-s21-guest-dos-exec-psp-return-source-map-001.md) assigns EXEC, PSP/arena/JFN/environment construction, entry transfer, termination and parent return to original NTDOS guest code. It retains the narrow `50:11` NTDOS guest-RAM staging seam and `50:36`/`50:3C` DEM notifications, rejects a host process or synthetic adapter substitute, and identifies continuous source-built NTIO-to-NTDOS-to-COMMAND reach as the smallest next closure.
...
> **T225 S19 P3 live-route inventory:** [the inventory](etc/research/t225-s19-legacy-runtime-route-inventory-001.md) confirms the formal CLI graph reaches DEM only through composition runtime and its package session; old `adapter_runtime` exact gates are historical derivative inputs, not a migration target.
> **T225 S19 P4 lifecycle-order correction:** [the source/ABI record](etc/research/t225-s19-dem-pdb-lifecycle-ordering-correction-001.md) and [focused package result](etc/evidence/t225-s19-dem-pdb-lifecycle-ordering-result-001.md) restore and verify the OpenNT `50:3C -> DOS_ABORT -> 50:02` order: PDB notification releases per-PSP search state only; a Direct opaque token remains valid until ordinary guest-selected close. The separate `50:25` baseline was resolved by P5; P4 makes no wider endpoint claim.
> **T225 S19 P5 DPB failure convergence:** [the source map](etc/research/t225-s19-dem-dpb-failure-source-map-001.md) restores OpenNT `demClientError` failure ordering for `50:25`: unadmitted drives return AX=15; admitted-but-unavailable snapshot drives take the no-LastError AX=5 fallback. [The regression](etc/evidence/t225-s19-dem-dpb-failure-regression-result-001.md) passes all 73 DEM identities in both Direct and Readonly, while retaining P4 token ordering. Native validation remains separate.
> **T225 S19 P6 native package validation:** [the bounded result](etc/evidence/t225-s19-dem-native-package-validation-result-001.md) rebuilds the current 302-edge MSVC x64 /MT native closure and runs Direct/Readonly once each at 100,000 internal ticks. Both preserve the identical 33-record BOP/#UD observation sequence and normal budget terminal. This validates the complete DEM package result only; it admits no trace-led singleton work.
...
> **T225 S10 P3 source/ABI result:** [the native run-control map](etc/research/t225-s10-native-run-control-observation-map-001.md) shows that the engine already carries a finite tick budget to the mantle, but the native CLI fixes it at one million and exports neither a normal result before its worker returns nor the fixture-only adapter BOP observation. Current profile observations require a guest `UD2` and therefore cannot annotate the immutable primary images. This identifies a separate generic run-control admission; no BOP, provider, guest-byte, Bochs CPU/device or firmware change is authorized.

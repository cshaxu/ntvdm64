...
> **Current effective packet: M0 T198 S82.** Its governing brief is the active
> packet table below.

**Active: M0 T198 S82 -- source-map the current guest initialization path that
reaches `073B:0939` without observed BOP `5F`.**
...
| --- | --- |
| Identifier Mode | M0 T198 S82, Ordinary Mode. |
| Admission And Approval | S81 observes no BOP `5F` before the C000 transfer; see `etc/research/t198-s81-bop5f-passive-observation-001.md`. |
| Objective | Map the exact available source-built NTIO/NTDOS image ranges, initialization branches and control-flow ownership leading to `073B:0939`; distinguish an unexecuted `SpcKbd` installation path from a missing/overwritten guest segment. |
| Non-goals | No guest binary changes, BOP/provider implementation, `use_host_int10` write, CLI profile selection, ROM/device import, memory mapping change, or Bochs core/mantle edit. |
| Reference Baseline | S81 passive observation; source-built NTIO/NTDOS input manifests; OpenNT `sysinit1.asm`, `spckbd.asm` and NTDOS source/build records; instruction history. |
| Files And ABI Surface | Evidence/image-layout ledger only. No runtime ABI, build input or source implementation change. |
| Applicable Rules | rules/EXECUTION.md, rules/ARCHITECTURE.md, rules/CODING.md, rules/DOCUMENT.md, design/GOAL.md, design/ARCHITECTURE.md, design/CODING.md, and etc/operations/policy/source-policy.md. |
| Verification | Hash and inspect the actual NTIO/NTDOS generated byte sources, map physical ranges/segments against the trace, inspect original assembler control flow and document unresolved offset gaps; run documentation governance and diff checks. |
| Expected Markers | Image range map, segment/offset ownership table, source-path decision tree, and explicit proof or absence for the `SpcKbd` installation sequence. |
| Asset Needs | Existing repository and pinned adopted source only; no network/import action. |
| Reporting Requirements | Record exact generated-image hashes/ranges, source references, trace correlation, all unbound offsets and a confidence-qualified next ownership boundary. |
| Stop Conditions | Resolution requires guessed symbol offsets, guest byte mutation, new ROM input, BOP/provider behavior, device admission or a core/mantle edit. Stop and retain the unknown mapping. |
| Exit Criteria | Source-backed disposition of the `073B` path (or evidence-backed unresolved source/image gap) with no runtime semantic change. |
| Original Owner Request | Second phase: based on the new architecture, comprehensively run the BOP instruction table with global structure rather than incremental per-service hacks. |
| Similar-Issue Sweep | Cover NTIO image header/relocation, NTDOS placement, SYSINIT/SpcKbd installation, resident segments, far jumps, fixture-specific mock endpoints and source-map absence. |

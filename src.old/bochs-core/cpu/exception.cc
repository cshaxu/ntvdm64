...
/* DIVERGENCE(BX-CORE-DIV-001,BX-UD-001,BX-UD-002): retained real/V86
 * guard plus one private, selector-blind overlay call below. */
...
void BX_CPU_C::exception(unsigned vector, Bit16u error_code)
{
  ...
  /* DIVERGENCE(BX-UD-001,BX-UD-002): selector-blind private overlay
   * decision.  Native exception delivery continues on a zero return. */
  if (BX_CPU_THIS_PTR overlay_handle_exception(vector, error_code)) return;

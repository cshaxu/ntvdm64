...
| `SOFTPC-DIV-007` | OpenNT's INT 06/unexpected-interrupt source expects direct SoftPC stack, PIC-port and BDA access. | Bochs mechanics must remain typed and bounded; a source provider may not retain raw machine pointers or own a BOP route. | The neutral interrupt-06 operation state machine emits only typed RAM/port actions; `app` composes the source-owned entry around it. | `interrupt06_provider.{c,h}` |
| `SOFTPC-DIV-008` | Reached OpenNT DEM bodies obtain registers, SAS aliases and native handles through process-global CCPU/SAS product state. | The standalone composition exposes only a bounded synchronous call, scoped guest-memory leases and session-owned opaque handles. | Preserve the historical helper spelling and call-local register/result shape in the DEM CCPU/SAS facade; source-specific span selection remains a pending BOP-owner split and is not a new adapter dispatcher. | `dem/{ccpu_sas_facade,direct_context}.{c,h}` |

...
| `HOST-DIV-031` | OpenNT `cmdexec.c` creates a process using temporary NTVDM global standard handles and product-owned child lifecycle state. | A standalone session must not mutate its own standard handles or expose native `HANDLE` values outside the host boundary. | Retain `cmdexec.c` ordering and ANSI inputs; move endpoint lookup, `CreateProcessA`, Job ownership, completion event and result recording to a host-owned opaque-ID child capability. | `opennt_host_child_runtime.h`, `opennt_host_child_runtime.c`, `command/opennt_command_composition.c` |
| `HOST-DIV-031` | OpenNT `cmdexec.c` creates a process using temporary NTVDM global standard handles and product-owned child lifecycle state. | A standalone session must not mutate its own standard handles or expose native `HANDLE` values outside the host boundary. | Retain `cmdexec.c` ordering and ANSI inputs; move endpoint lookup, `CreateProcessA`, Job ownership, completion event and result recording to a host-owned opaque-ID child capability. | `opennt_host_child_runtime.h`, `opennt_host_child_runtime.c`, `command/opennt_command_composition.c` |

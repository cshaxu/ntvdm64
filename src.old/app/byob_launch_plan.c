...
#define BYOB_LAUNCH_PLAN_VERSION 2u

static int valid(const byob_launch_plan *value)
{
    uint32_t i;
    if (!value || value->version != BYOB_LAUNCH_PLAN_VERSION ||
        (value->slot_count != 1u && value->slot_count != 2u) ||
        value->first.version != 1u || value->first.tail_bytes > BYOB_LAUNCH_DECLARATION_TAIL_BYTES ||
        (value->first.target_kind < BYOB_LAUNCH_TARGET_KIND_COM ||
         value->first.target_kind > BYOB_LAUNCH_TARGET_KIND_PIF)) return 0;
...
{
    if (!out_value || !selection || (selection->declared_target_count != 1u &&
        ...
        !byob_launch_declaration_from_arguments(&out_value->first,
            selection->declared_targets[0].component.file_name, argument_count, arguments)) return 0;
    out_value->version = BYOB_LAUNCH_PLAN_VERSION;
    out_value->slot_count = selection->declared_target_count;
    ...
    wchar_t output[BYOB_LAUNCH_PLAN_ENV_CHARS])
{
    ...
    output[4] = value->first.target_kind == BYOB_LAUNCH_TARGET_KIND_COM ? L'c' :
        value->first.target_kind == BYOB_LAUNCH_TARGET_KIND_EXE ? L'e' :
        value->first.target_kind == BYOB_LAUNCH_TARGET_KIND_BAT ? L'b' : L'p';
    ...
    const wchar_t *input)
{
    ...
    if (bytes > BYOB_LAUNCH_DECLARATION_TAIL_BYTES || wcslen(input) != 8u + bytes * 2u) return 0;
    memset(out_value, 0, sizeof(*out_value)); out_value->version = BYOB_LAUNCH_PLAN_VERSION;
    out_value->slot_count = (uint32_t)(input[2] - L'0'); out_value->first.version = 1u;
    out_value->first.target_kind = kind == L'c' ? BYOB_LAUNCH_TARGET_KIND_COM :
        kind == L'e' ? BYOB_LAUNCH_TARGET_KIND_EXE :
        kind == L'b' ? BYOB_LAUNCH_TARGET_KIND_BAT : BYOB_LAUNCH_TARGET_KIND_PIF;

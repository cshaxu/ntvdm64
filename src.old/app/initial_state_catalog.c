    ...
    uint32_t chars, runtime_initial_state *out)
{
  uint32_t i;
  if (!out) return 0;
  runtime_initial_state_clear(out);
  if (!digest || chars != 64u) return 0;
  for (i=0; i<64u; ++i) if (digest[i] != (uint16_t)(uint8_t)evidence[i]) return 0;
  out->disposition=RUNTIME_INITIAL_STATE_PRESENT; out->range_count=3u; out->payload_bytes=(uint32_t)sizeof(bytes);
  ...
  runtime_guest_write_initialize(&out->ranges[0],0x40u,48u,0u);
  runtime_guest_write_initialize(&out->ranges[1],0x496u,1u,48u);
  runtime_guest_write_initialize(&out->ranges[2],0x714u,4u,49u);
  return runtime_initial_state_admitted(out,0x100000u);
}

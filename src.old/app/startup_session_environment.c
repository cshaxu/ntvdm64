...
#define RUNTIME_STARTUP_SESSION_ENV_PROFILE L"NTVDM64_ADAPTER_PROFILE"
#define RUNTIME_STARTUP_SESSION_ENV_ROOT L"NTVDM64_ADAPTER_ROOT"

static runtime_startup_session runtime_startup_environment_session;
static int runtime_startup_environment_attempted;
static int runtime_startup_environment_installed;

int runtime_startup_session_install_from_environment(
    runtime_startup_session *session)
{
    ...
    return runtime_startup_session_install(session, profile, root) ? 1 : -1;
}

int runtime_startup_session_prepare_from_environment(
    runtime_startup_plan *plan, const uint8_t **payload,
    uint64_t *payload_bytes)
{
    int install_result;
    if (runtime_startup_environment_installed) {
        return runtime_startup_session_prepare_ntio_plan(
    ...
    install_result = runtime_startup_session_install_from_environment(
...
int runtime_startup_session_copy_host_drive_inventory_from_environment(
    uint8_t out_types[26])
{
    if (!runtime_startup_environment_installed) return 0;
    return runtime_startup_session_copy_host_drive_inventory(

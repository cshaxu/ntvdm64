...
#include "adapter-win32/facade/demfile_create_observation.h"
...
static int parse_mutation_mode(const wchar_t *text, uint32_t *out_mode)
{
    ...
    if (_wcsicmp(text, L"direct") == 0) {
        *out_mode = RUNTIME_ENGINE_MUTATION_MODE_DIRECT;
        return 1;
    }
    if (_wcsicmp(text, L"readonly") == 0) {
        *out_mode = RUNTIME_ENGINE_MUTATION_MODE_READONLY;
        return 1;
    }
    return 0;
}

/* The engine/machine contract already owns the finite tick watchdog. This
...
static void print_bop_sequence(const struct runtime_bop_sequence_observation *sequence)
{
    uint32_t index;
    if (sequence == 0) return;
    wprintf(L"ntvdm64: bop-sequence count=%u overflow=%u\n",
        sequence->record_count, sequence->overflowed);
    for (index = 0u; index < sequence->record_count; ++index) {
        const struct runtime_bop_sequence_observation_record *record =
            &sequence->records[index];
        wprintf(L"ntvdm64: bop[%u] cs=%04x eip=%08x selector=%02x service=%02x has-service=%u disposition=%u eax=%08x ebx=%08x ecx=%08x edx=%08x esi=%08x edi=%08x flags=%08x ds=%04x es=%04x ss=%04x\n",
...
static void print_command_bootstrap_observation(
    const struct runtime_command_bootstrap_observation *observation)
{
    uint32_t index;
    if (observation == 0) return;
    wprintf(L"ntvdm64: command-bootstrap observed=%u read-failed=%u cs=%04x eip=%08x physical=%08x bytes=",
        observation->observed, observation->read_failed, observation->cs,
        observation->eip, observation->physical_address);
    for (index = 0u; index < RUNTIME_COMMAND_BOOTSTRAP_OBSERVATION_BYTES;
        ++index) wprintf(L"%02x", observation->command[index]);
    wprintf(L"\n");
}
static void print_command_current_dir_observation(
    const struct runtime_command_current_dir_observation *observation)
{
    uint32_t index;
    if (observation == 0) return;
    wprintf(L"ntvdm64: command-current-dir observed=%u read-failed=%u ds=%04x esi=%08x physical=%08x bytes=",
        observation->observed, observation->read_failed, observation->ds,
        observation->esi, observation->physical_address);
    for (index = 0u; index < RUNTIME_COMMAND_CURRENT_DIR_OBSERVATION_BYTES;
        ++index) wprintf(L"%02x", observation->directory[index]);
    wprintf(L"\n");
}
static void print_dem_open_observation(const struct runtime_dem_open_observation *observation)
{
    uint32_t index;
    if (!observation) return;
    wprintf(L"ntvdm64: dem-open observed=%u count=%u read-failed=%u ds=%04x esi=%08x mode=%u physical=%08x disposition=%u gpr-mask=%08x ax=%04x bp=%04x flags-mask=%08x flags=%08x path=",
    ...
    for (index = 0u; index < RUNTIME_DEM_OPEN_OBSERVATION_PATH_BYTES; ++index) wprintf(L"%02x", observation->path[index]);
    wprintf(L"\n");
}
static void print_demfile_create_observation(
    const struct runtime_demfile_create_observation *observation)
{
    uint32_t index;
    if (!observation) return;
    wprintf(L"ntvdm64: demfile-create calls=%u conversion-error=%u first-error=%u retry-error=%u access=%08x share=%08x creation=%08x flags=%08x path=",
    ...
    for (index = 0u; index < RUNTIME_DEMFILE_CREATE_OBSERVATION_PATH_CHARS &&
...
static void print_ntdos_exec_entry_observation(
    const struct runtime_ntdos_exec_entry_observation *observation)
{
    if (observation == 0) return;
    wprintf(L"ntvdm64: ntdos-exec-entry observed=%u read-failed=%u cs=%04x eip=%08x pdb=%04x physical=%08x exit=%08x ctrl-c=%08x fatal=%08x parent=%04x environment=%04x\n",
        observation->observed, observation->read_failed, observation->cs,
...
static void print_generic_ud_sequence(const struct runtime_generic_ud_sequence_observation *sequence)
{
    uint32_t index, byte_index;
    if (sequence == 0) return;
    wprintf(L"ntvdm64: ud-sequence count=%u overflow=%u\n",
        sequence->record_count, sequence->overflowed);
    for (index = 0u; index < sequence->record_count; ++index) {
        const struct runtime_generic_ud_sequence_observation_record *record =
            &sequence->records[index];
        wprintf(L"ntvdm64: ud[%u] cs=%04x eip=%08x mode=%u disposition=%u window=",
...
static void print_software_interrupt_observation(void)
{
    ...
    uint32_t index;
    uint32_t count = runtime_machine_software_interrupt_observation_count();
    wprintf(L"ntvdm64: software-interrupts count=%u\n", count);
    for (index = 0u; index < count; ++index) {
        if (runtime_machine_software_interrupt_observation_get(index, &record))
            wprintf(L"ntvdm64: software-interrupt[%u] sequence=%llu vector=%02x mode=%u cs=%04x eip=%08llx flags=%08x ax=%04x bx=%04x cx=%04x dx=%04x ds=%04x es=%04x ss=%04x sp=%04x\n",
...
static void print_interrupt_return_observation(void)
{
    ...
    uint32_t index;
    uint32_t count = runtime_machine_interrupt_return_observation_count();
    wprintf(L"ntvdm64: interrupt-returns count=%u\n", count);
    for (index = 0u; index < count; ++index) {
        if (runtime_machine_interrupt_return_observation_get(index, &record))
            wprintf(L"ntvdm64: interrupt-return[%u] sequence=%llu mode=%u width=%u cs=%04x eip=%08llx flags=%08x ax=%04x bx=%04x cx=%04x dx=%04x ds=%04x es=%04x ss=%04x sp=%08x\n", index, (unsigned long long)record.sequence, record.execution_mode, record.operand_width, record.cs, (unsigned long long)record.rip, record.eflags, record.ax, record.bx, record.cx, record.dx, record.ds, record.es, record.ss, record.sp);
    }
}
#endif
static int result_exit(const struct app_lifecycle_audit *audit)
{
    if (!audit || !app_lifecycle_audit_valid(audit)) return 1;
    if (audit->presentation == APP_LIFECYCLE_PRESENTATION_CONTROLLED_GUEST_TERMINAL ||
        audit->presentation == APP_LIFECYCLE_PRESENTATION_ORDINARY_GUEST_COMPLETION)
        return 0;
    if (audit->presentation == APP_LIFECYCLE_PRESENTATION_REJECTED_ENGINE_REQUEST ||
        audit->presentation == APP_LIFECYCLE_PRESENTATION_REJECTED_COMPOSITION)
        return 3;
    if (audit->presentation == APP_LIFECYCLE_PRESENTATION_EXECUTION_BUDGET) return 4;
    return 1;
}
...
int wmain(int argc, wchar_t **argv)
{
    ...
    wchar_t target_full[MAX_PATH], launch_text[BYOB_LAUNCH_PLAN_ENV_CHARS];
...
#endif
        ;
    uint32_t mutation_mode = RUNTIME_ENGINE_MUTATION_MODE_DIRECT;
    ...
    struct app_dos_safe_alias dos_root_alias;
    ...
    struct app_lifecycle_policy lifecycle_policy;
    struct app_lifecycle_audit lifecycle_audit;
    ...
    target = argv[index];
    app_dos_safe_alias_clear(&dos_root_alias);
    if (!GetFullPathNameW(target, MAX_PATH, target_full, 0) ||
        ...
        !app_dos_safe_alias_admit(config_full,
        ...
        !byob_launch_plan_from_arguments(&launch, &selection, argc - index - 1, argv + index + 1) ||
        !byob_launch_plan_to_environment(&launch, launch_text)) {
        app_dos_safe_alias_release(&dos_root_alias);
        fwprintf(stderr, L"ntvdm64: sibling dos/wow16 bundle admission failed\n"); return 3;
    }
    ...
    app_lifecycle_policy_clear(&lifecycle_policy);
    lifecycle_policy.instruction_tick_budget = instruction_tick_budget;
    if (!app_lifecycle_policy_valid(&lifecycle_policy)) {
        app_dos_safe_alias_release(&dos_root_alias);
        return 3;
    }
    runtime_engine_request_clear(&request);
    if (!copied_text(request.profile_descriptor, RUNTIME_ENGINE_MAX_DESCRIPTOR_CHARS,
            target_full, &request.profile_descriptor_chars) ||
        !copied_text(request.root_descriptor, RUNTIME_ENGINE_MAX_DESCRIPTOR_CHARS,
            dos_root_alias.admitted_root, &request.root_descriptor_chars) ||
        !copied_text(request.launch_descriptor, RUNTIME_ENGINE_MAX_LAUNCH_CHARS,
    ...
    if (validate_only) {
        wprintf(L"ntvdm64: request mode=%u budget=%llu guest-memory=%llu reserved-base=%llu reserved-bytes=%llu observe-bop-sequence=%u observe-command-bootstrap=%u observe-ud-sequence=%u observe-first-fault=%u observe-terminal-position=%u\n",
    ...
    if (observe_bop_sequence) runtime_bop_sequence_observation_enable(1u);
    if (observe_command_bootstrap) runtime_command_bootstrap_observation_enable(1u);
    if (observe_command_current_dir) runtime_command_current_dir_observation_enable(1u);
    if (observe_dem_open) runtime_dem_open_observation_enable(1u);
    if (observe_demfile_create) runtime_demfile_create_observation_enable(1u);
    if (observe_ntdos_exec_entry) runtime_ntdos_exec_entry_observation_enable(1u);
    if (observe_generic_ud_sequence) runtime_generic_ud_sequence_observation_enable(1u);
    if (observe_terminal_position) runtime_machine_stage_terminal_position_observation_enable(1u);
#if RUNTIME_ENABLE_MACHINE_SOFTWARE_INTERRUPT_OBSERVATION
    if (observe_software_interrupts && !runtime_machine_software_interrupt_observation_configure(RUNTIME_SOFTWARE_INTERRUPT_OBSERVATION_CAPACITY_MAX)) {
        app_dos_safe_alias_release(&dos_root_alias);
        return 1;
    }
#endif
#if RUNTIME_ENABLE_MACHINE_INTERRUPT_RETURN_OBSERVATION
    if (observe_interrupt_returns && !runtime_machine_interrupt_return_observation_configure(RUNTIME_INTERRUPT_RETURN_OBSERVATION_CAPACITY_MAX)) {
        app_dos_safe_alias_release(&dos_root_alias);
        return 1;
    }
#endif
#if RUNTIME_ENABLE_MACHINE_PHYSICAL_WRITE_OBSERVATION
    if (observe_physical_write && !runtime_physical_write_observation_configure(
...
#endif
#if RUNTIME_ENABLE_MACHINE_INSTRUCTION_HISTORY
    if (observe_terminal_history) runtime_machine_stage_terminal_history_observation_enable(1u);
    if (observe_terminal_cs_transitions) runtime_machine_stage_terminal_cs_transitions_observation_enable(1u);
#endif
#if RUNTIME_ENABLE_MACHINE_INSTRUCTION_HISTORY_PROVENANCE
    if (observe_terminal_provenance) runtime_machine_stage_terminal_provenance_observation_enable(1u);
    if (observe_terminal_provenance) runtime_machine_stage_terminal_cs_provenance_observation_enable(1u);
#endif
    if (observe_first_fault) {
        runtime_machine_first_fault_observation_enable(1);
        runtime_machine_segment_access_observation_enable(1);
    }
    if (!app_console_cancellation_begin(&cancellation_event)) {
        if (observe_bop_sequence) runtime_bop_sequence_observation_enable(0u);
        if (observe_command_bootstrap) runtime_command_bootstrap_observation_enable(0u);
        if (observe_command_current_dir) runtime_command_current_dir_observation_enable(0u);
        if (observe_dem_open) runtime_dem_open_observation_enable(0u);
        if (observe_demfile_create) runtime_demfile_create_observation_enable(0u);
        if (observe_ntdos_exec_entry) runtime_ntdos_exec_entry_observation_enable(0u);
        if (observe_generic_ud_sequence) runtime_generic_ud_sequence_observation_enable(0u);
        if (observe_terminal_position) runtime_machine_stage_terminal_position_observation_enable(0u);
#if RUNTIME_ENABLE_MACHINE_SOFTWARE_INTERRUPT_OBSERVATION
        if (observe_software_interrupts) runtime_machine_software_interrupt_observation_configure(0u);
#endif
#if RUNTIME_ENABLE_MACHINE_INTERRUPT_RETURN_OBSERVATION
        if (observe_interrupt_returns) runtime_machine_interrupt_return_observation_configure(0u);
#endif
#if RUNTIME_ENABLE_MACHINE_INSTRUCTION_HISTORY
        if (observe_terminal_history) runtime_machine_stage_terminal_history_observation_enable(0u);
        if (observe_terminal_cs_transitions) runtime_machine_stage_terminal_cs_transitions_observation_enable(0u);
#endif
#if RUNTIME_ENABLE_MACHINE_INSTRUCTION_HISTORY_PROVENANCE
        if (observe_terminal_provenance) runtime_machine_stage_terminal_provenance_observation_enable(0u);
        if (observe_terminal_provenance) runtime_machine_stage_terminal_cs_provenance_observation_enable(0u);
#endif
        if (observe_first_fault) {
    ...
    if (!app_engine_worker_run(&request, cancellation_event, &result,
            &cancellation_accepted)) {
        app_console_cancellation_end();
        if (observe_bop_sequence) runtime_bop_sequence_observation_enable(0u);
        if (observe_command_bootstrap) runtime_command_bootstrap_observation_enable(0u);
        if (observe_command_current_dir) runtime_command_current_dir_observation_enable(0u);
        if (observe_dem_open) runtime_dem_open_observation_enable(0u);
    ...
    if (observe_software_interrupts) {
        ...
        runtime_machine_software_interrupt_observation_configure(0u);
    }
#endif
#if RUNTIME_ENABLE_MACHINE_INTERRUPT_RETURN_OBSERVATION
    if (observe_interrupt_returns) {
        print_interrupt_return_observation();
        runtime_machine_interrupt_return_observation_configure(0u);
    }
#endif
#if RUNTIME_ENABLE_MACHINE_PHYSICAL_WRITE_OBSERVATION
    if (observe_physical_write) {
        struct runtime_physical_write_observation physical_write;
        if (runtime_physical_write_observation_copy(&physical_write)) {
            uint32_t physical_write_index;
            wprintf(L"ntvdm64: physical-write address=%llx bytes=%u captured=%u sequence=%llu cs=%04x base=%llx eip=%llx ss=%04x sp=%04x data=",
            ...
            wprintf(L"\n");
        } else wprintf(L"ntvdm64: physical-write unavailable\n");
        runtime_physical_write_observation_reset();
    }
#endif
    if (observe_bop_sequence) {
        struct runtime_bop_sequence_observation sequence;
        if (runtime_bop_sequence_observation_copy(&sequence))
            print_bop_sequence(&sequence);
        else
            wprintf(L"ntvdm64: bop-sequence unavailable\n");
        runtime_bop_sequence_observation_enable(0u);
    }
    if (observe_command_bootstrap) {
        ...
        if (runtime_command_bootstrap_observation_copy(&observation))
            ...
            wprintf(L"ntvdm64: command-bootstrap unavailable\n");
        runtime_command_bootstrap_observation_enable(0u);
    }
    if (observe_command_current_dir) {
        struct runtime_command_current_dir_observation observation;
        if (runtime_command_current_dir_observation_copy(&observation))
            print_command_current_dir_observation(&observation);
        else wprintf(L"ntvdm64: command-current-dir unavailable\n");
        runtime_command_current_dir_observation_enable(0u);
    }
    if (observe_dem_open) {
        struct runtime_dem_open_observation observation;
        if (runtime_dem_open_observation_copy(&observation)) print_dem_open_observation(&observation);
        else wprintf(L"ntvdm64: dem-open unavailable\n");
        runtime_dem_open_observation_enable(0u);
    }
    if (observe_demfile_create) {
        struct runtime_demfile_create_observation observation;
        if (runtime_demfile_create_observation_copy(&observation))
            print_demfile_create_observation(&observation);
        else wprintf(L"ntvdm64: demfile-create unavailable\n");
        runtime_demfile_create_observation_enable(0u);
    }
    if (observe_ntdos_exec_entry) {
        ...
        else wprintf(L"ntvdm64: ntdos-exec-entry unavailable\n");
        runtime_ntdos_exec_entry_observation_enable(0u);
    }
    if (observe_first_fault) {
        struct runtime_first_fault_observation first_fault;
        struct runtime_segment_access_observation segment_access;
        if (runtime_machine_first_fault_observation_copy(&first_fault))
            wprintf(L"ntvdm64: first-fault vector=%u error=%u cs=%04x eip=%08x eax=%08x eflags=%08x mode=%u\n",
            ...
            wprintf(L"ntvdm64: first-fault unavailable\n");
        if (runtime_machine_segment_access_observation_copy(&segment_access))
            wprintf(L"ntvdm64: segment-access kind=%u branch=%u seg-index=%u selector=%04x offset=%08x width=%u limit=%08x valid=%08x\n", segment_access.access_kind, segment_access.branch_kind, segment_access.segment_index, segment_access.segment_selector, segment_access.offset, segment_access.width, segment_access.limit_scaled, segment_access.cache_valid);
        else
            wprintf(L"ntvdm64: segment-access unavailable\n");
        runtime_machine_segment_access_observation_enable(0);
        runtime_machine_first_fault_observation_enable(0);
    }
    if (observe_terminal_position) {
        struct runtime_machine_stage_terminal_position position;
        if (runtime_machine_stage_terminal_position_observation_copy(&position))
            wprintf(L"ntvdm64: terminal-position cs=%04x eip=%08x\n", position.cs, position.eip);
        else wprintf(L"ntvdm64: terminal-position unavailable\n");
        runtime_machine_stage_terminal_position_observation_enable(0u);
    }
#if RUNTIME_ENABLE_MACHINE_INSTRUCTION_HISTORY
    if (observe_terminal_history) {
        struct runtime_machine_stage_terminal_history history;
        uint32_t history_index;
        if (runtime_machine_stage_terminal_history_observation_copy(&history)) {
            wprintf(L"ntvdm64: budget-terminal-history count=%u\n", history.count);
            for (history_index = 0u; history_index < history.count; ++history_index)
                wprintf(L"ntvdm64: history[%u] sequence=%llu cs=%04x eip=%016llx ss=%04x sp=%04x\n", history_index, (unsigned long long)history.records[history_index].sequence, history.records[history_index].cs, (unsigned long long)history.records[history_index].rip, history.records[history_index].ss, history.records[history_index].sp);
        } else wprintf(L"ntvdm64: budget-terminal-history unavailable\n");
        runtime_machine_stage_terminal_history_observation_enable(0u);
    }
    if (observe_terminal_cs_transitions) {
        ...
        if (runtime_machine_stage_terminal_cs_transitions_observation_copy(&transitions)) {
            wprintf(L"ntvdm64: budget-terminal-cs-transitions count=%u\n", transitions.value.count);
            for (transition_index = 0u; transition_index < transitions.value.count; ++transition_index) {
                const struct runtime_instruction_history_transition *transition =
                    &transitions.value.transitions[transition_index];
                wprintf(L"ntvdm64: cs-transition[%u] previous=%04x:%08x ss=%04x sp=%04x bp=%04x sequence=%llu current=%04x:%08x ss=%04x sp=%04x bp=%04x sequence=%llu\n", transition_index,
        ...
        } else wprintf(L"ntvdm64: budget-terminal-cs-transitions unavailable\n");
        runtime_machine_stage_terminal_cs_transitions_observation_enable(0u);
    }
#endif
#if RUNTIME_ENABLE_MACHINE_INSTRUCTION_HISTORY_PROVENANCE
    if (observe_terminal_provenance) {
        ...
        if (runtime_machine_stage_terminal_provenance_observation_copy(&provenance)) {
            wprintf(L"ntvdm64: budget-terminal-provenance cs=%04x eip=%08x ss=%04x sp=%04x instruction-valid=%u stack-valid=%u instruction=", provenance.cs, provenance.eip, provenance.ss, provenance.sp, provenance.instruction_valid, provenance.stack_valid);
            for (provenance_index = 0u; provenance_index < RUNTIME_INSTRUCTION_HISTORY_PREDECESSOR_BYTES; ++provenance_index) wprintf(L"%02x", provenance.instruction_bytes[provenance_index]);
            wprintf(L" stack=");
            for (provenance_index = 0u; provenance_index < RUNTIME_INSTRUCTION_HISTORY_STACK_BYTES; ++provenance_index) wprintf(L"%02x", provenance.stack_bytes[provenance_index]);
            wprintf(L"\n");
        } else wprintf(L"ntvdm64: budget-terminal-provenance unavailable\n");
        {
            struct runtime_machine_stage_terminal_cs_provenance transition;
            if (runtime_machine_stage_terminal_cs_provenance_observation_copy(&transition)) {
                wprintf(L"ntvdm64: budget-terminal-cs-transition previous=%04x:%08x bp=%04x current=%04x:%08x bp=%04x predecessor-valid=%u successor-valid=%u stack-valid=%u predecessor=", transition.value.transition.previous.cs, (unsigned)transition.value.transition.previous.rip, transition.value.transition.previous.bp, transition.value.transition.current.cs, (unsigned)transition.value.transition.current.rip, transition.value.transition.current.bp, transition.value.predecessor_valid, transition.value.successor_valid, transition.value.stack_valid);
                for (provenance_index = 0u; provenance_index < RUNTIME_INSTRUCTION_HISTORY_PREDECESSOR_BYTES; ++provenance_index) wprintf(L"%02x", transition.value.predecessor_bytes[provenance_index]);
                wprintf(L" successor=");
                for (provenance_index = 0u; provenance_index < RUNTIME_INSTRUCTION_HISTORY_SUCCESSOR_BYTES; ++provenance_index) wprintf(L"%02x", transition.value.successor_bytes[provenance_index]);
                wprintf(L" stack=");
                for (provenance_index = 0u; provenance_index < RUNTIME_INSTRUCTION_HISTORY_STACK_BYTES; ++provenance_index) wprintf(L"%02x", transition.value.stack_bytes[provenance_index]);
                wprintf(L"\n");
            } else wprintf(L"ntvdm64: budget-terminal-cs-transition unavailable\n");
            runtime_machine_stage_terminal_cs_provenance_observation_enable(0u);
        }
        runtime_machine_stage_terminal_provenance_observation_enable(0u);
    }
#endif
    if (observe_generic_ud_sequence) {
        struct runtime_generic_ud_sequence_observation sequence;
        if (runtime_generic_ud_sequence_observation_copy(&sequence))
            print_generic_ud_sequence(&sequence);
        else
            wprintf(L"ntvdm64: ud-sequence unavailable\n");
        runtime_generic_ud_sequence_observation_enable(0u);
    }
    lifecycle_policy.cancellation_request = cancellation_accepted ?
        APP_LIFECYCLE_CANCELLATION_REQUESTED :
        APP_LIFECYCLE_CANCELLATION_NONE;
    if (!app_lifecycle_classify(&lifecycle_policy, &result,
            &lifecycle_audit) || !app_lifecycle_audit_valid(&lifecycle_audit)) {
        app_dos_safe_alias_release(&dos_root_alias);
        return 1;
    }
    wprintf(L"ntvdm64: terminal=%u detail=%u lifecycle=%u presentation=%u cancellation=%u budget=%llu\n",
...
usage:
    fwprintf(stderr, L"usage: ntvdm64 --dos-root directory --wow16-root directory [--mutation-mode direct|readonly] [--instruction-tick-budget positive-decimal] [--observe-bop-sequence] [--observe-command-bootstrap] [--observe-command-current-dir] [--observe-dem-open] [--observe-demfile-create] [--observe-ud-sequence] [--observe-first-fault] [--observe-terminal-position]"
#if RUNTIME_ENABLE_MACHINE_SOFTWARE_INTERRUPT_OBSERVATION

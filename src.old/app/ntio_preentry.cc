...
#define RUNTIME_NTIO_PREENTRY_BYTES 0x8400u
#define RUNTIME_NTIO_PREENTRY_PHYSICAL 0x700u
#define RUNTIME_NTIO_PREENTRY_PRESERVE 0x714u
#define RUNTIME_NTIO_PREENTRY_PRESERVE_BYTES 4u
#define RUNTIME_NTIO_PREENTRY_CS 0x0070u
    ...
    runtime_finite_run_request *request, Bit64u tick_budget, Bit32u ips)
{
  if (ntio == 0 || identity == 0 || request == 0 || ntio->bytes == 0 ||
      ntio->byte_count != RUNTIME_NTIO_PREENTRY_BYTES ||
      identity->bytes != RUNTIME_NTIO_PREENTRY_BYTES ||
  ...
  request->entry_physical_address = RUNTIME_NTIO_PREENTRY_PHYSICAL;
  request->entry_cs = RUNTIME_NTIO_PREENTRY_CS;
  request->entry_eip = 0;
  request->instruction_tick_budget = tick_budget;
  request->ips = ips;
  request->preserve_physical_address = RUNTIME_NTIO_PREENTRY_PRESERVE;
  request->preserve_byte_count = RUNTIME_NTIO_PREENTRY_PRESERVE_BYTES;
  if (!runtime_preentry_input_make_write_action(preentry_input,

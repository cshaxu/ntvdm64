...
static int runtime_startup_session_load(
    ...
    const wchar_t *profile_path, const wchar_t *byob_root)
{
    ...
    runtime_startup_session_destroy(session);
    ...
    if (byob_profile_validate_file_select(profile_path, byob_root, &selection) !=
        ...
        !runtime_startup_session_load(byob_root, &selection, session)) {
        runtime_startup_session_destroy(session);
        return 0;
    }
    ...
    uint64_t *payload_bytes)
{
    ...
    if (!runtime_startup_plan_preflight(plan, 0x100000u,

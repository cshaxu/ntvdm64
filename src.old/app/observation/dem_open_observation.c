#include "dem_open_observation.h"

#include <string.h>
static uint32_t enabled;
static struct runtime_dem_open_observation record;
static void clear(void) { memset(&record, 0, sizeof(record)); record.magic = RUNTIME_DEM_OPEN_OBSERVATION_MAGIC; record.abi_version = RUNTIME_DEM_OPEN_OBSERVATION_VERSION; record.struct_bytes = sizeof(record); record.enabled = enabled; }
void runtime_dem_open_observation_enable(uint32_t value) { enabled = value == 1u; clear(); }
int runtime_dem_open_observation_copy(struct runtime_dem_open_observation *value) { if (!value || !enabled) return 0; *value = record; return 1; }
    ...
    runtime_dem_open_read read, void *read_state)
{
    ...
    if (physical > UINT32_MAX - RUNTIME_DEM_OPEN_OBSERVATION_PATH_BYTES ||
        !read(read_state, physical, record.path, RUNTIME_DEM_OPEN_OBSERVATION_PATH_BYTES)) { record.read_failed = 1u; return; }

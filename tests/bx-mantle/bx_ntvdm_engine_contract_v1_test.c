...
int main(void)
{
    ...
    if (request.mutation_mode != RUNTIME_ENGINE_MUTATION_MODE_V1_DIRECT ||
        !runtime_engine_request_v1_valid(&request)) return 1;
    request.mutation_mode = RUNTIME_ENGINE_MUTATION_MODE_V1_READONLY;
    if (!runtime_engine_request_v1_valid(&request)) return 2;
    request.mutation_mode = 3u;
    if (runtime_engine_request_v1_valid(&request)) return 3;
    ...
    if (runtime_engine_request_v1_valid(&request)) return 8;
    request.guest_memory_bytes = UINT64_C(0x200000);
    if (!runtime_engine_request_v1_valid(&request)) return 9;
    request.guest_memory_bytes = UINT64_C(0x1010000);
    if (runtime_engine_request_v1_valid(&request)) return 10;
    ...
    if (runtime_engine_request_v1_valid(&request)) return 11;
    request.guest_memory_bytes = UINT64_C(0x200000);
    if (!runtime_engine_request_v1_valid(&request)) return 12;
    request.reserved_memory_base = UINT64_C(0x101000);
    if (runtime_engine_request_v1_valid(&request)) return 13;
    request.reserved_memory_base = UINT64_C(0x100000);
    request.reserved_memory_bytes = UINT64_C(0x110000);
    if (runtime_engine_request_v1_valid(&request)) return 14;
    ...
    if (runtime_engine_request_v1_valid(&request)) return 4;
    request.instruction_tick_budget = UINT64_C(1);
    request.struct_bytes--;
    if (runtime_engine_request_v1_valid(&request)) return 5;
    request.struct_bytes = (uint32_t) sizeof(request);
    if (!runtime_engine_request_v1_valid(&request)) return 6;

    runtime_engine_result_v1_clear(&result);
    if (!runtime_engine_result_v1_valid(&result)) return 7;
    for (kind = RUNTIME_ENGINE_TERMINAL_V1_NONE;
         kind <= RUNTIME_ENGINE_TERMINAL_V1_HOST_CANCELLATION; ++kind) {
        if (!runtime_engine_result_v1_set(&result, kind, kind + 11u) ||
            !runtime_engine_result_v1_valid(&result) ||
            result.terminal_kind != kind || result.detail_code != kind + 11u)
            return (int) (20u + kind);
    }
    if (runtime_engine_result_v1_set(&result,
        RUNTIME_ENGINE_TERMINAL_V1_HOST_CANCELLATION + 1u, 0u)) return 40;
    result.reserved0 = 1u;
    if (runtime_engine_result_v1_valid(&result)) return 41;
    return 0;
}

...
foreach ($path in @($machineHeader, $machineSource, $picHeader, $picSource, $portSource)) {
    if (-not (Test-Path -LiteralPath $path -PathType Leaf)) { throw "Missing T225 S23 PIC input: $path" }
}
...
foreach ($pattern in @('BX-MANTLE-082-BEGIN', 'bx_ntvdm_mantle_pic_create_v1\(\)', 'pic == NULL',
        'bx_devices\.pluginPicDevice = pic;', 'pic->init\(\)', 'pic->fini\(\)',
        'bx_devices\.pluginPicDevice = &bx_devices\.stubPic;', 'bx_ntvdm_mantle_pic_destroy_v1\(pic\)')) {
    if ($machine -notmatch $pattern) { throw "Missing mantle PIC lifecycle operation: $pattern" }
}
...
Assert-Ordered 'bx_devices.init_empty_port_space()' 'bx_ntvdm_mantle_pic_create_v1()'
Assert-Ordered 'bx_ntvdm_mantle_pic_create_v1()' 'pic->init()'
Assert-Ordered 'pic->fini()' 'bx_devices.pluginPicDevice = &bx_devices.stubPic;'
Assert-Ordered 'bx_devices.pluginPicDevice = &bx_devices.stubPic;' 'bx_ntvdm_mantle_pic_destroy_v1(pic)'
Assert-Ordered 'bx_ntvdm_mantle_pic_destroy_v1(pic)' 'bx_devices.cleanup_empty_port_space()'

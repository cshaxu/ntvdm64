...
int main()
{
  ...
  struct runtime_machine_stage_v1_entry observed;
  ...
  if (runtime_machine_stage_v1_begin(&request) !=
      RUNTIME_MACHINE_STAGE_V1_REJECTED_INPUT ||
      runtime_machine_stage_v1_active()) return 1;
  ...
  if (runtime_machine_stage_v1_begin(&request) !=
      RUNTIME_MACHINE_STAGE_V1_REJECTED_ACTIVE) return 3;
  ...
  if (runtime_machine_stage_v1_arm_real_mode_entry(&entry) !=
      ...
      runtime_machine_stage_v1_copy_real_mode_entry(&observed) !=
      RUNTIME_MACHINE_STAGE_V1_OK || observed.cs != entry.cs ||
      observed.eip != entry.eip) return 4;
  entry.eip = 0x10000u;
  if (runtime_machine_stage_v1_arm_real_mode_entry(&entry) !=
      RUNTIME_MACHINE_STAGE_V1_REJECTED_ENTRY ||
      runtime_machine_stage_v1_copy_real_mode_entry(&observed) !=
      RUNTIME_MACHINE_STAGE_V1_OK || observed.cs != 0x70u ||
  ...
  if (runtime_machine_stage_v1_execute(&execution) !=
      RUNTIME_MACHINE_STAGE_V1_EXECUTION_BUDGET) return 7;
  if (runtime_machine_stage_v1_reset() != RUNTIME_MACHINE_STAGE_V1_OK ||
      runtime_machine_stage_v1_active()) return 8;
  ...
  if (!runtime_machine_stage_v1_request_valid(&request) ||
      runtime_machine_stage_v1_begin(&request) != RUNTIME_MACHINE_STAGE_V1_OK ||
      runtime_machine_stage_v1_reset() != RUNTIME_MACHINE_STAGE_V1_OK) return 9;
  return 0;
}

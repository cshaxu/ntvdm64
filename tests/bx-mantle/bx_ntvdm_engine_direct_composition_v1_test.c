...
static int request_set(struct runtime_engine_request_v1 *request,
    const wchar_t *profile, const wchar_t *root)
{
    ...
    if (!descriptor_set(request->profile_descriptor,
            RUNTIME_ENGINE_V1_MAX_DESCRIPTOR_CHARS, profile,
            &request->profile_descriptor_chars) ||
        !descriptor_set(request->root_descriptor,
            RUNTIME_ENGINE_V1_MAX_DESCRIPTOR_CHARS, root,
            &request->root_descriptor_chars) ||
        !descriptor_set(request->launch_descriptor,
            RUNTIME_ENGINE_V1_MAX_LAUNCH_CHARS, L"2,1,e,00",
            &request->launch_descriptor_chars)) return 0;
    request->instruction_tick_budget = UINT64_C(1000000);
    return runtime_engine_request_v1_valid(request);
}

static int terminal_valid(const struct runtime_engine_result_v1 *result)
{
    if (result->terminal_kind == RUNTIME_ENGINE_TERMINAL_V1_EXECUTION_BUDGET)
        return result->detail_code == RUNTIME_MACHINE_STAGE_V1_EXECUTION_BUDGET;
    if (result->terminal_kind == RUNTIME_ENGINE_TERMINAL_V1_CONTROLLED_GUEST_TERMINAL)
...
int wmain(int argc, wchar_t **argv)
{
    ...
    if (!runtime_engine_run_v1(&request, &result) ||

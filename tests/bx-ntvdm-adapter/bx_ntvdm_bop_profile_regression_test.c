...
static int classify(uint8_t selector, uint32_t count) {
    ...
    for(n=0;n<count;n++) { uint8_t b[4]={0xc4,0xc4,selector,(uint8_t)n};
        ...
        if(selector==0x56u||selector==0x57u){bx_ntvdm_system_plane_record_v1 p;bad|=!bx_ntvdm_system_plane_v1_classify(&i,&s,&p);bad|=p.service!=n;}
    } return bad;
}
int main(void) { bx_ntvdm_instruction_window_v1 w; bx_ntvdm_bop_ingress_v1 i; int bad=0; uint32_t n;
    bad|=classify(0x50u,73); bad|=classify(0x54u,17); bad|=classify(0x52u,12); bad|=classify(0x53u,25);bad|=classify(0x56u,16);bad|=classify(0x57u,49);

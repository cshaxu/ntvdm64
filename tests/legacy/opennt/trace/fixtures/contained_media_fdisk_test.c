...
BOOL runner_media_configure_fdisk_image(BYTE, LPCSTR, BOOL);
...
int main(void) {
    ...
    CloseHandle(file);
    if (runner_media_configure_fdisk_image(3, "\\\\.\\PhysicalDrive0", FALSE)) { DeleteFileA(path); return 4; }
    if (!runner_media_configure_fdisk_image(2, path, TRUE) || !nt_fdisk_init(2, &bpb, &geometry) || bpb.SectorSize != 512 || geometry.BytesPerSector != 512) { DeleteFileA(path); return 5; }

...
$cleanupMatch = [regex]::Match($source, '(?s)bx_machine_minimal_machine_status bx_machine_minimal_machine_c::cleanup\(void\)\s*\{(.*?)\n\}')
...
foreach ($pattern in @(
        'class bx_machine_minimal_machine_c',
        'bx_machine_minimal_machine_status initialize\(Bit64u guest, Bit64u host\);',
        'bx_machine_minimal_machine_status cleanup\(void\);',
...
foreach ($declaration in @($memory, $iodev)) {
    if ($declaration -notmatch 'friend class bx_machine_minimal_machine_c;') {
        throw 'Missing BX-MACH-026 friend declaration'
    }
}

foreach ($pattern in @(
        'logfunctions \*pluginlog = &bx_machine_minimal_machine_log;',
        ...
        'bx_machine_minimal_sim_initialize\(\)',
...
Assert-Ordered 'SAFE_GET_GENLOG();' 'bx_machine_minimal_sim_initialize()'
Assert-Ordered 'bx_machine_minimal_sim_initialize()' 'bx_mem.init_memory_without_sim(guest, host)'

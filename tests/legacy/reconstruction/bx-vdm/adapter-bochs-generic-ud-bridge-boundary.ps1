...
foreach ($pattern in @('struct bx_ntvdm_generic_ud_event_v1',
        ...
        'bx_ntvdm_machine_generic_ud_bridge_v1', 'bx_ntvdm_machine_generic_ud_bridge_v2',
        'bx_ntvdm_machine_generic_ud_stop_observed')) {
...
if ($core -notmatch 'bx_ntvdm_machine_generic_ud_bridge_v1' -or
    $core -notmatch 'bx_ntvdm_machine_generic_ud_bridge_v2' -or
    $core -notmatch 'SetCR0\(context_cr0\)' -or
    $core -notmatch 'writeEFlags\(machine_context_outcome\.eflags_values' -or
    $core -notmatch 'bx_pc_system\.kill_bochs_request = 1;') {
    throw 'The core must retain only the fixed machine bridge and controlled-stop mechanics.'
}

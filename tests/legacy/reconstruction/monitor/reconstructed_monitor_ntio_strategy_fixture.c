...
int main(int argc, char **argv)
{
    ...
    reconstructed_monitor_run_result run_result;
    uint32_t ptrsav;
    int result = 1;
    ...
    if (reconstructed_monitor_monitor_initialize(&monitor, ram, real_mode_ram_bytes, NULL, NULL) !=
    ...
    run_result = reconstructed_monitor_monitor_run(&monitor, 2u);
    if (run_result != RECONSTRUCTED_MONITOR_BUDGET_EXHAUSTED ||
        reconstructed_monitor_monitor_get_last_stop(&monitor, &stop) != RECONSTRUCTED_MONITOR_NO_STOP_EVENT) {

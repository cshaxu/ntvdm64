...
int main(void)
{
  ...
  if (!runtime_machine_checked_ram_write(address, shared, sizeof(shared))) return 2;
  ...
  runtime_cpu_state_initialize(&cpu, RUNTIME_CPU_EXECUTION_PROTECTED);
  cpu.eax = 1u;
  ...
  if (runtime_machine_stage_reset() != RUNTIME_MACHINE_STAGE_OK) return 11;
  return 0;
}

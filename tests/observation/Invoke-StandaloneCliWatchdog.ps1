...
$stdout = if ($UseInheritedConsole) { $null } else { $record + '.stdout.log' }
$stderr = if ($UseInheritedConsole) { $null } else { $record + '.stderr.log' }
$started = [DateTime]::UtcNow
$start = @{ FilePath = $exe; ArgumentList = $Arguments; PassThru = $true; NoNewWindow = $true }
if (!$UseInheritedConsole) {
...
$p.Dispose()
[ordered]@{exe=$exe;sha256=(Get-FileHash $exe -Algorithm SHA256).Hash.ToLowerInvariant();arguments=$Arguments;pid=$testProcessId;startedUtc=$started.ToString('o');elapsedMilliseconds=([DateTime]::UtcNow-$started).TotalMilliseconds;inheritedConsole=[bool]$UseInheritedConsole;exitedWithinWatchdog=$exited;killedByWatchdog=$killed;exitCode=$exitCode;stdout=$stdout;stderr=$stderr} | ConvertTo-Json | ForEach-Object { [IO.File]::WriteAllText($record,$_+[Environment]::NewLine,[Text.UTF8Encoding]::new($false)) }

...
int main(int argc, char **argv)
{
    ...
    if (root == NULL || strcmp(root, "C:\\installed\\softpc") != 0)
        return 2;
    root = session_mvdm_system_root(&instance);
    if (root == NULL || strcmp(root, "C:\\installed") != 0) return 3;
    /* The source-shaped root has no filesystem entry and must fail the distinct

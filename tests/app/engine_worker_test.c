...
{
    if (!runtime_engine_request_valid(request)) return 0;
    ...
    return runtime_engine_result_set(result,
        RUNTIME_ENGINE_TERMINAL_EXECUTION_BUDGET, 77u);
}

int runtime_engine_request_cancellation(uint32_t reason)
{
    if (!cancellation_mode || reason != RUNTIME_CANCELLATION_USER_REQUEST ||
...
static void request_set(struct runtime_engine_request *request)
{
...
int main(void)
{
    ...
    if (!app_engine_worker_run(&request, event, &result,
            &cancellation_accepted) || cancellation_accepted != 0u ||
        result.terminal_kind != RUNTIME_ENGINE_TERMINAL_EXECUTION_BUDGET ||
    ...
    if (!app_engine_worker_run(&request, event, &result,
        ...
        result.terminal_kind != RUNTIME_ENGINE_TERMINAL_HOST_CANCELLATION ||
        result.detail_code != RUNTIME_CANCELLATION_USER_REQUEST) return 4;
    CloseHandle(event);
    if (app_engine_worker_run(&request, NULL, &result,

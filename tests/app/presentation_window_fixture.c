...
int main(void)
{
    ...
    if (!session_valid(&owner) || !app_presentation_window_prepare(&window,
            &owner) || !app_presentation_window_open(&window)) return 1;
    ...
    if (!app_presentation_window_active(&window) ||
        ...
        owner.state != SESSION_STATE_ACTIVE || !session_dispose(&owner))
        return 4;
    return 0;
}

...
static void event_initialize(runtime_exception_event_v1 *e)
...
static int invoke(fixture_context *c,runtime_exception_event_v1 *e,runtime_cpu_state_v1 *cpu,runtime_cpu_result_v2 *r,runtime_command_misc_session *s,uint32_t service)
{ runtime_command_misc_call call;memset(&call,0,sizeof(call));call.magic=RUNTIME_COMMAND_MISC_CALL_MAGIC;call.abi_version=RUNTIME_COMMAND_MISC_CALL_VERSION;call.struct_bytes=sizeof(call);call.service=service;call.boundary=e;call.cpu=cpu;call.result=r;call.guest_state=c;call.guest_read=guest_read;call.guest_write=guest_write;call.session=s;return runtime_command_misc_invoke(&call)&&r->disposition==RUNTIME_CPU_RESULT_V2_RESUME&&r->resume_rip==e->fault_rip+4u; }
int main(void)
{
    fixture_context c; runtime_exception_event_v1 e; runtime_cpu_state_v1 cpu;
    runtime_cpu_result_v2 r; runtime_command_misc_session s;
    ...
    memset(&c,0,sizeof(c));event_initialize(&e);runtime_cpu_state_v1_initialize(&cpu,RUNTIME_CPU_EXECUTION_REAL);runtime_command_misc_session_initialize(&s);
    ...
    runtime_host_handle_manager_reset(s.handles);
    runtime_command_misc_session_initialize(&s);
    ...
    if(r.cpu_delta.gpr16_values[3]!=1u||r.cpu_delta.gpr16_values[1]!=0u||
        !runtime_host_handle_manager_lookup_handle(s.handles,
    ...
    runtime_command_misc_set_test_system_directory(directory);bPifFastPaste=FALSE;cpu.edx=1u;cpu.ds=0x100u;cpu.esi=0x100u;cpu.ecx=0x200u;
    if(!invoke(&c,&e,&cpu,&r,&s,RUNTIME_COMMAND_MISC_GET_KBD_LAYOUT)||r.cpu_delta.gpr16_values[2]!=1u||strstr((CHAR *)c.guest+0x1100,"KB16.COM")==NULL){fprintf(stderr,"kbd dx=%u program=%s options=%s\n",r.cpu_delta.gpr16_values[2],c.guest+0x1100,c.guest+0x1200);return 17;}
    RegOverridePredefKey(HKEY_LOCAL_MACHINE,NULL);RegCloseKey(root);RegDeleteTreeA(HKEY_CURRENT_USER,"Software\\ntdos64-t231-kbd");runtime_command_misc_set_test_system_directory(NULL);CloseHandle(kb16);CloseHandle(keyboard);DeleteFileA(kb16_path);DeleteFileA(keyboard_path);
    runtime_host_handle_manager_reset(s.handles);

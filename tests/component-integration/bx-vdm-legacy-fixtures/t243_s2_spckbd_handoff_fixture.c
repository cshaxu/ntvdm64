...
#include "opennt-bop/ingress/spckbd_handoff_v2_generic_ud_bridge.h"
...
int runtime_mantle_checked_ram_write_v1(uint64_t address, const uint8_t *bytes,
    uint32_t byte_count)
{
...
int main(void)
{
    ...
    initialize(&event);
    runtime_spckbd_handoff_v2_display_state_set(2u);
    ...
    if (runtime_spckbd_handoff_v2_generic_ud_dispatch(&event, &outcome) ||
        ram[0x3040u] != 0x44u) return 2;
    word_at(0x2100u + 32u * 2u, 0xbeefu); writable = 0;
    if (runtime_spckbd_handoff_v2_generic_ud_dispatch(&event, &outcome)) return 3;
    writable = 1; event.execution_mode = RUNTIME_CPU_EXECUTION_PROTECTED;
    if (runtime_spckbd_handoff_v2_generic_ud_recognizes(&event)) return 4;
    event.execution_mode = RUNTIME_CPU_EXECUTION_REAL; event.eax = 0xbeeu;
    if (runtime_spckbd_handoff_v2_generic_ud_recognizes(&event)) return 5;

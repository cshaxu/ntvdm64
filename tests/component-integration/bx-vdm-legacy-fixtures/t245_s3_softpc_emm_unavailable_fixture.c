...
#include "opennt-bop/ingress/softpc_emm_unavailable_v2_generic_ud_bridge.h"
#include "opennt-bop/ingress/opennt_bop_route.h"
...
int main(void)
{
    ...
    if (!runtime_softpc_emm_unavailable_v2_generic_ud_recognizes(&event) ||
        !runtime_softpc_emm_unavailable_v2_generic_ud_dispatch(&event,
    ...
    if (runtime_softpc_emm_unavailable_v2_generic_ud_recognizes(&event)) return 3;
    event.window[2] = 0x66u;
    event.fault_rip = UINT64_MAX - 2u;
    if (runtime_softpc_emm_unavailable_v2_generic_ud_dispatch(&event,
            &outcome)) return 4;
    return 0;
}

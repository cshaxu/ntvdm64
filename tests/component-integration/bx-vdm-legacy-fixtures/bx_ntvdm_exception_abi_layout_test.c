...
int main(void)
{
    ...
    if (!runtime_exception_event_v1_valid(&event) ||
        runtime_exception_event_v1_valid(0)) {
        return 2;
    }
    if (!runtime_exception_result_v1_resume(&result, event.fault_rip + 2u) ||
        runtime_exception_result_v1_resume(0, event.fault_rip)) {
        return 3;
    }
    ...
    event.kind = 0u;
    if (runtime_exception_event_v1_valid(&event)) { return 5; }
    event.kind = RUNTIME_EXCEPTION_EVENT_CPU_EXCEPTION;
    ...
    if (!runtime_exception_dispatch_v1(&event, &result) ||
        result.disposition != RUNTIME_EXCEPTION_RESULT_PASS_THROUGH ||
        runtime_exception_dispatch_v1(0, &result)) { return 7; }
    runtime_cpu_state_v1_initialize(&state, RUNTIME_CPU_EXECUTION_REAL);
    if (!runtime_exception_dispatch_state_v1(&event, &state, &result) ||
        result.disposition != RUNTIME_EXCEPTION_RESULT_PASS_THROUGH) { return 8; }
    state.execution_mode = 0u;
    if (runtime_exception_dispatch_state_v1(&event, &state, &result)) { return 9; }
    runtime_cpu_state_v1_initialize(&state, RUNTIME_CPU_EXECUTION_REAL);
    runtime_instruction_window_v1_capture(&window, 0, 0u);
    if (!runtime_exception_dispatch_state_window_v1(&event, &state, &window,

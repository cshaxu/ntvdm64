...
{ fixture_context *c = s; uint32_t guest_handle; if (!c || !runtime_host_handle_manager_publish(c->handles, h, RUNTIME_HOST_HANDLE_OWNED, &guest_handle, e)) return 0; if (t) *t = guest_handle; return 1; }
static int lookup(void *s, uint32_t t, HANDLE *h)
{ fixture_context *c = s; if (h) *h = INVALID_HANDLE_VALUE; return c != NULL && t != 0u && t != UINT32_MAX && runtime_host_handle_manager_lookup_handle(c->handles, t, h); }
static int release(void *s, uint32_t t, DWORD *e)
{ fixture_context *c = s; if (!c || t == 0u || t == UINT32_MAX) { if (e) *e = ERROR_INVALID_HANDLE; return 0; } return runtime_host_handle_manager_release(c->handles, t, e); }
...
int main(void)
{
    ...
    if (!runtime_host_handle_manager_initialize(state.handles)) return 1;
    ...
    if (!runtime_demdisp_invoke(&call) ||
    ...
    if (!runtime_demdisp_invoke(&call) ||
        ...
        (result.cpu_delta.gpr16_write_mask & ((1u << 0) | (1u << 5))) !=
            ((1u << 0) | (1u << 5)) ||
        !runtime_host_handle_manager_valid(state.handles)) return 7;
    ...
    if (!runtime_host_handle_manager_release(state.handles, fcb_handle, NULL)) return 7;
    memcpy(state.guest + 0x1000u, temporary, strlen(temporary) + 1u);
    runtime_cpu_state_v1_initialize(&cpu, RUNTIME_CPU_EXECUTION_REAL);
    cpu.ds = 0x100u;
    call.service = 0x31u;
    if (!runtime_demdisp_invoke(&call) ||
        (result.eflags_values & RUNTIME_CPU_RESULT_V2_EFLAGS_CF) != 0u) return 8;
    ...
    if (!runtime_demdisp_invoke(&call) ||
        ...
        !runtime_host_handle_manager_valid(state.handles)) return 11;
    fcb_handle = ((uint32_t)result.cpu_delta.gpr16_values[0] << 16u) |
        result.cpu_delta.gpr16_values[5];
    runtime_cpu_state_v1_initialize(&cpu, RUNTIME_CPU_EXECUTION_REAL);
    cpu.eax = fcb_handle >> 16u;
    ...
    if (!runtime_demdisp_invoke(&call) ||
        (result.eflags_values & RUNTIME_CPU_RESULT_V2_EFLAGS_CF) != 0u ||
        runtime_host_handle_manager_lookup_handle(state.handles,
            fcb_handle, &seed)) return 12;
    runtime_cpu_state_v1_initialize(&cpu, RUNTIME_CPU_EXECUTION_REAL);
    call.service = 0x3cu;
    if (!runtime_demdisp_invoke(&call) ||
        result.disposition != RUNTIME_CPU_RESULT_V2_RESUME) return 13;

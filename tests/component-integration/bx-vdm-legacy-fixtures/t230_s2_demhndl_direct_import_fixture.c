...
static void direct_initialize(runtime_dem_direct_context *direct,
    ...
    runtime_cpu_state_v1 *cpu, runtime_cpu_result_v2 *result)
{
    ...
    call.guest_read = guest_read;
    call.guest_write = guest_write;
    return runtime_demdisp_invoke(&call) && CurrentISVC == call.service &&
        result->disposition == RUNTIME_CPU_RESULT_V2_RESUME &&
        result->resume_rip == event->fault_rip + 4u &&
    ...
    runtime_cpu_state_v1 *cpu, runtime_cpu_result_v2 *result)
{
    ...
    call.guest_read = guest_read;
    call.guest_write = guest_write;
    runtime_instruction_window_v1_capture(&window, instruction,
...
int main(void)
{
    ...
    if (!runtime_dem_direct_context_valid(&direct) ||
        runtime_demdisp_handler_at(RUNTIME_DEMHNDL_READ) != demRead) return 3;
    ...
    if (!invoke(&state, &direct, &event, &cpu, &result,
            RUNTIME_DEMHNDL_WRITE) || result.cpu_delta.gpr16_values[0] != 5u ||
        (result.eflags_values & RUNTIME_CPU_RESULT_V2_EFLAGS_CF) != 0u) return 5;

    cpu.ebx = 0u; cpu.esi = 0u; cpu.ecx = 0u; cpu.edx = 0u; cpu.eflags = 0u;
    if (!invoke(&state, &direct, &event, &cpu, &result,
            RUNTIME_DEMHNDL_CHG_FILE_PTR) || result.cpu_delta.gpr16_values[0] != 0u)
        return 5;
    memset(state.guest + address, 0, 5u);
    cpu.ecx = 5u; cpu.eflags = 0x40u;
    if (!invoke(&state, &direct, &event, &cpu, &result,
            RUNTIME_DEMHNDL_READ) || result.cpu_delta.gpr16_values[0] != 5u ||
    ...
    if (!invoke(&state, &direct, &event, &cpu, &result,
            RUNTIME_DEMHNDL_FILE_TIMES) ||
        (result.eflags_values & RUNTIME_CPU_RESULT_V2_EFLAGS_CF) != 0u) return 7;
    if (!invoke(&state, &direct, &event, &cpu, &result,
            RUNTIME_DEMHNDL_COMMIT)) return 8;
    if (!invoke(&state, &direct, &event, &cpu, &result,
            RUNTIME_DEMHNDL_PIPE_DATA_EOF) ||
        (result.eflags_write_mask & RUNTIME_CPU_RESULT_V2_EFLAGS_ZF) == 0u ||
        (result.eflags_values & RUNTIME_CPU_RESULT_V2_EFLAGS_ZF) != 0u ||
        (result.eflags_values & RUNTIME_CPU_RESULT_V2_EFLAGS_CF) == 0u) return 9;
    if (!invoke(&state, &direct, &event, &cpu, &result,
            RUNTIME_DEMHNDL_PIPE_EOF)) return 10;
    ...
    if (!invoke(&state, &direct, &event, &cpu, &result,
            RUNTIME_DEMHNDL_CHG_FILE_PTR) ||
        result.cpu_delta.gpr16_values[0] != ERROR_INVALID_HANDLE ||
        (result.eflags_values & RUNTIME_CPU_RESULT_V2_EFLAGS_CF) == 0u) return 11;
    ...
    if (!invoke(&state, &direct, &event, &cpu, &result,
            RUNTIME_DEMHNDL_CHG_FILE_PTR) ||
        result.cpu_delta.gpr16_values[0] != ERROR_INVALID_HANDLE ||
        (result.eflags_values & RUNTIME_CPU_RESULT_V2_EFLAGS_CF) == 0u) return 12;

    cpu.eax = 1u;
    cpu.ebp = 1u;
    cpu.ecx = 0xffffu; cpu.edx = 0xffffu;
    if (!invoke(&state, &direct, &event, &cpu, &result,
            RUNTIME_DEMHNDL_CLOSE) || !state.released) return 13;
    DeleteFileW(path);

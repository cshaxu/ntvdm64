...
int main(void)
{
 fixture_context c;bx_ntvdm_exception_event_v1 e;bx_ntvdm_cpu_state_v1 cpu;bx_ntvdm_cpu_result_v2 r;bx_ntvdm_command_misc_session s;PARAMBLOCK p;CHAR path[MAX_PATH],converted[16]; ANSI_STRING ansi; UNICODE_STRING unicode; uint32_t scs=0x4000u,tail=0x2800u;
 ...
 if(!invoke(&c,&e,&cpu,&r,&s)||r.disposition!=BX_NTVDM_CPU_RESULT_V2_RESUME||r.eflags_values!=0u||r.cpu_delta.segment_write_mask!=((1u<<0)|(1u<<3))||r.cpu_delta.segment_values[3]!=(USHORT)(scs>>4)||r.cpu_delta.segment_values[0]!=(USHORT)((scs+offsetof(SCSINFO,SCS_ParamBlock))>>4))return 4;

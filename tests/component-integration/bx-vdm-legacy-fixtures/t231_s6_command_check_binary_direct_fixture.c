...
static void event_init(runtime_exception_event_v1 *e)
...
static int invoke(fixture_context *c,runtime_exception_event_v1 *e,runtime_cpu_state_v1 *cpu,runtime_cpu_result_v2 *r,runtime_command_misc_session *s)
...
int main(void)
{
 fixture_context c;runtime_exception_event_v1 e;runtime_cpu_state_v1 cpu;runtime_cpu_result_v2 r;runtime_command_misc_session s;PARAMBLOCK p;CHAR path[MAX_PATH],converted[16]; ANSI_STRING ansi; UNICODE_STRING unicode; uint32_t scs=0x4000u,tail=0x2800u;
 memset(&c,0,sizeof(c));event_init(&e);runtime_cpu_state_v1_initialize(&cpu,RUNTIME_CPU_EXECUTION_REAL);runtime_command_misc_session_initialize(&s);
 ...
 if(!invoke(&c,&e,&cpu,&r,&s)||r.disposition!=RUNTIME_CPU_RESULT_V2_RESUME||r.eflags_values!=0u||r.cpu_delta.segment_write_mask!=((1u<<0)|(1u<<3))||r.cpu_delta.segment_values[3]!=(USHORT)(scs>>4)||r.cpu_delta.segment_values[0]!=(USHORT)((scs+offsetof(SCSINFO,SCS_ParamBlock))>>4))return 4;
 if(strcmp(s.scs_info.SCS_ComSpec,"C:\\NTDOS64\\COMMAND.COM")||s.scs_info.SCS_CmdTail[0]<(UCHAR)(3u+strlen(path))||memcmp(s.scs_info.SCS_CmdTail+1,"/z ",3u)||memcmp(c.guest+scs,&s.scs_info,sizeof(s.scs_info)))return 5;

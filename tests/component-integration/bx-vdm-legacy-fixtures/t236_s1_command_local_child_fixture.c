...
static int invoke(t236_fixture_context *context,
    ...
    runtime_cpu_result_v2 *result, runtime_command_misc_session *session,
    uint32_t service)
{
    ...
    uint32_t service)
{
    ...
    if (!invoke(context, event, cpu, result, session, service) ||
        result->disposition != RUNTIME_CPU_RESULT_V2_PENDING) return 0;
    for (attempt = 0u; attempt < 100u; ++attempt) {
        Sleep(10u);
        if (!invoke(context, event, cpu, result, session, service)) return 0;
        if (result->disposition != RUNTIME_CPU_RESULT_V2_PENDING) return 1;
    }
    return 0;
}
...
int main(void)
{
    ...
    if (!invoke_pending_completion(&context, &event, &cpu, &result, &session,
            RUNTIME_COMMAND_MISC_EXEC)) return 11;
    if (result.disposition != RUNTIME_CPU_RESULT_V2_RESUME) return 12;
    if ((result.eflags_values & RUNTIME_CPU_RESULT_V2_EFLAGS_CF) != 0u) return 13;
    ...
    if (!CreatePipe(&pipe_read, &pipe_write, &security, 0u) ||
        ...
        !runtime_host_handle_manager_publish(session.handles, pipe_write,
            RUNTIME_HOST_HANDLE_BORROWED, &standard_token, &error)) return 4;
    standard_handles[1] = standard_token;
    cpu.eax = 0x0102u;
    memcpy(context.guest + 0x1000u, "echo T236\r", 10u);
    if (!invoke_pending_completion(&context, &event, &cpu, &result, &session,
            RUNTIME_COMMAND_MISC_EXEC)) return 51;
    if ((result.eflags_values & RUNTIME_CPU_RESULT_V2_EFLAGS_CF) != 0u) return 52;
    ...
    if (!invoke(&context, &event, &cpu, &result, &session,
            RUNTIME_COMMAND_MISC_EXEC) ||
        result.disposition != RUNTIME_CPU_RESULT_V2_PENDING) return 57;

...
static int cf(const runtime_cpu_result_v2*r){return (r->eflags_values&RUNTIME_CPU_RESULT_V2_EFLAGS_CF)!=0;}
static int invoke(context*c,runtime_dem_direct_context*d,runtime_exception_event_v1*e,runtime_cpu_state_v1*cpu,runtime_cpu_result_v2*r,uint32_t svc){runtime_demhndl_call x;memset(&x,0,sizeof(x));x.magic=RUNTIME_DEMHNDL_CALL_MAGIC;x.abi_version=RUNTIME_DEMHNDL_CALL_VERSION;x.struct_bytes=sizeof(x);x.service=svc;x.direct=d;x.boundary=e;x.cpu=cpu;x.result=r;x.guest_state=c;x.guest_read=readg;x.guest_write=writeg;return runtime_demdisp_invoke(&x)&&r->disposition==RUNTIME_CPU_RESULT_V2_RESUME;}
static void cpuinit(runtime_cpu_state_v1*c){runtime_cpu_state_v1_initialize(c,RUNTIME_CPU_EXECUTION_REAL);c->ds=0x100;c->es=0x100;}
int main(void){char tmp[MAX_PATH],dir[MAX_PATH],old[MAX_PATH];context c;runtime_dem_direct_context d;runtime_exception_event_v1 e;runtime_cpu_state_v1 cpu;runtime_cpu_result_v2 r;CDS cds;DWORD n;char drive;

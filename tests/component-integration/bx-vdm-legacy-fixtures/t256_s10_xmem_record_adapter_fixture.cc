...
int main(void)
{
  ...
  if (runtime_dpmi_xmem_record_adapter_allocate(&adapter, 0x10u, 0x10000u,
      &first_address, &first_id) != RUNTIME_DPMI_XMEM_RECORD_ADAPTER_MANTLE_FAILURE)
    return 2;
  ...
  if (!runtime_mantle_checked_ram_write_v1(first_address, written,
  ...
  if (runtime_dpmi_xmem_record_adapter_reallocate(&adapter, first_id, 0x20000u,
      ...
      !runtime_mantle_checked_ram_read_v1(replacement_address, observed,

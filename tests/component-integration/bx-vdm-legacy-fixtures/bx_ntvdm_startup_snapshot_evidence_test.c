...
int main(void)
{
    ...
    runtime_startup_snapshot_range_v1 ranges[2] = {
    ...
    runtime_startup_snapshot_transaction_v1_initialize(&transaction, &event,
        &state, ranges, 2u);
    if (GetTempPathW(MAX_PATH, root) == 0 || GetTempFileNameW(root, L"n64", 0u, root) == 0 ||
        !DeleteFileW(root) || !CreateDirectoryW(root, 0) ||
        !runtime_startup_snapshot_evidence_v1_write(root, L"evidence.json",
            &transaction, output, sizeof(output), UINT64_C(0x1122334455667788)) ||
        runtime_startup_snapshot_evidence_v1_write(root, L"..\\bad.json",

...
int main(void)
{
    ...
    struct runtime_dem_open_observation_v1 value;
    ...
    runtime_dem_open_observation_v1_enable(1u);
    runtime_dem_open_observation_v1_consider(&event, &outcome, read_guest, ram);
    memcpy(ram->bytes + address, "O:\\X\\COMMAND.COM", 17u);
    runtime_dem_open_observation_v1_consider(&event, &outcome, read_guest, ram);
    if (!runtime_dem_open_observation_v1_copy(&value) || !value.observed ||

...
static int carry_set(const runtime_cpu_result_v2 *result)
{ return (result->eflags_values & RUNTIME_CPU_RESULT_V2_EFLAGS_CF) != 0u; }
    ...
    runtime_cpu_result_v2 *result, uint32_t service)
{
    ...
    return runtime_demdisp_invoke(&call) && result->disposition == RUNTIME_CPU_RESULT_V2_RESUME;
}
...
int main(void)
{
    ...
    runtime_cpu_state_v1 cpu; runtime_cpu_result_v2 result; fixture_context state;
    HANDLE file;

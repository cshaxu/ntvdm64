...
int main(void)
{
    ...
    runtime_observation_transaction_v1_initialize(&transaction, &event, &state, &range);
    if (sizeof(transaction) != 144u || !runtime_observation_transaction_v1_preflight(
        &transaction, UINT64_C(0x100000), 8u)) return 1;
    transaction.guest_read.length = 0u;
    if (runtime_observation_transaction_v1_preflight(&transaction, UINT64_C(0x100000), 0u)) return 2;
    transaction.guest_read.length = 8u;
    transaction.guest_read.address = UINT64_C(0xfffff);
    if (runtime_observation_transaction_v1_preflight(&transaction, UINT64_C(0x100000), 8u)) return 3;
    transaction.guest_read.address = UINT64_C(0x700);
    transaction.flags = 1u;
    if (runtime_observation_transaction_v1_preflight(&transaction, UINT64_C(0x100000), 8u)) return 4;
    transaction.flags = 0u;
    transaction.boundary.vector = 5u;
    if (runtime_observation_transaction_v1_preflight(&transaction, UINT64_C(0x100000), 8u)) return 5;
    runtime_observation_transaction_v1_initialize(0, 0, 0, 0);
    return runtime_observation_transaction_v1_preflight(0, 0u, 0u) ? 6 : 0;
}

...
#include "adapter-softpc/bx_ntvdm_normal_return_observation_v1.h"
...
extern "C" void t198_s23_native_ntio_boundary_bind_observation_composition(const bx_ntvdm_boot_namespace_composition_v1 *value);
extern "C" unsigned t198_s23_native_ntio_boundary_dem_lifecycle_count(void);
extern "C" int t198_s23_native_ntio_boundary_copy_dem_lifecycle(unsigned index, struct t198_s121_dem_lifecycle_entry_v1 *entry);
...
int main()
{
...
#endif
  byob_profile_selection p; byob_launch_plan_v2 launch; bx_ntvdm_boot_namespace_composition_v1 c; bx_ntvdm_native_bop_composition_v1 native_bop; bx_ntvdm_host_drive_snapshot_v1 drives; bx_ntvdm_preentry_input_v1 preentry; t200_s3_cmdinfo_observation_v1 cmdinfo; uint8_t drive_types[26]={0};
  static bx_ntvdm_finite_run_request r; bx_ntvdm_finite_run_terminal_snapshot terminal; bx_ntvdm_generic_ud_event_v1 generic; bx_ntvdm_instruction_history_record_v1 history[8]; bx_ntvdm_instruction_history_transition_v1 transition; bx_ntvdm_instruction_history_provenance_v1 provenance; struct bx_ntvdm_normal_return_observation_v1 normal_return; unsigned terminal_valid, generic_valid, history_count, transition_valid, provenance_valid, normal_return_valid; int status;
...
#ifdef T203_S1_PASSIVE_NORMAL_RETURN_OBSERVATION
  bx_ntvdm_normal_return_observation_v1_enable(1u);
#endif
  memset(&terminal,0,sizeof(terminal)); memset(&generic,0,sizeof(generic)); memset(history,0,sizeof(history)); memset(&transition,0,sizeof(transition)); memset(&provenance,0,sizeof(provenance)); r.capture_terminal_snapshot=1; status=(int)bx_ntvdm_run_finite_bare_bytes(&r); terminal_valid=bx_ntvdm_finite_run_terminal_snapshot_get(&terminal); generic_valid=t198_s23_native_ntio_boundary_copy_first_generic_event(&generic); history_count=t198_s23_native_ntio_boundary_observed_first_generic_history_count(); for(unsigned n=0;n<history_count;n++)if(!t198_s23_native_ntio_boundary_copy_first_generic_history(n,&history[n]))return 1; transition_valid=t198_s23_native_ntio_boundary_copy_first_generic_cs_transition(&transition); provenance_valid=t198_s23_native_ntio_boundary_copy_first_generic_cs_provenance(&provenance); bx_ntvdm_native_bop_composition_v1_unbind(&native_bop); bx_ntvdm_boot_namespace_composition_v1_unbind(&c);
  normal_return_valid=bx_ntvdm_normal_return_observation_v1_copy(&normal_return); bx_ntvdm_normal_return_observation_v1_enable(0u);
  t200_s3_native_ntio_boundary_copy_cmdinfo_observation(&cmdinfo);
  fprintf(stderr,"t200-s3 cmdinfo observed=%u geometry=%u read=%u decoded=%u ds-dx=%04x:%04x env=%04x:%04x cmd=%04x:%04x:%u exe=%04x:%04x:%u accepted=%u outcome=%u bootstrap=%u launch=%u drives=%u delivered=%u reg=%u\n",cmdinfo.observed,cmdinfo.geometry_valid,cmdinfo.read_ok,cmdinfo.decoded,cmdinfo.ds,(unsigned)(uint16_t)cmdinfo.edx,cmdinfo.environment_segment,cmdinfo.environment_bytes,cmdinfo.command_segment,cmdinfo.command_offset,cmdinfo.command_bytes,cmdinfo.executable_segment,cmdinfo.executable_offset,cmdinfo.executable_bytes,cmdinfo.accepted,cmdinfo.disposition,cmdinfo.bootstrap_stage,cmdinfo.has_launch_plan,cmdinfo.has_drive_snapshot,cmdinfo.delivered,cmdinfo.launch_registration_valid);
  fprintf(stderr,"t198-s23 status=%d observed-5011=%u observed-503b-resume=%u observed-dta-resume=%u observed-hard-error-resume=%u observed-system-symbol-resume=%u observed-drive-resume=%u observed-first-generic-ud=%u fault=%04x:%04x mode=%u vector=%u window=%u:%02x%02x%02x%02x state=%u:%04x:%04x ss-sp=%04x:%04x ds-si=%04x:%04x es-di=%04x:%04x eax=%08x ebx=%08x ecx=%08x edx=%08x ebp=%08x flags=%08x observed-stop=%u next=%02x:%02x plane=%u/%u/%u error-lock=%u gset=%u terminal=%u:%04x:%04x snapshot=%u:%05x:%u call-dev-ad=%02x%02x%02x%02x history=%u\n",status,
    ...
    t198_s23_native_ntio_boundary_observed_selector(),t198_s23_native_ntio_boundary_observed_service(),c.plane.pending_kind,c.plane.pending_action_id,c.plane.has_dta,c.dem.error_lock.has_registration,c.dem.drive_view.gset.has_drive_snapshot,
  ...
  for(unsigned n=0;n<t198_s23_native_ntio_boundary_dem_lifecycle_count();++n){struct t198_s121_dem_lifecycle_entry_v1 e;if(!t198_s23_native_ntio_boundary_copy_dem_lifecycle(n,&e))return 1;fprintf(stderr,"t198-s23 dem-lifecycle=%u service=%02x accepted=%u state=%04x:%04x token=%08x disposition=%u ax=%04x bp=%04x ns-open=%u ns-file=%u ns-generation=%08x ns-offset=%llx\n",n,e.service,e.accepted,e.cs,e.eip,e.token,e.disposition,e.ax,e.bp,e.namespace_open,e.namespace_file_index,e.namespace_generation,(unsigned long long)e.namespace_offset);}
...
#ifdef T203_S1_PASSIVE_NORMAL_RETURN_OBSERVATION
  fprintf(stderr,"t203-s1 normal-return observed=%u captured=%u cs=%04x eip=%04x dx=%04x ax=%04x disposition=%u resume=%llx\n",normal_return_valid,normal_return.captured,normal_return.event.cs,(unsigned)normal_return.event.eip,normal_return.input_dx,normal_return.returned_ax,normal_return.outcome.disposition,(unsigned long long)normal_return.outcome.resume_rip);
  return status==BX_NTVDM_FINITE_RUN_COMPLETED_UD_STOP && terminal_valid && normal_return_valid && normal_return.magic==BX_NTVDM_NORMAL_RETURN_OBSERVATION_V1_MAGIC && normal_return.abi_version==BX_NTVDM_NORMAL_RETURN_OBSERVATION_V1_VERSION && normal_return.struct_bytes==sizeof(normal_return) && normal_return.captured==1u && normal_return.event.window_bytes>=4u && normal_return.event.window[0]==0xc4u && normal_return.event.window[1]==0xc4u && normal_return.event.window[2]==0x54u && normal_return.event.window[3]==0x0bu && normal_return.outcome.disposition==BX_NTVDM_GENERIC_UD_RESUME && normal_return.outcome.resume_rip==normal_return.event.fault_rip+4u && (normal_return.outcome.gpr16_write_mask&1u)!=0u && normal_return.returned_ax==normal_return.input_dx ? 0 : 2;
#endif

...
int main(void)
{
 runtime_exception_event event;runtime_cpu_state cpu;runtime_cpu_result result;runtime_command_misc_call call;
 memset(&event,0,sizeof(event));event.magic=RUNTIME_EXCEPTION_ABI_MAGIC;event.abi_version=RUNTIME_EXCEPTION_ABI_VERSION;event.struct_bytes=sizeof(event);event.kind=RUNTIME_EXCEPTION_EVENT_CPU_EXCEPTION;event.vector=6u;event.fault_rip=0x500u;runtime_cpu_state_initialize(&cpu,RUNTIME_CPU_EXECUTION_REAL);
 ...
 if(!runtime_command_misc_invoke(&call)||result.disposition!=RUNTIME_CPU_RESULT_STOP||result.resume_rip!=0u||result.cpu_delta.gpr16_write_mask!=0u)return 1;

...
#include "support/runtime_host_transaction_abi.h"
...
#include "support/runtime_host_transaction_abi.c"
...
int main(void)
{
    ...
    runtime_guest_write_initialize(&write, 0x8b00u, 0x200u, 0u);
    runtime_host_transaction_initialize(&transaction, &request, &state,
        &write);
    if (sizeof(transaction) != 136u ||
        !runtime_host_transaction_preflight(&transaction, 0x100000u,
            0x200u)) return 1;
    transaction.request.service = 0x49u;
    if (runtime_host_transaction_preflight(&transaction, 0x100000u,
        0x200u)) return 2;
    ...
    if (runtime_host_transaction_preflight(&transaction, 0x100000u,
        0x200u)) return 4;
    return runtime_host_transaction_preflight(0, 0u, 0u) ? 5 : 0;
}

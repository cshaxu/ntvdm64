    ...
    runtime_cpu_result *result, uint32_t service)
{
    ...
    return runtime_demdisp_invoke(&call) && runtime_cpu_result_valid(result);
}
...
int main(void)
{
    ...
    runtime_exception_event event; runtime_cpu_state cpu;
    runtime_cpu_result result; char temp[MAX_PATH], directory[MAX_PATH];
    ...
    reset_cpu(&cpu); if (!invoke(&state, &direct, &event, &cpu, &result, 0x3eu) ||
        result.disposition != RUNTIME_CPU_RESULT_RESUME) return 14;

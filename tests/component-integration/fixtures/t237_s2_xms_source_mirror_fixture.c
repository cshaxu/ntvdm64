...
static int invoke(fixture_memory *memory, uint32_t service,
    uint16_t ax, uint16_t bx, uint16_t dx, runtime_cpu_result *result)
{
...
static uint16_t result_gpr16(const runtime_cpu_result *result, uint32_t index)
{
...
int main(void)
{
    ...
    /* 52:00 and 52:06 require the machine A20 lifecycle, while 52:09

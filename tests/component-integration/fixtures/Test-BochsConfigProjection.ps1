...
try {
    ...
    $manifest = [ordered]@{
        ...
        additions = @([ordered]@{ name = 'runtime_ENABLE_EXCEPTION_INTERCEPT'; expectedSource = '<absent>'; newValue = '0'; classification = 'default-off' })
        derivedAssertions = @()
    }
    ...
    if ($projected -notmatch '#define runtime_ENABLE_EXCEPTION_INTERCEPT 0') { throw 'Default-off addition was not projected.' }
    ...
    $v2Synthetic = [ordered]@{
        ...
        additions = @(
            [ordered]@{ name = 'runtime_ENABLE_EXCEPTION_INTERCEPT'; expectedSource = '<absent>'; newValue = '0'; classification = 'default-off' },

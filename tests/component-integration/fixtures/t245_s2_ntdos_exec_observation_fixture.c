...
#include "opennt-bop/observation/ntdos_exec_observation.h"
...
int main(void)
{
    ...
    if (!runtime_ntdos_exec_observation_pdb_range(UINT16_C(0x1234),
            &physical, &bytes) || physical != UINT32_C(0x12340) ||
        bytes != RUNTIME_NTDOS_EXEC_PDB_PREFIX_BYTES) return 1;
    if (runtime_ntdos_exec_observation_pdb_range(UINT16_C(0xffff),
            &physical, &bytes)) return 2;
    if (runtime_ntdos_exec_observation_decode(UINT16_C(0x1234), prefix,
            RUNTIME_NTDOS_EXEC_PDB_PREFIX_BYTES - 1u, &observation) ||
        runtime_ntdos_exec_observation_decode(UINT16_C(0xffff), prefix,

...
int main(void)
{
    ...
    runtime_startup_snapshot_range ranges[3] = {
    ...
    runtime_startup_snapshot_transaction_initialize(
        &transaction, &event, &state, ranges, 3u);
    if (sizeof(transaction) != 240u ||
        !runtime_startup_snapshot_transaction_preflight(
            &transaction, UINT64_C(0x100000), 53u)) return 1;
    transaction.ranges[2].id = 12u;
    if (runtime_startup_snapshot_transaction_preflight(
    ...
    runtime_startup_snapshot_transaction_initialize(0, 0, 0, 0, 0u);
    return runtime_startup_snapshot_transaction_preflight(0, 0u, 0u) ? 6 : 0;
}

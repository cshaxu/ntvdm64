 ...
 * composition dependency.  The retained `` suffix denotes the frozen wire
 * layout, not a provider implementation selection.
 */
typedef enum runtime_host_service_family {
    RUNTIME_HOST_SERVICE_FAMILY_NONE = 0,
    RUNTIME_HOST_SERVICE_FAMILY_DEM = 1,
    RUNTIME_HOST_SERVICE_FAMILY_COMMAND = 2
} runtime_host_service_family;

typedef struct runtime_host_service_request {
    uint8_t selector;
    uint8_t service;
    uint16_t family;
} runtime_host_service_request;

...
static void make_event(struct runtime_generic_ud_event *event, uint8_t service)
{
...
static int expect(struct runtime_generic_ud_outcome *outcome, int carry,
    uint16_t ax)
{
...
static int mailslot_regression(void)
{
    ...
    const char name[] = "\\MAILSLOT\\runner-t251-s4";
    ...
    if (!runtime_redir_generic_ud_dispatch(&event, &outcome) ||
     ...
     * guest-owned continuation receives only copied 16:16 values. */
    {
        ...
        uint32_t token = 0u;
        uint32_t spins;
        server = CreateNamedPipeW(L"\\\\.\\pipe\\runner-t253-async", PIPE_ACCESS_OUTBOUND,
            PIPE_TYPE_BYTE | PIPE_READMODE_BYTE | PIPE_WAIT, 1u, 64u, 64u, 0u, NULL);
        if (server == INVALID_HANDLE_VALUE) return 0;
        client = CreateFileW(L"\\\\.\\pipe\\runner-t253-async", GENERIC_READ, 0u,
        ...
        if (client == INVALID_HANDLE_VALUE ||
            ...
            !VrAddOpenNamedPipeInfo(client, "\\\\.\\PIPE\\runner-t253-async") ||
        ...
        if (!runtime_redir_generic_ud_dispatch(&event, &outcome) || !expect(&outcome, 0, 0u)) {
            CloseHandle(server); return 0;
        }
        for (spins = 0u; spins < 100u; ++spins) {
            make_event(&event, 0x26u);
            if (!runtime_redir_generic_ud_dispatch(&event, &outcome)) { CloseHandle(server); return 0; }
    ...
    if (!runtime_redir_generic_ud_dispatch(&event, &outcome) || !expect(&outcome, 0, 64u)) return 0;
    ...
    if (!runtime_redir_generic_ud_dispatch(&event, &outcome) || !expect(&outcome, 0, 0u)) return 0;
    ...
    if (!runtime_redir_generic_ud_dispatch(&event, &outcome) || !expect(&outcome, 0, sizeof(message)) ||
...
int main(void)
{
    ...
    char bytes[8] = {0};
    char pipe_name[] = "\\\\localhost\\PIPE\\runner-t251-s3";
    ...
    server = CreateNamedPipeW(L"\\\\.\\pipe\\runner-t251-s3", PIPE_ACCESS_DUPLEX,
        PIPE_TYPE_BYTE | PIPE_READMODE_BYTE | PIPE_WAIT, 1u, 64u, 64u, 0u, NULL);
    ...
    client = CreateFileW(L"\\\\.\\pipe\\runner-t251-s3", GENERIC_READ | GENERIC_WRITE,

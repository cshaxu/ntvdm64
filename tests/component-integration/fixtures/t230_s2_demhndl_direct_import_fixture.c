...
int main(void)
{
    ...
    cpu.eflags = RUNTIME_CPU_RESULT_EFLAGS_CF;
    ...
    if (!invoke(&state, &direct, &event, &cpu, &result,
        ...
        (result.eflags_values & RUNTIME_CPU_RESULT_EFLAGS_CF) != 0u) return 5;
    ...
    if (!invoke(&state, &direct, &event, &cpu, &result,
        ...
        (result.eflags_values & RUNTIME_CPU_RESULT_EFLAGS_CF) != 0u) return 7;
    if (!invoke(&state, &direct, &event, &cpu, &result,
            RUNTIME_DEMHNDL_COMMIT)) return 8;
    if (!invoke(&state, &direct, &event, &cpu, &result,
            RUNTIME_DEMHNDL_PIPE_DATA_EOF) ||
        (result.eflags_write_mask & RUNTIME_CPU_RESULT_EFLAGS_ZF) == 0u ||
        (result.eflags_values & RUNTIME_CPU_RESULT_EFLAGS_ZF) != 0u ||
        (result.eflags_values & RUNTIME_CPU_RESULT_EFLAGS_CF) == 0u) return 9;
    if (!invoke(&state, &direct, &event, &cpu, &result,
    ...
    if (!invoke(&state, &direct, &event, &cpu, &result,
        ...
        (result.eflags_values & RUNTIME_CPU_RESULT_EFLAGS_CF) == 0u) return 11;
    ...
    if (!invoke(&state, &direct, &event, &cpu, &result,
            RUNTIME_DEMHNDL_CHG_FILE_PTR) ||
        result.cpu_delta.gpr16_values[0] != ERROR_INVALID_HANDLE ||
        (result.eflags_values & RUNTIME_CPU_RESULT_EFLAGS_CF) == 0u) return 12;

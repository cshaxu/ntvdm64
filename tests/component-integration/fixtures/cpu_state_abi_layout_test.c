...
int main(void)
{
    ...
    if (sizeof(state) != 72u || !runtime_cpu_state_valid(&state) ||
    ...
    if (runtime_cpu_state_valid(&state)) { return 2; }
    state.execution_mode = RUNTIME_CPU_EXECUTION_PROTECTED;
    state.reserved0 = 1u;
    if (runtime_cpu_state_valid(&state)) { return 3; }
    runtime_cpu_state_initialize(0, RUNTIME_CPU_EXECUTION_REAL);
    return runtime_cpu_state_valid(0) ? 4 : 0;
}

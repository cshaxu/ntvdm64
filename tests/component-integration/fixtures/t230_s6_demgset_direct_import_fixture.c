...
static int invoke(fixture_context *s, runtime_dem_direct_context *d,
    runtime_exception_event *e, runtime_cpu_state *cpu,
    runtime_cpu_result *r, uint32_t service, int search)
{ runtime_demhndl_call c; (void)search; memset(&c, 0, sizeof(c)); c.magic=RUNTIME_DEMHNDL_CALL_MAGIC; c.abi_version=RUNTIME_DEMHNDL_CALL_VERSION; c.struct_bytes=sizeof(c); c.service=service; c.direct=d; c.boundary=e; c.cpu=cpu; c.result=r; c.guest_state=s; c.guest_read=read_guest; c.guest_write=write_guest; return runtime_demdisp_invoke(&c) && r->disposition==RUNTIME_CPU_RESULT_RESUME; }
static void reset_cpu(runtime_cpu_state *c)
{ runtime_cpu_state_initialize(c, RUNTIME_CPU_EXECUTION_REAL); c->ds=0x100u; c->es=0x100u; }
...
int main(void)
{
    ...
    runtime_cpu_result result; ULONG far_dta = 0x02000000u;
    ...
    if (!invoke(&state,&direct,&event,&cpu,&result,0x25u,0) || carry(&result) || state.guest[0x1900u+3u]==0u) {
        fprintf(stderr, "dpb: invoke=%u cf=%u ax=%u sector-lo=%u last=%lu\n",
            result.disposition == RUNTIME_CPU_RESULT_RESUME, carry(&result),

...
static int invoke(t236_fixture_context *context,
    ...
    runtime_cpu_result *result, runtime_command_misc_session *session,
    uint32_t service)
{
    ...
    uint32_t service)
{
    ...
    if (!invoke(context, event, cpu, result, session, service) ||
        result->disposition != RUNTIME_CPU_RESULT_PENDING) return 0;
    for (attempt = 0u; attempt < 100u; ++attempt) {
        Sleep(10u);
        if (!invoke(context, event, cpu, result, session, service)) return 0;
        if (result->disposition != RUNTIME_CPU_RESULT_PENDING) return 1;
    }
    return 0;
}
...
int main(void)
{
    ...
    if (result.disposition != RUNTIME_CPU_RESULT_RESUME) return 12;
    ...
    if (!invoke(&context, &event, &cpu, &result, &session,
            RUNTIME_COMMAND_MISC_EXEC) ||
        result.disposition != RUNTIME_CPU_RESULT_PENDING) return 57;

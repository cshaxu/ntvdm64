...
int main(void)
{
    ...
    if (directory_bytes == 0u || directory_bytes >= sizeof(directory) ||
        sprintf_s(pif_path, sizeof(pif_path), "%sruntime-command-fixture.pif", directory) < 0 ||
        sprintf_s(target_path, sizeof(target_path), "%sruntime-target.com", directory) < 0 ||
        sprintf_s(config_path, sizeof(config_path), "%sruntime-config.nt", directory) < 0 ||
        sprintf_s(autoexec_path, sizeof(autoexec_path), "%sruntime-autoexec.nt", directory) < 0) return 1;

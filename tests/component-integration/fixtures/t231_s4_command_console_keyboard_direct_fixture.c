...
static void event_initialize(runtime_exception_event *e)
...
static int invoke(fixture_context *c,runtime_exception_event *e,runtime_cpu_state *cpu,runtime_cpu_result *r,runtime_command_misc_session *s,uint32_t service)
{ runtime_command_misc_call call;memset(&call,0,sizeof(call));call.magic=RUNTIME_COMMAND_MISC_CALL_MAGIC;call.abi_version=RUNTIME_COMMAND_MISC_CALL_VERSION;call.struct_bytes=sizeof(call);call.service=service;call.boundary=e;call.cpu=cpu;call.result=r;call.guest_state=c;call.guest_read=guest_read;call.guest_write=guest_write;call.session=s;return runtime_command_misc_invoke(&call)&&r->disposition==RUNTIME_CPU_RESULT_RESUME&&r->resume_rip==e->fault_rip+4u; }
int main(void)
{
    ...
    if(RegCreateKeyExA(HKEY_CURRENT_USER,"Software\\runner-t231-kbd",0,NULL,0,KEY_ALL_ACCESS,NULL,&root,&disposition)!=ERROR_SUCCESS)return 13;
    ...
    RegOverridePredefKey(HKEY_LOCAL_MACHINE,NULL);RegCloseKey(root);RegDeleteTreeA(HKEY_CURRENT_USER,"Software\\runner-t231-kbd");runtime_command_misc_set_test_system_directory(NULL);CloseHandle(kb16);CloseHandle(keyboard);DeleteFileA(kb16_path);DeleteFileA(keyboard_path);
    runtime_host_handle_manager_reset(s.handles);

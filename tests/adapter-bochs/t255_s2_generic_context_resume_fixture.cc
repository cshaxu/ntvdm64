  ...
  uint64_t resume_rip, uint32_t eax)
{
  ...
  fixture.outcome.abi_version = RUNTIME_GENERIC_UD_CONTEXT_OUTCOME_VERSION;
...
{
  struct runtime_a20_capability_request request;
...
static int test_real_to_protected(void)
{
  ...
  if (!begin_stage(code, sizeof(code)) ||
      !runtime_machine_checked_ram_write(0x900u, target, sizeof(target)) ||
      !runtime_machine_checked_ram_write(0xa00u, source, sizeof(source)) ||
      !runtime_machine_checked_ram_write(0x9fffeu, edge_before,
        sizeof(edge_before)) ||
  ...
  if (runtime_machine_execute_protected_range_action(&action) !=
      RUNTIME_PROTECTED_RANGE_ACTION_REJECTED_MODE) return 0;
  v1_calls = 0u;
  ...
  action.segment = 3u; action.offset = 0xa00u; action.byte_count = sizeof(source);
  if (runtime_machine_execute_protected_range_action(&action) !=
      RUNTIME_PROTECTED_RANGE_ACTION_OK || memcmp(action.bytes, source, sizeof(source))) return 0;
  runtime_protected_range_action_clear(&action);
  ...
  if (runtime_machine_execute_protected_range_action(&action) !=
      RUNTIME_PROTECTED_RANGE_ACTION_OK ||
      !runtime_machine_checked_ram_read(0xa10u, observed, sizeof(observed)) ||
      memcmp(observed, replacement, sizeof(observed))) return 0;
  runtime_protected_range_action_clear(&action);
  action.kind = RUNTIME_PROTECTED_RANGE_ACTION_WRITE;
  action.segment = 1u; action.offset = 0xa10u; action.byte_count = 1u;
  if (runtime_machine_execute_protected_range_action(&action) !=
      RUNTIME_PROTECTED_RANGE_ACTION_REJECTED_ACCESS) return 0;
  runtime_protected_range_action_clear(&action);
  action.kind = RUNTIME_PROTECTED_RANGE_ACTION_READ;
  action.segment = 6u; action.offset = 0xa00u; action.byte_count = 1u;
  if (runtime_machine_execute_protected_range_action(&action) !=
      RUNTIME_PROTECTED_RANGE_ACTION_REJECTED_INPUT) return 0;
  runtime_protected_range_action_clear(&action);
  action.kind = RUNTIME_PROTECTED_RANGE_ACTION_READ;
  action.segment = 3u; action.offset = 0x100000u; action.byte_count = 1u;
  if (!set_a20(0u) || runtime_machine_execute_protected_range_action(&action) !=
      RUNTIME_PROTECTED_RANGE_ACTION_REJECTED_MEMORY || !set_a20(1u))
    return 0;
  runtime_protected_range_action_clear(&action);
  action.kind = RUNTIME_PROTECTED_RANGE_ACTION_READ;
  action.segment = 3u; action.offset = 0xa0000u; action.byte_count = 1u;
  if (runtime_machine_execute_protected_range_action(&action) !=
      RUNTIME_PROTECTED_RANGE_ACTION_REJECTED_MEMORY) return 0;
  runtime_protected_range_action_clear(&action);
  action.kind = RUNTIME_PROTECTED_RANGE_ACTION_WRITE;
  ...
  if (runtime_machine_execute_protected_range_action(&action) !=
      ...
      !runtime_machine_checked_ram_read(0x9fffeu, edge_after,
...
static int test_protected_to_real(void)
{
  ...
  if (!begin_stage(code, sizeof(code)) ||
      !runtime_machine_checked_ram_write(0x900u, target, sizeof(target)) ||
      !runtime_machine_checked_ram_write(0xa00u, target, sizeof(target)) ||
...
static int test_invalid_context_rejected(void)
{
  ...
  return runtime_machine_execute_protected_range_action(&action) ==

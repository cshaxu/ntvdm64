...
#include "adapter-mvdm-host-out/softpc/software_interrupt_observation.h"
...
static int exercise_real_mode(void)
{
  ...
  if (!runtime_machine_software_interrupt_observation_configure(0u)) return 1;
  status = runtime_run_finite_bare_bytes(&request);
  if (status != RUNTIME_FINITE_RUN_COMPLETED_UD_STOP ||
      runtime_machine_software_interrupt_observation_count() != 0u) return 2;

  if (!runtime_machine_software_interrupt_observation_configure(8u)) return 3;
  status = runtime_run_finite_bare_bytes(&request);
  if (status != RUNTIME_FINITE_RUN_COMPLETED_UD_STOP ||
      runtime_machine_software_interrupt_observation_count() != 2u) return 4;
  memset(&record, 0, sizeof(record));
  if (!runtime_machine_software_interrupt_observation_get(0u, &record) ||
      record.version != RUNTIME_SOFTWARE_INTERRUPT_OBSERVATION_VERSION ||
      record.vector != 0x20u || record.execution_mode != 1u ||
      record.cs != 0u || record.rip != 0x102u) return 5;
  memset(&record, 0, sizeof(record));
  if (!runtime_machine_software_interrupt_observation_get(1u, &record) ||
...
static int exercise_protected_mode_negative(void)
{
  ...
  if (!runtime_machine_software_interrupt_observation_configure(8u)) return 1;
  status = runtime_run_finite_bare_bytes(&request);
  if (status != RUNTIME_FINITE_RUN_COMPLETED_UD_STOP) return 2;
  return runtime_machine_software_interrupt_observation_count() == 0u ? 0 : 3;
}
...
static int exercise_ring(void)
{
  ...
  if (!runtime_machine_software_interrupt_observation_configure(3u)) return 1;
  memset(&record, 0, sizeof(record));
  record.version = RUNTIME_SOFTWARE_INTERRUPT_OBSERVATION_VERSION;
  for (i = 0u; i != 5u; ++i) {
    record.sequence = i;
    record.vector = (uint8_t)(0x40u + i);
    runtime_machine_software_interrupt_observation_record(&record);
  }
  if (runtime_machine_software_interrupt_observation_count() != 3u) return 2;
  for (i = 0u; i != 3u; ++i) {
    memset(&record, 0, sizeof(record));
    if (!runtime_machine_software_interrupt_observation_get(i, &record) ||
        record.sequence != i + 2u || record.vector != 0x42u + i) return 3;
  }
  runtime_machine_software_interrupt_observation_reset();
  return runtime_machine_software_interrupt_observation_count() == 0u ? 0 : 4;
}

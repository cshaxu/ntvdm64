...
int main(void)
{
  ...
  runtime_first_fault_observation fault;
  ...
  if (status != RUNTIME_FINITE_RUN_COMPLETED_FIRST_FAULT_STOP ||
      !runtime_machine_first_fault_observation_observed()) return 1;
...

  {
    ...
    static runtime_mechanical_action target_write;

    runtime_mechanical_action_clear(&target_write);
    target_write.kind = RUNTIME_MECHANICAL_ACTION_WRITE;
    ...
    status = runtime_run_finite_bare_bytes(&request);
    if (status != RUNTIME_FINITE_RUN_COMPLETED_FIRST_FAULT_STOP ||
        !runtime_machine_first_fault_observation_observed()) return 5;
    memset(&fault, 0, sizeof(fault));
    if (!runtime_machine_first_fault_observation_copy(&fault) ||
...

  {
    ...
    if (machine.realmode_segment_limit_compatibility_active() ||
        machine.cleanup() != BX_MACHINE_MINIMAL_MACHINE_OK) return 6;
  }

...
static int action_append(struct runtime_mechanical_action *action,
  Bit64u physical, const void *bytes, Bit32u byte_count)
{
  Bit32u index = action->range_count;
  if (index >= RUNTIME_MECHANICAL_ACTION_MAX_RANGES || byte_count == 0u ||
      action->payload_bytes > RUNTIME_MECHANICAL_ACTION_MAX_BYTES - byte_count)
    return 0;
...
static int begin_native_cmdexec_stage(void)
{
  ...
  /* Test-only direct core state setup.  The product's public machine contract
...
int main()
{
  ...
  for (attempt = 0u; attempt < 100u; ++attempt) {
    ...
    if (status != RUNTIME_MACHINE_STAGE_EXECUTION_PENDING) break;
  }
  ...
  if (status != RUNTIME_MACHINE_STAGE_EXECUTION_BUDGET ||

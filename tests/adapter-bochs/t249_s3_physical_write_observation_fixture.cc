  ...
  void *, unsigned)
{
  ...
  record = (const struct runtime_physical_write_observation *)event;
  memcpy(&tag, (const uint8_t *)event + sizeof(*record), sizeof(tag));
  if (record->magic != RUNTIME_PHYSICAL_WRITE_OBSERVATION_MAGIC ||
...
int main()
{
  ...
  if (!machine_facade_bind_opaque_callback(observe_physical_write, 0)) return 10;
  if (runtime_physical_write_observation_configure(0x200u, 0u) ||
      runtime_physical_write_observation_copy(&value)) return 1;
  ...
  if (runtime_physical_write_observation_copy(&value) ||
      !bx_mem.copy_from_ordinary_ram(0x200u, 1u, actual) || actual[0] != first)
    return 3;
  if (!runtime_physical_write_observation_configure(0x300u, 2u)) return 4;
  bx_mem.writePhysicalPage(&bx_cpu, 0x200u, 1u, &first);
  if (runtime_physical_write_observation_copy(&value)) return 5;
  bx_mem.writePhysicalPage(&bx_cpu, 0x301u, 2u, second);
  if (!runtime_physical_write_observation_copy(&value) ||
      value.magic != RUNTIME_PHYSICAL_WRITE_OBSERVATION_MAGIC ||
      value.abi_version != RUNTIME_PHYSICAL_WRITE_OBSERVATION_VERSION ||
  ...
  runtime_physical_write_observation_reset();
  if (runtime_physical_write_observation_copy(&value)) return 8;
  machine_facade_unbind_opaque_callback();
  return machine.cleanup() == BX_MACHINE_MINIMAL_MACHINE_OK ? 0 : 9;
}
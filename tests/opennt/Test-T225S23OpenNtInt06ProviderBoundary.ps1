...
$headerPath = Join-Path $repositoryRoot 'src/opennt-bop/softpc/opennt_int06_provider.h'
$sourcePath = Join-Path $repositoryRoot 'src/opennt-bop/softpc/opennt_int06_provider.c'
...
foreach ($required in @(
    ...
    'OPENNT_INT06_PROVIDER_OPERATION_RAM_READ8',
    'OPENNT_INT06_PROVIDER_OPERATION_PORT_WRITE8',
    'opennt_int06_provider_begin',
    'opennt_int06_provider_begin_unexpected',
    's->phase = P_MASTER_OCW3',
    'P_IP_LO', 'P_WRITE_CS_LO', 'P_MASTER_OCW3', 'P_SLAVE_EOI', 'P_BDA',
    's->saved_ip == 0xffffu',
    's->master_isr == 0u ? 0xffu : s->master_isr',
    'OPENNT_INT06_PROVIDER_STATUS_FAILED'
)) {

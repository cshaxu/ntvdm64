...
static int step(struct opennt_int06_provider_v1_state *s, uint32_t kind, uint64_t address, uint8_t value, uint8_t read_value) {
  struct opennt_int06_provider_v1_operation o;
  if (!opennt_int06_provider_v1_next(s, &o) || o.kind != kind || o.address != address ||
      ((kind != OPENNT_INT06_PROVIDER_V1_OPERATION_RAM_READ8 && kind != OPENNT_INT06_PROVIDER_V1_OPERATION_PORT_READ8) && o.value != value)) return 0;
  return opennt_int06_provider_v1_accept(s, &o, read_value);
}
static int software_case(void) {
  struct opennt_int06_provider_v1_state s;
  opennt_int06_provider_v1_initialize(&s);
  if (!opennt_int06_provider_v1_begin(&s, 0x70u, 0x479u)) return 1;
  ...
  if (!step(&s,2,0x46bu,0xffu,0)||!opennt_int06_provider_v1_complete(&s)) return 4;
  return 0;
}
static int hardware_carry_slave_case(void) {
  struct opennt_int06_provider_v1_state s;
  opennt_int06_provider_v1_initialize(&s);
  if (!opennt_int06_provider_v1_begin(&s, 0x100u, 0xfffeu)) return 1;

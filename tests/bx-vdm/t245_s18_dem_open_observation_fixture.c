...
int main(void)
{
    ...
    struct bx_ntvdm_dem_open_observation_v1 value;
    ...
    bx_ntvdm_dem_open_observation_v1_consider(&event, &outcome, read_guest, ram);
    memcpy(ram->bytes + address, "O:\\X\\COMMAND.COM", 17u);
    bx_ntvdm_dem_open_observation_v1_consider(&event, &outcome, read_guest, ram);
    if (!bx_ntvdm_dem_open_observation_v1_copy(&value) || !value.observed ||

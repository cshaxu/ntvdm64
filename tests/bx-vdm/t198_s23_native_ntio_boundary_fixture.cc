...
int main()
{
  ...
  static bx_ntvdm_finite_run_request r; bx_ntvdm_finite_run_terminal_snapshot terminal; bx_ntvdm_generic_ud_event_v1 generic; unsigned terminal_valid, generic_valid; int status;
  ...
  if(!bx_ntvdm_finite_run_terminal_snapshot_configure_ordinary_range(0u,0u)||bx_ntvdm_finite_run_terminal_snapshot_configure_ordinary_range(0x100000u,1u)||!bx_ntvdm_finite_run_terminal_snapshot_configure_ordinary_range(0x0a80u,16u))return 1;
  memset(&terminal,0,sizeof(terminal)); memset(&generic,0,sizeof(generic)); r.capture_terminal_snapshot=1; status=(int)bx_ntvdm_run_finite_bare_bytes(&r); terminal_valid=bx_ntvdm_finite_run_terminal_snapshot_get(&terminal); generic_valid=t198_s23_native_ntio_boundary_copy_first_generic_event(&generic); bx_ntvdm_boot_namespace_composition_v1_unbind(&c);
  fprintf(stderr,"t198-s23 status=%d observed-5011=%u observed-503b-resume=%u observed-dta-resume=%u observed-hard-error-resume=%u observed-system-symbol-resume=%u observed-drive-resume=%u observed-first-generic-ud=%u fault=%04x:%04x mode=%u vector=%u window=%u:%02x%02x%02x%02x state=%u:%04x:%04x ss-sp=%04x:%04x ds-si=%04x:%04x es-di=%04x:%04x eax=%08x ebx=%08x ecx=%08x edx=%08x ebp=%08x flags=%08x observed-stop=%u next=%02x:%02x plane=%u/%u/%u error-lock=%u gset=%u terminal=%u:%04x:%04x snapshot=%u:%05x:%u:%02x%02x%02x%02x\n",status,
    ...
    terminal_valid,terminal.cs,terminal.eip,terminal.valid,(unsigned)terminal.captured_physical_address,terminal.captured_byte_count,terminal.captured_bytes[4],terminal.captured_bytes[5],terminal.captured_bytes[6],terminal.captured_bytes[7]);
  return t198_s23_native_ntio_boundary_observed_first_generic_ud() && generic_valid &&
    ...
    terminal.captured_physical_address==0x0a80u && terminal.captured_byte_count==16u &&
    terminal.captured_bytes[4]==0xffu && terminal.captured_bytes[5]==0xffu &&
    terminal.captured_bytes[6]==0xffu && terminal.captured_bytes[7]==0xffu &&
    memcmp(terminal.captured_bytes,t198_s23_ntio_bytes+(0x0a80u-0x700u),16u)!=0?0:2;
}

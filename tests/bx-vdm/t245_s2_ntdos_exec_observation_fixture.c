...
int main(void)
{
    ...
    put_u16(prefix + BX_NTVDM_NTDOS_EXEC_PDB_PARENT_OFFSET, UINT16_C(0x1234));
    put_u16(prefix + BX_NTVDM_NTDOS_EXEC_PDB_ENVIRONMENT_OFFSET, UINT16_C(0x4321));
    if (!bx_ntvdm_ntdos_exec_observation_v1_pdb_range(UINT16_C(0x1234),
            &physical, &bytes) || physical != UINT32_C(0x12340) ||
        bytes != BX_NTVDM_NTDOS_EXEC_PDB_PREFIX_BYTES) return 1;
    if (bx_ntvdm_ntdos_exec_observation_v1_pdb_range(UINT16_C(0xffff),
            &physical, &bytes)) return 2;
    if (bx_ntvdm_ntdos_exec_observation_v1_decode(UINT16_C(0x1234), prefix,
            BX_NTVDM_NTDOS_EXEC_PDB_PREFIX_BYTES - 1u, &observation) ||
        bx_ntvdm_ntdos_exec_observation_v1_decode(UINT16_C(0xffff), prefix,

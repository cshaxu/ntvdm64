...
int main(void)
{
 ...
 if(!GetModuleFileNameA(NULL,path,MAX_PATH)||strlen(path)>=120u)return 1;
 ...
 if(!invoke(&c,&e,&cpu,&r,&s)||r.disposition!=BX_NTVDM_CPU_RESULT_V2_RESUME||r.eflags_values!=0u||r.cpu_delta.segment_write_mask!=((1u<<0)|(1u<<3))||r.cpu_delta.segment_values[3]!=(USHORT)(scs>>4)||r.cpu_delta.segment_values[0]!=(USHORT)((scs+offsetof(SCSINFO,SCS_ParamBlock))>>4))return 2;
 if(strcmp(s.scs_info.SCS_ComSpec,"C:\\NTDOS64\\COMMAND.COM")||s.scs_info.SCS_CmdTail[0]<(UCHAR)(3u+strlen(path))||memcmp(s.scs_info.SCS_CmdTail+1,"/z ",3u)||memcmp(c.guest+scs,&s.scs_info,sizeof(s.scs_info)))return 3;
 DontCheckDosBinaryType=TRUE;if(!invoke(&c,&e,&cpu,&r,&s)||r.eflags_values!=0u){DontCheckDosBinaryType=FALSE;return 4;}DontCheckDosBinaryType=FALSE;

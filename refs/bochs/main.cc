...
#if defined(BX_NTVDM_ENABLE_EXECUTION_PLAN) && BX_NTVDM_ENABLE_EXECUTION_PLAN
/* The plan is opaque to the simulator: its validation and policy live at the
 * caller boundary.  This is the sole post-machine-init, pre-CPU-loop consumer.
 * It applies an already checked RAM transaction atomically before registers. */
static int bx_apply_requested_execution_plan(void)
{
  ...
  else if (!BX_MEM(0)->ordinary_ram_readable(
        (bx_phy_address)plan.preserved_state_address,
        (Bit64u)plan.preserved_state_bytes)) failure_stage = 2;
  else if (!BX_MEM(0)->copy_from_ordinary_ram(
        (bx_phy_address)plan.preserved_state_address,
        (Bit64u)plan.preserved_state_bytes, preserved)) failure_stage = 3;
  else if (!BX_MEM(0)->copy_to_ordinary_ram(
        (bx_phy_address)plan.payload_write.guest_physical_address,
        (Bit64u)payload_bytes, (const Bit8u *)payload)) failure_stage = 4;
  else if (!BX_MEM(0)->copy_to_ordinary_ram(
        (bx_phy_address)plan.preserved_state_address,
        (Bit64u)plan.preserved_state_bytes, preserved)) failure_stage = 5;
  if (failure_stage != 0) {
    BX_ERROR(("requested execution plan rejected at generic stage %u", failure_stage));
    return -1;
  }

  BX_CPU(0)->apply_real_mode_entry(plan.entry_cpu.cs,
    (Bit32u)plan.entry_cpu.eip);
  BX_INFO(("requested execution plan applied"));
  return 1;
}
#endif

#if defined(BX_NTVDM_ENABLE_DEFERRED_STARTUP_PLAN) && BX_NTVDM_ENABLE_DEFERRED_STARTUP_PLAN
static int bx_prepare_deferred_startup_plan(void)
{
  return bx_ntvdm_adapter_runtime_v1_prepare_deferred_startup_from_environment();
}
#endif
...
int bx_begin_simulation (int argc, char *argv[])
{
  ...
  bx_init_hardware();
#if defined(BX_NTVDM_ENABLE_DEFERRED_STARTUP_PLAN) && BX_NTVDM_ENABLE_DEFERRED_STARTUP_PLAN
  if (bx_prepare_deferred_startup_plan() < 0) {
    BX_ERROR(("deferred startup plan rejected"));
    return -1;
  }
#endif

  #if defined(BX_NTVDM_ENABLE_EXECUTION_PLAN) && BX_NTVDM_ENABLE_EXECUTION_PLAN
  if (bx_apply_requested_execution_plan() < 0) {
    BX_ERROR(("requested execution plan rejected before CPU loop"));
    return 0;
  }
  #endif

...
| --- | --- | --- | --- | --- |
| `ADAPTER-BOP-001` | NT4 connected BOP dispatch directly to its product host. | The modern component graph must not make Bochs or its generic CPU entry selector-aware. | A same-runtime, `app`-owned route binding forwards only the copied event and typed outcome; the route remains in `opennt-bop`. | `bx_ntvdm_bop_ingress_v1.c`, `bx_ntvdm_bop_ingress_v1.h` |

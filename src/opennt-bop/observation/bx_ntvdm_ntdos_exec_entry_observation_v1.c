#include "bx_ntvdm_ntdos_exec_entry_observation_v1.h"

#include <string.h>

static uint32_t enabled;
static struct runtime_ntdos_exec_entry_observation_v1 record;

static void clear(void)
{
    memset(&record, 0, sizeof(record));
    record.magic = RUNTIME_NTDOS_EXEC_ENTRY_OBSERVATION_V1_MAGIC;
    record.abi_version = RUNTIME_NTDOS_EXEC_ENTRY_OBSERVATION_V1_VERSION;
    record.struct_bytes = sizeof(record);
    record.enabled = enabled;
}

void runtime_ntdos_exec_entry_observation_v1_enable(uint32_t value)
{
    enabled = value == 1u;
    clear();
}
    ...
    runtime_ntdos_exec_entry_read_v1 read, void *read_state)
{
    ...
    if (!runtime_ntdos_exec_observation_v1_pdb_range(pdb_segment,
            &physical, &bytes) ||
        !read(read_state, physical, prefix, bytes) ||
        !runtime_ntdos_exec_observation_v1_decode(pdb_segment, prefix, bytes,
            &record.pdb)) {

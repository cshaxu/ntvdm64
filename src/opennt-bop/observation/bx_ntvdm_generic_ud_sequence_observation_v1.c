...
static struct runtime_generic_ud_sequence_observation_v1 record;

static void clear(void)
{
    memset(&record, 0, sizeof(record));
    record.magic = RUNTIME_GENERIC_UD_SEQUENCE_OBSERVATION_V1_MAGIC;
    record.abi_version = RUNTIME_GENERIC_UD_SEQUENCE_OBSERVATION_V1_VERSION;
    record.struct_bytes = sizeof(record);
}

void runtime_generic_ud_sequence_observation_v1_enable(uint32_t value)
{
    enabled = value == 1u;
    clear();
}
...
{
    struct runtime_generic_ud_sequence_observation_record_v1 *slot;
    if (!enabled || event == 0 || outcome == 0 ||
        event->window_bytes > RUNTIME_GENERIC_UD_WINDOW_BYTES) return;
    if (record.record_count >=
        RUNTIME_GENERIC_UD_SEQUENCE_OBSERVATION_V1_MAX_RECORDS) {

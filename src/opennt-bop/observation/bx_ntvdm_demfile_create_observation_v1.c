...
static struct runtime_demfile_create_observation_v1 record;

static void clear(void)
{
    memset(&record, 0, sizeof(record));
    record.magic = RUNTIME_DEMFILE_CREATE_OBSERVATION_V1_MAGIC;
    record.abi_version = RUNTIME_DEMFILE_CREATE_OBSERVATION_V1_VERSION;
    record.struct_bytes = sizeof(record);
    record.enabled = enabled;
}

void runtime_demfile_create_observation_v1_enable(uint32_t value)
{
    enabled = value == 1u;
    clear();
}
    ...
    DWORD first_error, DWORD retry_error)
{
    ...
    if (path == NULL) return;
    for (index = 0u; index + 1u < RUNTIME_DEMFILE_CREATE_OBSERVATION_V1_PATH_CHARS &&

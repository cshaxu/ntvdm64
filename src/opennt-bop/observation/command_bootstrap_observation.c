#include "command_bootstrap_observation.h"
...
static struct runtime_command_bootstrap_observation record;

static void clear(void)
{
    memset(&record, 0, sizeof(record));
    record.magic = RUNTIME_COMMAND_BOOTSTRAP_OBSERVATION_MAGIC;
    record.abi_version = RUNTIME_COMMAND_BOOTSTRAP_OBSERVATION_VERSION;
    record.struct_bytes = sizeof(record);
    record.enabled = enabled;
}

void runtime_command_bootstrap_observation_enable(uint32_t value)
{
    enabled = value == 1u;
    clear();
}
    ...
    runtime_command_bootstrap_read read, void *read_state)
{
    ...
    if (physical > UINT32_MAX - RUNTIME_COMMAND_BOOTSTRAP_OBSERVATION_BYTES ||
        !read(read_state, physical, record.command,
            RUNTIME_COMMAND_BOOTSTRAP_OBSERVATION_BYTES)) {

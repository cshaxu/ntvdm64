...
| `BOP-DIV-032` | `nt_pif.c` and COMMAND's `cmdpif.h` expose different historical `PIF_DATA` definitions. | Direct linking would cross incompatible layouts. | Keep the exact parser under its sole current COMMAND/BOP composition owner and translate only its compatible result in the component-local translation unit. | `softpc.new/host/src/nt_pif.c`, `command/opennt_pif_composition.c:33`, `command/opennt_pif_composition.h` |

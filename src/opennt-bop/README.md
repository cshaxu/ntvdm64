...
| `BOP-DIV-039` | `demfile.c`/`demhndl.c` receive `VrInitialized` as a VDMREDIR-DLL function-pointer import, alongside the historical DEM/SoftPC/OEM header closure. | The standalone static link cannot define that import once in every original translation unit, nor link the historical DLL loader. | Retain the original pointer call shape; one composition definition binds it after `LoadVdmRedir` to re-rooted `vrinit.c`, while named OEM/CCPU/SAS seams remain unchanged. | `dem/demfile.c:25`, `dem/demhndl.c:21`, `dem/opennt_demfile_composition.c` |
...
| `BOP-DIV-046` | `demdasd.c` / `demioctl.c` use NT4 raw-disk and recursive SoftPC/FDC composition. | The modern runner has neither the private NT disk path nor recursive SoftPC execution. | Retain the exact `nt_fdisk_*` spelling and source failure branches; public geometry/I/O is bounded by host admission, while FDC paths fail explicitly. | `dem/opennt_demdasd_ioctl_composition.c:104`, `dem/opennt_demdasd_ioctl_composition.c:151` |

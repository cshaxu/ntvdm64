...
{ runtime_demhndl_terminate(); }
LPVOID runtime_demmisc_get_vdm_pointer(ULONG far_pointer, ULONG bytes,
    UCHAR protected_mode)
{ (void)bytes; (void)protected_mode; return runtime_demhndl_get_vdm_addr(
    (USHORT)(far_pointer >> 16), (USHORT)far_pointer); }
HANDLE runtime_demmisc_create_file_oem(LPCSTR path, DWORD access, DWORD share,
    ...
    LPDWORD read_out, LPOVERLAPPED overlapped)
{
    ...
    if (success && read_out != NULL && *read_out != 0u &&
        !runtime_demhndl_loader_write(buffer, *read_out)) {
...
void runtime_demmisc_module_load(LPCSTR module,LPCSTR path,WORD segment,DWORD bytes)
{
...
void runtime_demmisc_module_segment_move(LPCSTR module,LPCSTR path,WORD old_segment,WORD new_segment)
...
{
    runtime_opennt_rc_error_dialog((UINT)error, (CHAR *)text,
        (CHAR *)reserved);
}
...
void runtime_demmisc_host_fdisk_reset(void) { HostFdiskReset(); }

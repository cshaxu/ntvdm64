...
/* OpenNT source: src/opennt/base/mvdm/dos/dem/demhndl.c.  The historical
...
USHORT  usDX,usCX;
    ...
    /* OpenNT source: src/opennt/base/mvdm/dos/dem/demhndl.c used
...
LONG    lLoc;
    ...
    if (!getZF()) {
        /* OpenNT demhndl.c used ULONG here.  Modern Win32 declares the
         * SetFilePointer high-part parameter as PLONG; retain zero semantics
         * with its exact signed ABI type to make the direct import warning-free. */
        LONG    Zero = 0;
...
{
/* OpenNT's control flow initializes hFile whenever option is not the
 * device-time case. Initialize it for modern /W4 flow analysis without
 * changing any reachable original path. */
HANDLE  hFile = NULL;
WORD    wDate,wTime;
...
UCHAR	uchOpt = 0;
    ...
    /* OpenNT demhndl.c assigned an untyped 16-bit register value to UCHAR.
     * The shim returns USHORT; the original BL contract is retained explicitly. */
    uchOpt = (UCHAR)getBL();
    if(uchOpt != 2)
	hFile = GETHANDLE(getAX(),getBP());

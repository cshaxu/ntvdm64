    ...
    USHORT SearchAttr)
{
    ...
    /* OpenNT source stored its 32-bit host pointer here.  The x64 Direct
     * shim preserves the guest-width validation reference instead. */
    ...
    try {
         ...
         if (pwcFile) {
             ...
             if (pwc) {
                 ...
                 /* OpenNT was a 32-bit host process.  This is host-private
                  * Unicode-buffer arithmetic, so retain the byte-distance
                  * algorithm with uintptr_t rather than truncate x64 host
                  * pointers; no guest pointer crosses this seam. */
                 PathName->Length = (USHORT)((uintptr_t)pwc - (uintptr_t)PathName->Buffer);
   ...
   //
    ...
    while (TRUE) {
       ...
       if ( DirectoryInfo->NextEntryOffset ) {
           pFFindEntry->FindBufferNext = (PVOID)((uintptr_t)DirectoryInfo +
    ...
    )
{
    ...
    if (pFFindEntry->FindBufferNext) {
        ...
        BytesOffset = (ULONG)((uintptr_t)pFFindEntry->FindBufferNext -
                      (uintptr_t)pFFindEntry->FindBufferBase);
        ...
        while (DirInfo->NextEntryOffset) {
            DirInfo = (PVOID)((uintptr_t)DirInfo + DirInfo->NextEntryOffset);
            }
        LastDirInfo = DirInfo;
        ...
        BytesLeft = pFFindEntry->FindBufferLength -
                     ((uintptr_t)DirInfo - (uintptr_t)pFFindEntry->FindBufferBase);
        }
    else {
        DirInfo = pFFindEntry->FindBufferBase;
        LastDirInfo = NULL;
    ...
    while (BytesLeft > sizeof(FILE_BOTH_DIR_INFORMATION) + sizeof(LONGLONG)) {
       ...
       if (LastDirInfo) {
           LastDirInfo->NextEntryOffset =(ULONG)((uintptr_t)DirInfo - (uintptr_t)LastDirInfo);
           }
       else {
           pFFindEntry->FindBufferNext = pFFindEntry->FindBufferBase;
           }
...

{
    ...
    if (DeviceNameData) {
        ...
        DeviceName = (PWSTR)((uintptr_t)FileName + (DeviceNameData >> 16));

...
/* DIVERGENCE(BOP-DIV-102): original cmdExec32 directly owns a CCPU worker,
 * BaseSrv re-entry and process-wide standard-handle swap.  The required
 * bounded one-session replacement exceeds a mirror-local edit, so the
 * same-signature body is private overlay code.  The original cmdExec and
 * cmdExecComspec32 call shape below remains unchanged. */
#include "opennt-bop-overlay/command/cmdexec32.c"

...
char    CmdLine[MAX_PATH];
	...
	// return exit code
		...
		if (!pfdata.CloseOnExit){
		    ...
		    /* DIVERGENCE(BOP-DIV-013): ULONG is the historical width; make the
    ...
    *pIsDosBinary = 1;
    ...
    while (*pSrc && *pSrc != '.') {
     ...
     cb = (ULONG)strlen(CmdLine); /* DIVERGENCE(BOP-DIV-015): preserve historical ULONG. */
...
int     upperDrive;
    ...
    if (lpszzEnv) {
	    ...
	    if(*lpszzEnv == '=' &&
		    ...
		    upperDrive <= 'Z' &&
		    /* DIVERGENCE(BOP-DIV-018): the original ULONG casts assumed a 32-bit host

...
#undef RMB_ICON_STOP
#undef RMB_EDIT
#define PIF_DATA RUNTIME_OPENNT_PARSER_PIF_DATA
#define pfdata runtime_command_pif_parser_global
#define GetPIFData runtime_command_pif_parse_original
#define GetPIFConfigFiles runtime_command_pif_original_get_config_files
#define RcMessageBox runtime_command_pif_parser_message_box
#define ch_malloc runtime_command_pif_parser_malloc
#pragma warning(push)
...
RUNTIME_OPENNT_PARSER_PIF_DATA runtime_command_pif_parser_global;
    ...
    CHAR *second, ULONG options)
{
    ...
    return runtime_opennt_rc_message_box(error, first, second, options);
}
...
BOOL GetPIFData(PIF_DATA *data, char *pif_name)
{
    ...
    memcpy(&runtime_command_pif_parser_global, data,
        sizeof(runtime_command_pif_parser_global));
    return runtime_command_pif_parse_original(
        (RUNTIME_OPENNT_PARSER_PIF_DATA *)data, pif_name);
}
    ...
    char *config_path, char *autoexec_path)
{
    ...
    runtime_command_pif_original_get_config_files(TRUE, config_path);
    runtime_command_pif_original_get_config_files(FALSE, autoexec_path);
    free(data.CmdLine);
    free(data.StartDir);
    free(data.StartFile);
    free(data.WinTitle);

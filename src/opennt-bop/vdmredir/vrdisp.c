...
static __declspec(thread) const struct bx_ntvdm_generic_ud_event_v1 *VrEvent;
static __declspec(thread) struct bx_ntvdm_generic_ud_outcome_v1 *VrOutcome;
static __declspec(thread) bx_ntvdm_vr_dispatch_body_fn VrBody;
...
int bx_ntvdm_vr_dispatch_with_frame(uint8_t service,const struct bx_ntvdm_generic_ud_event_v1 *event,struct bx_ntvdm_generic_ud_outcome_v1 *outcome,bx_ntvdm_vr_dispatch_body_fn body) { int handled; if (!event || !outcome || !body || VrBody) return 0; VrEvent=event; VrOutcome=outcome; VrBody=body; handled=VrDispatch(service); VrBody=NULL; VrEvent=NULL; VrOutcome=NULL; return handled; }

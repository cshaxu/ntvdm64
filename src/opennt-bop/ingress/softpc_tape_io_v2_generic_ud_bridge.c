...
int runtime_softpc_tape_io_v2_generic_ud_recognizes(
    const struct runtime_generic_ud_event_v1 *event)
{
    return event != 0 && event->magic == RUNTIME_GENERIC_UD_EVENT_V1_MAGIC &&
        ...
        event->window[1] == 0xc4u && event->window[2] == 0x15u &&
        ((event->eax >> 8) & 0xffu) == INT15_EMS_DETERMINE;
}

int runtime_softpc_tape_io_v2_generic_ud_dispatch(
    const struct runtime_generic_ud_event_v1 *event,
    struct runtime_generic_ud_outcome_v1 *outcome)
{
    uint16_t ax;
    if (!runtime_softpc_tape_io_v2_generic_ud_recognizes(event) ||
        outcome == 0 || event->fault_rip > UINT64_MAX - 3u ||
        !runtime_softpc_tape_io_begin((uint16_t)event->eax, &ax)) return 0;
    cassette_io();
    if (!runtime_softpc_tape_io_end(&ax)) return 0;

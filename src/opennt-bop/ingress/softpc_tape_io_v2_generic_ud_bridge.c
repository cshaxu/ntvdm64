...
int bx_ntvdm_softpc_tape_io_v2_generic_ud_recognizes(
...
int bx_ntvdm_softpc_tape_io_v2_generic_ud_dispatch(
    const struct bx_ntvdm_generic_ud_event_v1 *event,
    struct bx_ntvdm_generic_ud_outcome_v1 *outcome)
{
    ...
    if (!bx_ntvdm_softpc_tape_io_v2_generic_ud_recognizes(event) ||
        ...
        !bx_ntvdm_softpc_tape_io_begin((uint16_t)event->eax, &ax)) return 0;
    cassette_io();
    if (!bx_ntvdm_softpc_tape_io_end(&ax)) return 0;

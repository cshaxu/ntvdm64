...
static int is_real_machine_bop(const struct runtime_generic_ud_event_v1 *event,
    uint8_t selector)
{
...
static int preflight_writes(const struct runtime_generic_ud_event_v1 *event,
    uint8_t selector)
{
    ...
    return runtime_mechanical_action_v1_valid(&action) &&
        runtime_mantle_execute_mechanical_action_v1(&action);
}

static int execute_operation(const struct int06_provider_v1_operation *operation,
    uint32_t action_id, uint8_t *read_value)
{
    struct runtime_mechanical_action_v1 memory;
    struct runtime_port_action_v1 port;
    if (operation == 0 || read_value == 0) return 0;
    if (operation->kind == INT06_PROVIDER_V1_OPERATION_RAM_READ8 ||
        operation->kind == INT06_PROVIDER_V1_OPERATION_RAM_WRITE8) {
        runtime_mechanical_action_v1_clear(&memory);
        memory.action_id = action_id;
        memory.kind = operation->kind == INT06_PROVIDER_V1_OPERATION_RAM_READ8 ?
            RUNTIME_MECHANICAL_ACTION_V1_READ : RUNTIME_MECHANICAL_ACTION_V1_WRITE;
        ...
        if (!runtime_mechanical_action_v1_valid(&memory) ||
            !runtime_mantle_execute_mechanical_action_v1(&memory)) return 0;
        *read_value = memory.payload[0];
        return 1;
    }
    if (operation->kind != INT06_PROVIDER_V1_OPERATION_PORT_READ8 &&
        operation->kind != INT06_PROVIDER_V1_OPERATION_PORT_WRITE8) return 0;
    runtime_port_action_v1_initialize(&port);
    port.kind = operation->kind == INT06_PROVIDER_V1_OPERATION_PORT_READ8 ?
        RUNTIME_PORT_ACTION_V1_READ8 : RUNTIME_PORT_ACTION_V1_WRITE8;
    port.port = (uint16_t)operation->address;
    port.value = operation->value;
    if (!runtime_mantle_execute_port_action_v1(&port)) return 0;
    ...
    runtime_cpu_result_v2 *result)
{
    ...
    int06_provider_v1_initialize(&state);
    if ((selector == 0x02u && !int06_provider_v1_begin_unexpected(&state)) ||
        (selector == 0x06u && !int06_provider_v1_begin(&state,
            event->ss, event->esp)) || !preflight_writes(event, selector))
        return runtime_cpu_result_v2_stop(result);
    while (int06_provider_v1_next(&state, &operation)) {
        if (++count > 24u || !execute_operation(&operation, 0x6200u + count,
                &read_value) || !int06_provider_v1_accept(&state,
                &operation, read_value)) return runtime_cpu_result_v2_stop(result);
    }
    return int06_provider_v1_complete(&state) ?
        runtime_cpu_result_v2_resume(result, event->fault_rip + 3u) :
        runtime_cpu_result_v2_stop(result);
}

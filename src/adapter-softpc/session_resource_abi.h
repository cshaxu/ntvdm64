...
typedef enum runtime_session_resource_kind {
...
typedef struct runtime_session_resource {
    uint32_t kind;
    uint32_t flags;
    uint64_t byte_count;
    uint8_t sha256[32];
} runtime_session_resource;

typedef struct runtime_session_resources {
    uint32_t magic;
    uint32_t abi_version;
    uint32_t struct_bytes;
    uint32_t flags;
    runtime_session_resource resources[3];

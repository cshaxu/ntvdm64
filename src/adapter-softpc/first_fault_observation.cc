...
static struct runtime_first_fault_observation_v1 runtime_first_fault_event;

extern "C" void runtime_mantle_first_fault_observation_reset(void)
{
  runtime_first_fault_was_observed = 0;
  memset(&runtime_first_fault_event, 0, sizeof(runtime_first_fault_event));
}

extern "C" void runtime_mantle_first_fault_observation_enable(int enabled)
{
  if (enabled) runtime_mantle_first_fault_observation_reset();
  runtime_first_fault_fixture_enabled = enabled != 0;
}

extern "C" int runtime_mantle_first_fault_observation_enabled(void)
{
  return runtime_first_fault_fixture_enabled;
}

extern "C" void runtime_mantle_first_fault_observation_fixture_stop(int enabled)
{
  runtime_mantle_first_fault_observation_enable(enabled);
}

extern "C" int runtime_mantle_first_fault_observation_observed(void)
{
  return runtime_first_fault_was_observed;
}

extern "C" int runtime_mantle_first_fault_observation_copy(
...
extern "C" int runtime_mantle_first_fault_observation_v1(
  const struct runtime_first_fault_observation_v1 *event)
{
  if (!runtime_first_fault_fixture_enabled || runtime_first_fault_was_observed ||
      event == 0 || event->magic != RUNTIME_FIRST_FAULT_OBSERVATION_V1_MAGIC ||

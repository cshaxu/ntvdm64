...
#ifndef RUNTIME_PHYSICAL_IRQ_V1_H
#define RUNTIME_PHYSICAL_IRQ_V1_H

#include <stdint.h>

#define RUNTIME_PHYSICAL_IRQ_V1_MAX 15u
...
int runtime_mantle_request_physical_irq_v1(uint32_t irq);
...
int runtime_mantle_post_physical_irq_v1(uint32_t irq);
...
uint32_t runtime_mantle_drain_posted_physical_irqs_v1(void);

/* Clears unconsumed producer state during the active-machine lifecycle. */
void runtime_mantle_clear_posted_physical_irqs_v1(void);

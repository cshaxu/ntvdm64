...
int runtime_cpu_delta_v1_valid(const runtime_cpu_delta_v1 *delta)
{
    return delta != 0 && delta->magic == RUNTIME_CPU_DELTA_ABI_MAGIC &&
        delta->abi_version == RUNTIME_CPU_DELTA_ABI_VERSION &&
        delta->struct_bytes == sizeof(*delta) &&
        (delta->gpr16_write_mask & ~RUNTIME_CPU_DELTA_V1_GPR16_MASK) == 0u &&
        (delta->segment_write_mask & ~((1u << RUNTIME_CPU_DELTA_V1_SEGMENT_COUNT) - 1u)) == 0u &&
        delta->reserved0 == 0u;
}
...
{
    if (!runtime_cpu_delta_v1_valid(delta) ||
        segment_index >= RUNTIME_CPU_DELTA_V1_SEGMENT_COUNT) return 0;
...
{
    if (!runtime_cpu_delta_v1_valid(delta) ||
        register_index >= RUNTIME_CPU_DELTA_V1_GPR16_COUNT) return 0;

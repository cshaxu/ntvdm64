  ...
  const uint8_t *bytes, uint32_t byte_count, int write)
{
  ...
  return write ? runtime_machine_binding_v1_memory_writable(physical_address,
    byte_count) : runtime_machine_binding_v1_memory_readable(physical_address,
    byte_count);
}
  ...
  uint8_t *bytes, uint32_t byte_count)
{
  return runtime_mantle_checked_ram_range_valid(physical_address, bytes,
    byte_count, 0) && runtime_machine_binding_v1_memory_read(physical_address,
      byte_count, bytes);
}
...
{
  return runtime_mantle_checked_ram_range_valid(physical_address, bytes,
    byte_count, 1) && runtime_machine_binding_v1_memory_write(physical_address,
      byte_count, bytes);
}
  ...
  struct runtime_mechanical_action_v1 *action)
{
  ...
  for (index = 0; index < action->range_count; ++index) {
    ...
    int accessible = action->kind == RUNTIME_MECHANICAL_ACTION_V1_READ ?
      runtime_machine_binding_v1_memory_readable(range->physical_address,
        range->byte_count) : runtime_machine_binding_v1_memory_writable(
  ...
  for (index = 0; index < action->range_count; ++index) {
    ...
    int copied = action->kind == RUNTIME_MECHANICAL_ACTION_V1_READ ?
      runtime_machine_binding_v1_memory_read(range->physical_address,
        range->byte_count, bytes) : runtime_machine_binding_v1_memory_write(

...
| `SOFTPC-DIV-003` | OpenNT DEM obtains CCPU/SAS register, pointer and handle state from the historical product host. | Raw pointers and product-global state cannot cross the modern machine boundary. | Preserve source-facing helpers through a call-scoped copied frame, checked mapping and opaque handle record; a generic post-body hook never identifies a BOP service. | `opennt_dem_ccpu_sas_facade.c:1`, `opennt_dem_ccpu_sas_facade.h:162` |

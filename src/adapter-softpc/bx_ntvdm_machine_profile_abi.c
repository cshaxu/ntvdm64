    ...
    bx_ntvdm_machine_profile_v1 *profile)
{
    ...
    profile->magic = BX_NTVDM_MACHINE_PROFILE_ABI_MAGIC;
    profile->abi_version = BX_NTVDM_MACHINE_PROFILE_ABI_VERSION;
    ...
    const bx_ntvdm_guest_range *guest_read)
{
    ...
    if (profile->observation_count >= BX_NTVDM_MACHINE_PROFILE_V1_MAX_OBSERVATIONS)
        return 0;
    ...
    uint32_t observation_count)
{
    ...
    if (profile == 0 || observation_ids == 0 || observation_count == 0u ||
        observation_count > BX_NTVDM_MACHINE_PROFILE_V1_MAX_OBSERVATIONS ||
        profile->magic != BX_NTVDM_MACHINE_PROFILE_ABI_MAGIC ||
        profile->abi_version != BX_NTVDM_MACHINE_PROFILE_ABI_VERSION ||
        profile->struct_bytes != sizeof(*profile) || profile->flags != 0u ||
        profile->reserved0 != 0u || profile->trigger_observation_id != 0u ||
    ...
    const bx_ntvdm_machine_profile_v1 *profile, uint64_t aperture_bytes)
{
    uint32_t index;
    uint32_t prior;
    if (profile == 0 || profile->magic != BX_NTVDM_MACHINE_PROFILE_ABI_MAGIC ||
        profile->abi_version != BX_NTVDM_MACHINE_PROFILE_ABI_VERSION ||
        profile->struct_bytes != sizeof(*profile) || profile->flags != 0u ||
        profile->reserved0 != 0u || profile->observation_count == 0u ||
        profile->observation_count > BX_NTVDM_MACHINE_PROFILE_V1_MAX_OBSERVATIONS)
        return 0;
    for (index = 0u; index < profile->observation_count; ++index) {
        const bx_ntvdm_machine_observation_v1 *observation =
            &profile->observations[index];
        if (observation->id == 0u || observation->flags != 0u ||
            observation->guest_read.length == 0u ||
            !bx_ntvdm_guest_range_within(aperture_bytes,
    ...
    if (profile->trigger_observation_id != 0u || profile->snapshot_trigger_count != 0u) {
        ...
        if (profile->trigger_observation_id != 0u) {
            ...
            if (profile->snapshot_trigger_count >
                BX_NTVDM_MACHINE_PROFILE_V1_MAX_OBSERVATIONS) return 0;
        ...
        for (index = 0u; index < BX_NTVDM_MACHINE_PROFILE_V1_MAX_OBSERVATIONS;
            ++index) {
            if (profile->snapshot_trigger_ids[index] != 0u) return 0;
        }
    }
...
int bx_ntvdm_machine_profile_v1_prepare_neutral_ud2_snapshot_trigger(
    ...
    uint64_t *resume_rip)
{
    bx_ntvdm_startup_snapshot_range_v1 ranges[
        BX_NTVDM_MACHINE_PROFILE_V1_MAX_OBSERVATIONS];
    uint32_t index;
    if (profile == 0 || window == 0 || transaction == 0 || resume_rip == 0 ||
        !bx_ntvdm_machine_profile_v1_valid(profile, aperture_bytes) ||
    ...
    bx_ntvdm_startup_snapshot_transaction_v1_initialize(transaction, boundary,
        cpu_before, ranges, profile->snapshot_trigger_count);
    if (!bx_ntvdm_startup_snapshot_transaction_v1_preflight(transaction,
    ...
    bx_ntvdm_observation_transaction_v1 *transaction)
{
    ...
    if (transaction == 0 || id == 0u ||
        !bx_ntvdm_machine_profile_v1_valid(profile, aperture_bytes)) return 0;
    for (index = 0u; index < profile->observation_count; ++index) {
        if (profile->observations[index].id == id) {
            bx_ntvdm_observation_transaction_v1_initialize(transaction, boundary,
                cpu_before, &profile->observations[index].guest_read);
            return bx_ntvdm_observation_transaction_v1_preflight(transaction,
    ...
    uint64_t *resume_rip)
{
    if (profile == 0 || window == 0 || transaction == 0 || resume_rip == 0 ||
        ...
        boundary->fault_rip > UINT64_MAX - profile->trigger_resume_bytes)
        return 0;
    if (!bx_ntvdm_machine_profile_v1_prepare_observation(profile,

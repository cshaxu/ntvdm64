...
/* DIVERGENCE(OPENNT-HOST-062): complete WOW cleanup owner with explicit
 * native context. Bound procedure indexing by elements, not bytes. Propagate
 * allocation/probe/native class-release failure through the WOW BOOL ABI;
 * a failed native release cannot satisfy original DestroyClass loop progress. */
#define WOW_ORIGINAL_CLEANUP
...
VOID PseudoDestroyClassWindows(PWND pwndParent, PCLS pcls, wow_cleanup_context *context)
{
    PWND pwnd;
    PTHREADINFO pti;
    ...
    for (pwnd = pwndParent->spwndChild; pwnd != NULL; pwnd = pwnd->spwndNext) {
        ...
        if (pwnd->spwndChild != NULL)
            PseudoDestroyClassWindows(pwnd, pcls, context);
    }
}
        ...
        for (i = 0; i < 2; ++i) {
            while (*ppcls != NULL) {
                ...
                if (HIWORD((*ppcls)->hModule) == (WORD)hInstance) {
                    if ((*ppcls)->cWndReferenceCount == 0) {
                         ...
                         */
                        PseudoDestroyClassWindows(PtiCurrent()->rpdesk->pDeskInfo->spwnd, *ppcls, context);
		    ...
		    continue;
                ...
                if (SelectorInSelList(SelListOrg, nSelOrg, sel)) {
                    ...
                    for (iSel = 0; iSel < sizeof(spfnwp) / sizeof(spfnwp[0]); iSel++) {
                        if (atom == gpsi->atomSysClass[iSel]) {
                            (*ppcls)->lpfnWndProc = (WNDPROC_PWND)spfnwp[iSel];
                            break;
                        }
                    }
                    if (iSel == sizeof(spfnwp) / sizeof(spfnwp[0]))

...
bx_bool BX_MEM_C::init_memory_without_sim(Bit64u guest, Bit64u host)
{
  ...
  if (BX_MEM_THIS actual_vector != NULL || BX_MEM_THIS vector != NULL ||
      BX_MEM_THIS blocks != NULL || BX_MEM_THIS memory_handlers != NULL) {
    return 0;
  }
...
bx_bool BX_MEM_C::ordinary_ram_writable(bx_phy_address addr, Bit64u len)
{
  ...
  bx_phy_address last;
  bx_phy_address page_last;
  struct memory_handler_struct *memory_handler;

  if (len == 0 ||
      len > (Bit64u)((bx_phy_address)-1) ||
      addr > ((bx_phy_address)-1) - (bx_phy_address)len)
    return 0;
  last = addr + (bx_phy_address)len - 1;

  // Validate the complete span before the first copy.  In particular, reject
  // direct-access handlers too: a returned pointer alone does not establish
  // that the target is ordinary RAM.
  current = addr;
  while (1) {
    page_last = current | 0xfff;
    if (page_last > last) page_last = last;
    if (A20ADDR(current) != current || A20ADDR(page_last) != page_last ||
        current >= BX_MEM_THIS len)
      return 0;
    memory_handler = BX_MEM_THIS memory_handlers[current >> 20];
    while (memory_handler) {
      if (memory_handler->begin <= page_last &&
          memory_handler->end >= current)
        return 0;
      memory_handler = memory_handler->next;
    }
#if BX_SUPPORT_MONITOR_MWAIT
    if (BX_MEM_THIS is_monitor(current & ~((bx_phy_address)0xfff), 0xfff))
      return 0;
#endif
    if (BX_MEM_THIS getHostMemAddr(NULL, current, BX_WRITE) == NULL)
      return 0;
...
bx_bool BX_MEM_C::backing_ram_accessible(bx_phy_address addr, Bit64u len)
{
  ...
  while (1) {
    ...
    if (BX_MEM_THIS is_monitor(current & ~((bx_phy_address)0xfff), 0xfff))
      return 0;
#endif
    // get_vector intentionally avoids getHostMemAddr, which applies A20.
...
bx_bool BX_MEM_C::copy_to_backing_ram(bx_phy_address addr, Bit64u len,
                                       const Bit8u *data)
{
...
bx_bool BX_MEM_C::copy_from_backing_ram(bx_phy_address addr, Bit64u len,
                                         Bit8u *data)
{
...
bx_bool BX_MEM_C::copy_to_ordinary_ram(bx_phy_address addr, Bit64u len,
                                        const Bit8u *data)
{
  bx_phy_address current;
  Bit64u remaining;
...
bx_bool BX_MEM_C::ordinary_ram_readable(bx_phy_address addr, Bit64u len)
{
  ...
  // Do not expose a handler, ROM, device or persistent guest-memory mapping.
  // The complete span must be ordinary RAM before copying its first byte.
  current = addr;
  while (1) {
    page_last = current | 0xfff;
    if (page_last > last) page_last = last;
    ...
    if (BX_MEM_THIS is_monitor(current & ~((bx_phy_address)0xfff), 0xfff))
      return 0;
#endif
    if (BX_MEM_THIS getHostMemAddr(NULL, current, BX_READ) == NULL)
      return 0;
    if (page_last == last) break;
    current = page_last + 1;
  }
...
bx_bool BX_MEM_C::copy_from_ordinary_ram(bx_phy_address addr, Bit64u len,
                                          Bit8u *data)
{
  bx_phy_address current;
  bx_phy_address page_last;
  ...
  if (data == NULL || !BX_MEM_THIS ordinary_ram_readable(addr, len))
    return 0;

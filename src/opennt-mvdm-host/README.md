...
| MVDM-HOST-DIV-009 | Write `cmdGetCurrentDir`'s `DS:SI` result directly through an unbounded `GetVDMAddr` pointer. | A guest pointer cannot be retained as host memory on x64 or across the modern component boundary. | Retain the original drive/environment order and CF/AX outcomes; stage the at-most-67-byte OEM result in a local buffer, then use the session-owned adapter-softpc numeric descriptor and exact synchronous guest write. | `dos/command/cmdmisc.c` |

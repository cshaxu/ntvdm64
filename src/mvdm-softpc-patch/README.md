...
| MVDM-SOFTPC-PATCH-002 | `softpc.new/base/ccpu386` reached SAS/fast-BOP hooks: `EDL_fast_bop`, `c_VirtualiseInstruction`, `c_sas_touch`, timestamp helpers | `O:\\repos.external\\ntvdmx64` commit `84a13d2e7bb1a55d11148971e5b9c8ec99f670bf`, `ntvdmpatch/patches/common/fmstubs.c`, SHA-256 `03F45AD3124379B2462AFA5434EF479012421C45096C6CCDCE4296B151BF3025` | Supplies the NTVDMx64 patch's original fallback definitions for unresolved CCPU hooks and timing helpers | Owner-directed import: the patch is part of the historical SoftPC patch set and must remain available as evidence and an x86 build input; its breakpoint behavior is preserved rather than silently replaced | `patches/common/fmstubs.c` (byte-identical import) | Not bound yet. `c_sas_touch` and `c_VirtualiseInstruction` retain the original patch's `int 3` behavior; any later non-breakpoint path must use a registered mapping-manager lease, not a host-pointer cast | Hash-verified and compiled/archived on x86 as `ntvdmx64-softpc-patch-evidence.lib`; x64 keeps the exact source as x86-only patch evidence because its inline assembly is not x64-composable | A focused interface decision proves an original or adapter-backed implementation for each reached hook; the source remains retained as patch evidence |

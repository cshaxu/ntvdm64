...
{
    store(&reason, APP_SESSION_CANCELLATION_V1_NONE);
#if defined(RUNTIME_CANCELLATION_TESTING)
    store(&test_polls, 0u);
#endif
...
int app_session_cancellation_v1_request(uint32_t input)
{
    uint32_t expected = APP_SESSION_CANCELLATION_V1_NONE;
    if (input != APP_SESSION_CANCELLATION_V1_USER_REQUEST ||
        load(&active) == 0u ||
...
{
    return load(&active) == 0u ? APP_SESSION_CANCELLATION_V1_NONE :
        load(&reason);
}

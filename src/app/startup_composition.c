...
typedef struct runtime_dem_startup {
    byob_image ntio, ntdos, command, target;
    runtime_initial_state initial_state;
...
static runtime_dem_startup runtime;
...
{
    wchar_t launch_text[BYOB_LAUNCH_PLAN_ENV_CHARS];
    const wchar_t *extension;
    uint32_t expected_kind;
    size_t bytes;
    if (launch_input == NULL || selection == NULL ||
        !descriptor_to_wide(launch_input, launch_chars, launch_text,
            BYOB_LAUNCH_PLAN_ENV_CHARS) ||
        !byob_launch_plan_from_environment(&runtime.launch, launch_text) ||
        runtime.launch.slot_count != selection->declared_target_count) return 0;
    extension = wcsrchr(selection->target.file_name, L'.');
    expected_kind = extension != NULL && _wcsicmp(extension, L".COM") == 0 ? BYOB_LAUNCH_TARGET_KIND_COM :
        extension != NULL && _wcsicmp(extension, L".EXE") == 0 ? BYOB_LAUNCH_TARGET_KIND_EXE :
        extension != NULL && _wcsicmp(extension, L".BAT") == 0 ? BYOB_LAUNCH_TARGET_KIND_BAT :
        extension != NULL && _wcsicmp(extension, L".PIF") == 0 ? BYOB_LAUNCH_TARGET_KIND_PIF : 0u;
    ...
    const uint16_t *launch, uint32_t launch_chars, uint32_t mutation_mode)
{
    ...
    /* the startup path is intentionally Direct-only; legacy mutation profiles are not a
    ...
    if (mutation_mode != 1u || launch_chars == 0u ||
        ...
        (selection.declared_target_count = 1u, selection.has_target_placement = 1u,
         ...
         selection.guest_display_state = BYOB_GUEST_DISPLAY_STATE_STREAM_IO,
    ...
    if (selection.has_machine_external_initial_state != 0u &&
        !runtime_initial_state_catalog_select(
    ...
    runtime.installed = 1;
    runtime_spckbd_handoff_display_state_set(
...
static int startup_plan(runtime_startup_plan *plan,
    const uint8_t **payload, uint64_t *payload_bytes)
{
    ...
    entry.cs = 0x70u;
    runtime_guest_write_initialize(&write, 0x700u,
        (uint64_t)runtime.ntio.byte_count, 0u);
    runtime_startup_plan_initialize(plan, &write, &entry, 0x714u, 4u);
    if (!runtime_startup_plan_preflight(plan, UINT64_C(0x100000),
    ...
    struct runtime_machine_stage_request *request)
{
    ...
    if (request == NULL || !startup_plan(&plan, &payload, &payload_bytes) ||
        payload_bytes > RUNTIME_MECHANICAL_ACTION_MAX_BYTES) return 0;
    runtime_machine_stage_request_clear(request);
    if (runtime.initial_state.range_count != 0u &&
        !runtime_initial_state_action_prepare(&runtime.initial_state,
            &request->initial_state_action)) return 0;
    /* This is the selector-blind machine ABI initializer.  Request clear owns
    ...
    /* The machine sees only a configured IVT entry.  The provider-specific
    ...
    struct runtime_machine_stage_entry *entry)
{
    ...
    return runtime_machine_stage_entry_valid(entry);
}

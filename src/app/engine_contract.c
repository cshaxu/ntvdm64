...
{
    return mutation_mode == RUNTIME_ENGINE_MUTATION_MODE_DIRECT ||
        mutation_mode == RUNTIME_ENGINE_MUTATION_MODE_READONLY;
}

static int terminal_kind_valid(uint32_t terminal_kind)
{
    return terminal_kind <= RUNTIME_ENGINE_TERMINAL_HOST_CANCELLATION;
}
...
void runtime_engine_request_clear(struct runtime_engine_request *request)
{
    ...
    request->magic = RUNTIME_ENGINE_REQUEST_MAGIC;
    request->abi_version = RUNTIME_ENGINE_CONTRACT_VERSION;
...
{
    if (request == 0 || request->magic != RUNTIME_ENGINE_REQUEST_MAGIC ||
        request->abi_version != RUNTIME_ENGINE_CONTRACT_VERSION ||
        ...
        !descriptor_valid(request->profile_descriptor,
            ...
            RUNTIME_ENGINE_MAX_DESCRIPTOR_CHARS) ||
        ...
        !descriptor_valid(request->launch_descriptor,
            ...
            RUNTIME_ENGINE_MAX_LAUNCH_CHARS)) return 0;
    return 1;
}
...
void runtime_engine_result_clear(struct runtime_engine_result *result)
{
    ...
    result->magic = RUNTIME_ENGINE_RESULT_MAGIC;
    result->abi_version = RUNTIME_ENGINE_CONTRACT_VERSION;
...
{
    return result != 0 && result->magic == RUNTIME_ENGINE_RESULT_MAGIC &&
        result->abi_version == RUNTIME_ENGINE_CONTRACT_VERSION &&

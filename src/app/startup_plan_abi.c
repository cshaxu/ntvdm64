...
static int runtime_startup_entry_delta_valid(
    const runtime_cpu_state *entry)
{
    return entry != 0 && runtime_cpu_state_valid(entry) &&
    ...
    uint64_t aperture_bytes, uint64_t payload_bytes)
{
    ...
    if (plan == 0 || plan->magic != RUNTIME_STARTUP_PLAN_ABI_MAGIC ||
        ...
        !runtime_guest_write_preflight(&plan->payload_write,
            aperture_bytes, payload_bytes) ||
        !runtime_startup_entry_delta_valid(&plan->entry_cpu) ||

...
static void render(HANDLE output,DTASKMGR_STATE *state,DTASKMGR_WORKER *items,ULONG count)
{
    ...
    fwprintf(stdout,L"\nUp/Down select   K/Delete end selected worker   Q/Esc exit\n");
    fflush(stdout);
}
static DWORD refresh(DTASKMGR_STATE *state,DTASKMGR_WORKER **items,ULONG *count)
{
    ...
    RpcTryExcept {
        error=Client_TaskSnapshot(state->binding,GetCurrentProcess(),APP_PROTOCOL_VERSION,
...
static DWORD request_terminate(DTASKMGR_STATE *state)
{
    ...
    RpcTryExcept {
        error=Client_TerminateWorker(state->binding,GetCurrentProcess(),APP_PROTOCOL_VERSION,

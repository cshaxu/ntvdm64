...
`mvdm_wow_callback_transaction` is a copied, one-session synchronous guest
transaction for callback-reached numeric values. It uses the monitor-owned
task-frame projection and returns only AX:DX reread from the callback frame.
It does not expose a raw guest/native pointer or add a callback worker, CCPU
executor, CSRSS broker or WOW provider.

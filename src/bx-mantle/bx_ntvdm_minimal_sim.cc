...
static bx_param_c *bx_ntvdm_find_param(const char *from, bx_param_c *base)
{
  ...
  return bx_ntvdm_find_param(separator + 1, child);
}

class bx_ntvdm_minimal_sim_c : public bx_simulator_interface_c {
public:
  bx_ntvdm_minimal_sim_c() { is_sim_thread_func = NULL; }
  ...
  virtual bx_param_c *get_param(const char *pname, bx_param_c *base = NULL)
  {
    ...
    return bx_ntvdm_find_param(pname, base);
  }
...
static bx_ntvdm_minimal_sim_c *bx_ntvdm_minimal_sim = NULL;

bx_ntvdm_minimal_sim_status bx_ntvdm_minimal_sim_initialize(void)
{
#if BX_CPU_LEVEL != 5 || BX_SUPPORT_X86_64
  return BX_NTVDM_MINIMAL_SIM_PROFILE_CONFIGURATION_UNSUPPORTED;
#else
  if (SIM != NULL && SIM != bx_ntvdm_minimal_sim) {
    return BX_NTVDM_MINIMAL_SIM_ALREADY_BOUND;
  }

  if (SIM != NULL) {
    bx_param_bool_c *reset = SIM->get_param_bool(BXPN_RESET_ON_TRIPLE_FAULT);
    return (reset != NULL && reset->get() == 0)
      ? BX_NTVDM_MINIMAL_SIM_OK
      : BX_NTVDM_MINIMAL_SIM_RESET_PARAM_NOT_FALSE;
  }

  siminterface_log = new logfunctions();
  siminterface_log->put("minimal_sim", "CTRL");
  bx_ntvdm_minimal_sim = new bx_ntvdm_minimal_sim_c();
  SIM = bx_ntvdm_minimal_sim;

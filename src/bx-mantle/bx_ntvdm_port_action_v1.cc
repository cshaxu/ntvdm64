...
{ if (!bx_ntvdm_port_action_v1_lifecycle_active || !bx_ntvdm_port_action_v1_valid(action)) return 0; if (action->kind == BX_NTVDM_PORT_ACTION_V1_READ8) action->value = (uint8_t)bx_devices.inp(action->port, 1u); else bx_devices.outp(action->port, action->value, 1u); return 1; }
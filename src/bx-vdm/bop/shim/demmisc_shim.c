...
{
    /* DIVERGENCE: VDDCreateUserHook requires the NT4 VDD broker.  Do not
...
{
    /* DIVERGENCE: these symbols are VDD/debugger notifications, not DOS
...
{
    /* DIVERGENCE: a debugger console can write guest-visible input only once
...
{
    /* DIVERGENCE: HostFloppyReset is FDC/DMA/CMOS machine behavior, not a

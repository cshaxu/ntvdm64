...
#define FCB_PATH_BYTES BX_NTVDM_DEM_PATH_V1_MAX_OEM_BYTES
    ...
    struct bx_ntvdm_mechanical_action_v1 *out)
{
    ...
    for (index = 0u; index < gather->range_count; ++index) {
        if (gather->ranges[index].length > UINT32_MAX ||
            offset > out->payload_bytes - (uint32_t)gather->ranges[index].length) return 0;
    ...
    struct bx_ntvdm_mechanical_action_v1 *action_out)
{
    ...
    if (provider == 0 || action_out == 0 || !matches(service, boundary, cpu, window) ||
        !bx_ntvdm_dem_fcb_path_route_partition_v1_owns_service(service) ||
        !bx_ntvdm_dem_whole_provider_v1_valid(provider)) return 0;
    if (service == 0x07u) {
        if (!physical(cpu->es, (uint16_t)cpu->edi, FCB_PATH_BYTES, &ranges[0].address)) return 0;
    } else if (!physical(cpu->ds, (uint16_t)cpu->esi, FCB_PATH_BYTES, &ranges[0].address)) {
        return 0;
    }
    ranges[0].length = FCB_PATH_BYTES;
    if (service == 0x20u) {
        if (!physical(cpu->es, (uint16_t)cpu->edi, FCB_PATH_BYTES, &ranges[1].address)) return 0;
        ranges[1].length = FCB_PATH_BYTES;
        count = 2u;
    }
    return bx_ntvdm_dem_whole_provider_v1_prepare_gather(provider, service,
        boundary, cpu, ranges, count, &gather) && action_from_gather(provider, &gather, action_out);
}
    ...
    bx_ntvdm_cpu_result_v2 *result_out)
{
    ...
    char first[FCB_PATH_BYTES], second[FCB_PATH_BYTES];
    uint32_t copied_bytes = 0u;
    uint32_t expected = service == 0x20u ? 2u * FCB_PATH_BYTES : FCB_PATH_BYTES;
    if (provider == 0 || boundary == 0 || cpu == 0 || read_action == 0 || result_out == 0 ||
        !bx_ntvdm_dem_fcb_path_route_partition_v1_owns_service(service) ||
        read_action->kind != BX_NTVDM_MECHANICAL_ACTION_V1_READ ||
        !bx_ntvdm_mechanical_action_v1_valid(read_action) || read_action->payload_bytes != expected)
        return 0;
    ...
    memcpy(first, copied, FCB_PATH_BYTES);
    if (memchr(first, '\0', sizeof(first)) == 0) return 0;
    if (service == 0x20u) {
        memcpy(second, copied + FCB_PATH_BYTES, FCB_PATH_BYTES);
        if (memchr(second, '\0', sizeof(second)) == 0) return 0;
        return bx_ntvdm_dem_fcb_wildcard_partition_v1_dispatch(provider, service,
    ...
    return bx_ntvdm_dem_fcb_handle_partition_v1_dispatch(provider, service,
        boundary, cpu, first, 0, 0u, 0, result_out) && bx_ntvdm_cpu_result_v2_valid(result_out);
}

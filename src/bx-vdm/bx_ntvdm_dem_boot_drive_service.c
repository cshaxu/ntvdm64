    ...
    bx_ntvdm_exception_result_v1 *result)
{
    uint32_t drive;
    uint16_t ax;
    if (drives == 0 || !bx_ntvdm_host_drive_snapshot_v1_valid(drives) ||
        event == 0 || cpu_before == 0 || window == 0 || result == 0 ||
        !bx_ntvdm_exception_event_v1_valid(event) ||
        !bx_ntvdm_cpu_state_v1_valid(cpu_before) ||
        !bx_ntvdm_instruction_window_v1_valid(window) || event->vector != 6u ||
        cpu_before->execution_mode != BX_NTVDM_CPU_EXECUTION_REAL ||
        ...
        window->bytes[3] != 0x0du || event->fault_rip > UINT64_MAX - 4u)
        return 0;
    if (preferred_drive < 26u &&
        (drives->admitted_mask & (UINT32_C(1) << preferred_drive)) != 0u)
        drive = preferred_drive;
    else if ((drives->admitted_mask & (UINT32_C(1) << 2u)) != 0u &&
        drives->types[2] == DRIVE_FIXED) drive = 2u;
    else {
        for (drive = 0u; drive < 26u; ++drive) {
            if ((drives->admitted_mask & (UINT32_C(1) << drive)) != 0u &&
                drives->types[drive] == DRIVE_FIXED) break;
        }
        if (drive == 26u) return 0;
    }
    ax = (uint16_t)((cpu_before->eax & 0xff00u) | (drive + 1u));
    if (!bx_ntvdm_exception_result_v1_resume(result, event->fault_rip + 4u) ||
        !bx_ntvdm_cpu_delta_v1_set_gpr16(&result->cpu_delta, 0u, ax)) {
        bx_ntvdm_exception_result_v1_pass_through(result);
        return 0;

...
int bx_ntvdm_system_plane_v1_valid(const bx_ntvdm_system_plane_record_v1 *r) {
    return r && r->magic==BX_NTVDM_SYSTEM_PLANE_V1_MAGIC && r->abi_version==1u &&
        ...
        r->component>=BX_NTVDM_SYSTEM_COMPONENT_REDIR_LIFECYCLE && r->component<=BX_NTVDM_SYSTEM_COMPONENT_DEBUG_DISPATCH &&
...
static uint32_t redir_component(uint8_t n) { if(n<2)return 1; if(n<9)return 2; if(n<15)return 3; if(n==15)return 1;
    if(n>=16&&n<=26)return 4; if(n>=27&&n<=34)return 5; if(n>=35&&n<=40)return 6;
    if(n>=47&&n<=49)return 6; return 7; }
int bx_ntvdm_system_plane_v1_classify(const bx_ntvdm_bop_ingress_v1 *i,
    ...
    if(i->route!=BX_NTVDM_BOP_ROUTE_MAPPED_DEFERRED || s->disposition!=BX_NTVDM_BOP_PROVIDER_DEFERRED ||
       ...
       !((i->family==BX_NTVDM_BOP_FAMILY_REDIR && s->provider_family==BX_NTVDM_BOP_PROVIDER_REDIR && i->service<50u) ||
         (i->family==BX_NTVDM_BOP_FAMILY_DEBUGGER && s->provider_family==BX_NTVDM_BOP_PROVIDER_DEBUGGER && i->has_service==0u))) return 0;
    r->service=i->service; r->provider_family=s->provider_family;
    r->component=i->family==BX_NTVDM_BOP_FAMILY_REDIR?redir_component(i->service):BX_NTVDM_SYSTEM_COMPONENT_DEBUG_DISPATCH;

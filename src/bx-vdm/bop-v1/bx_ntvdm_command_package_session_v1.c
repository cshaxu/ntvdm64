...
{ uint8_t payload[12];uint64_t address;
  ...
  if(address>UINT64_C(0x100000)-sizeof(payload)||!read_one(s,address,sizeof(payload),payload)) {
    if(s->launch_execution_provider.stream_session.rejected_record_count!=UINT32_MAX)++s->launch_execution_provider.stream_session.rejected_record_count;
    return 0;
  }
  return bx_ntvdm_command_stream_session_v1_validate_std_handles(&s->launch_execution_provider.stream_session,payload,sizeof(payload)); }
...
int bx_ntvdm_command_package_session_v1_copy_ordinary_completion(const bx_ntvdm_command_package_session_v1 *s)
{ return bx_ntvdm_command_package_session_v1_valid(s) &&
    bx_ntvdm_cmd_get_next_ordinary_completion_v1(&s->bootstrap_provider.get_next); }
...
{ return bx_ntvdm_command_package_session_v1_valid(s) && s->has_mutation_profile &&
    bx_ntvdm_command_launch_execution_provider_v1_admit_cli_streams(
        &s->launch_execution_provider, &s->mutation_profile); }
...
{ bx_ntvdm_guest_gather_read_action_v1 read;bx_ntvdm_multi_write_transaction_v1 write;uint8_t payload[BX_NTVDM_MULTI_WRITE_MAX_PAYLOAD];uint32_t env=0u;
  if(s&&s->bootstrap_provider.comspec.stage==BX_NTVDM_CMD_COMSPEC_BOOTSTRAP_ENVIRONMENT_DELIVERED)env=s->bootstrap_provider.comspec.environment_bytes;

    ...
    const bx_ntvdm_readonly_namespace_v1 *namespace_value)
{
    ...
    return bx_ntvdm_command_boot_input_v1_initialize_paths(value,
        namespace_value->drive_index, namespace_value->files[1].path,
        namespace_value->files[2].path);
}
...
{
    if (value == 0 || drive_index >= 26u) return 0;
    memset(value, 0, sizeof(*value));
    value->magic = BX_NTVDM_COMMAND_BOOT_INPUT_V1_MAGIC;
    value->abi_version = BX_NTVDM_COMMAND_BOOT_INPUT_V1_VERSION;
    value->struct_bytes = sizeof(*value);
    value->drive_index = drive_index;
    if (!copy_guest_path(value->config_path, config_path) ||
        !copy_guest_path(value->autoexec_path, autoexec_path)) {
        memset(value, 0, sizeof(*value));
        return 0;
    }

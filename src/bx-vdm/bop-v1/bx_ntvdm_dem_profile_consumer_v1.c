    ...
    uint32_t permitted_mode_mask)
{
    ...
    registration.owner_id = BX_NTVDM_MUTATION_OWNER_V1_DEM;
...
{
    return consumer != 0 &&
        consumer->magic == BX_NTVDM_DEM_PROFILE_CONSUMER_V1_MAGIC &&
        consumer->abi_version == BX_NTVDM_DEM_PROFILE_CONSUMER_V1_VERSION &&
        consumer->struct_bytes == sizeof(*consumer) && consumer->reserved0 == 0u &&
...
{
    if (consumer == 0 || profile == 0 ||
        !bx_ntvdm_mutation_profile_v1_valid(profile)) return 0;
    memset(consumer, 0, sizeof(*consumer));
    consumer->magic = BX_NTVDM_DEM_PROFILE_CONSUMER_V1_MAGIC;
    consumer->abi_version = BX_NTVDM_DEM_PROFILE_CONSUMER_V1_VERSION;
    consumer->struct_bytes = sizeof(*consumer);
    consumer->profile = *profile;
    return bx_ntvdm_dem_profile_consumer_v1_valid(consumer);
}
...
{
    if (!bx_ntvdm_dem_profile_consumer_v1_valid(consumer)) return 0;
    return bx_ntvdm_mutation_profile_v1_resolve(&consumer->profile,
        BX_NTVDM_MUTATION_OWNER_V1_DEM, mutation_class, policy_result_out);
}

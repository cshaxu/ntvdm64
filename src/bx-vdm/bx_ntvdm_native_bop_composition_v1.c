...
{ bx_ntvdm_exception_event_v1 b;bx_ntvdm_cpu_state_v1 c;bx_ntvdm_instruction_window_v1 w;bx_ntvdm_bop_ingress_v1 i;bx_ntvdm_bop_provider_selection_v1 p;bx_ntvdm_cpu_result_v2 r;
  if(!valid(active)||!active->bound||!o||!unpack(e,&b,&c,&w)||!bx_ntvdm_bop_ingress_v1_dispatch(&b,&c,&w,&i,&r)||r.disposition!=BX_NTVDM_CPU_RESULT_V2_PASS_THROUGH||!bx_ntvdm_bop_provider_registry_v1_select(&i,&p))return 0;

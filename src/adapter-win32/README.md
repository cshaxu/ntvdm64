...
`include/compiler/` and `include/opennt-api/` are production ABI input
...
| `WIN32-DIV-003` | `softpc.new/host/src/nt_pif.c` calls the NTVDM product's OEM path/environment/title helpers. | Those private exports are unavailable to a standalone program. | Retain the reached helper signatures and results through public OEM/ANSI Win32 APIs. | `facade/opennt_pif_oem_facade.c:1`, `include/opennt-api/opennt_pif_oem_facade.h` |
| `WIN32-DIV-004` | Recovered COMMAND callers link selected RTL conversion, environment and NTSTATUS mapping routines through NT4 host composition. | That product import closure is not an independently linkable application contract. | Preserve the reached names, argument layouts, allocation/failure rules and status mapping through public Win32 plus bounded host-private storage. | `facade/opennt_rtl_compat.c:12`, `include/opennt-api/opennt_rtl_types.h` |
| `WIN32-DIV-005` | OpenNT DEM calls OEM directory exports supplied by the historical NTVDM host. | Those private product exports are unavailable to a standalone process. | Retain the original names, parameters and last-error behavior through OEM/ANSI public Win32 calls. | `facade/opennt_dem_oem_facade.c:1`, `include/opennt-api/opennt_dem_oem_facade.h` |
| `WIN32-DIV-006` | OpenNT DEM calls OEM file exports supplied by the historical NTVDM host. | The original TEB static-Unicode buffer and product exports are not public modern API. | Retain original function spellings, parameter and retry order through public OEM conversion and Win32 file APIs. | `facade/opennt_dem_file_oem_facade.c:1`, `include/opennt-api/opennt_dem_file_oem_facade.h` |
| `WIN32-DIV-007` | OpenNT DEM queries OEM host computer and disk information through NTVDM product composition. | Those product exports do not form an independent API closure. | Retain original names, argument and Win32 error behavior through public ANSI Win32 calls. | `facade/opennt_dem_gset_facade.c:1`, `include/opennt-api/opennt_dem_gset_facade.h` |
| `WIN32-DIV-008` | OpenNT DEM searches through OEM find exports and calls the removed invasive `NtVdmControl`. | The old product API is absent from supported modern Windows. | Retain source spellings: public OEM find calls, public process heap, and `STATUS_NOT_IMPLEMENTED` so the imported slow fallback remains observable. | `facade/opennt_dem_search_facade.c:1`, `include/opennt-api/opennt_dem_search_facade.h` |
| `WIN32-DIV-009` | OpenNT `GetNextVDMCommand` and `SetVDMCurrentDirectories` transfer VDM command data and re-entry state through the NT4 BaseSrv product broker. | BaseSrv/CSR is not a public, standalone application API. | Retain the original `VDMINFO` ABI, names, flags, calling conventions and ordered failure results; bind a copied session-input record only for the reached caller/worker scope. | `facade/opennt_vdm_api_facade.h:1`, `facade/opennt_vdm_api_facade.c:1` |
| `WIN32-DIV-010` | NT4 VDM callers can read a hidden `=X:` current-directory environment entry. | Public modern process environments do not reliably surface that hidden entry. | Preserve the fallback with `GetCurrentDirectoryA` after the normal `GetEnvironmentVariableA` query. | `facade/opennt_command_oem_facade.c`, `facade/opennt_command_oem_facade.h` |
| `WIN32-DIV-011` | OpenNT COMMAND reads the active keyboard layout through a private console export. | The private export is not linkable from modern public Win32 import libraries. | Use public `GetKeyboardLayoutNameA`, preserving the same returned layout-name contract. | `facade/opennt_command_oem_facade.c`, `facade/opennt_command_oem_facade.h` |

...
| `BX-DPMI-MECH-002` | Bochs normally changes real-mode stack/control state only by decoded instructions. | Recovered historical host paths need a copied, comparison-guarded real-mode CS/SS/SP/IP/FLAGS transition while the CPU loop is returned. | The private member body validates real mode before any mutation, loads real-mode SS/CS, restores SP/FLAGS/IP and invalidates prefetch. It owns no interrupt, DPMI, BOP or guest-memory semantics. | `cpu/realmode_profile.cc`, `../bochs-core/cpu/cpu.h` |

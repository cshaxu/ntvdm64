...
| `BX-MACH-027` | Bochs keyboard initialization assumes plugin, SIM, GUI, mouse and paste product facilities. | Only the non-product teardown body lives here; original `keyboard.cc` retains the same-shaped plugin entry and is the sole overlay caller. | `iodev/keyboard_lifecycle.cc`, `../bochs-core/iodev/keyboard.{cc,h}` |
...
| `BX-CORE-DIV-002` | Segment-limit faults are native CPU mechanics. | The default-off copied observation record is constructed privately; the adopted `access32.cc` retains only the same native fault path plus one member call. | `cpu/segment_access_hook.cc`, `../bochs-core/cpu/access32.cc` |
| `BX-CORE-DIV-003` | Instruction, software-interrupt and physical-write events are native CPU/memory mechanics. | Each default-off copied record is constructed privately and sent through the opaque core callback; the adopted mirrors retain only member call sites. | `cpu/instruction_history_hook.cc`, `cpu/software_interrupt_hook.cc`, `cpu/physical_write_hook.cc`, `../bochs-core/cpu/{instr.h,soft_int.cc,cpu.h}`, `../bochs-core/memory/memory.cc` |
| `BX-CORE-DIV-004` | IRET is a native CPU transfer. | The default-off copied interrupt-return record is private; the adopted transfer files retain only member call sites. | `cpu/interrupt_return_hook.cc`, `../bochs-core/cpu/{ctrl_xfer16.cc,ctrl_xfer32.cc}` |

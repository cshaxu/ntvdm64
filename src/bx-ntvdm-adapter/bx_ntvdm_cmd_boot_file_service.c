    ...
    uint8_t payload[BX_NTVDM_MULTI_WRITE_MAX_PAYLOAD])
{
    ...
    if (namespace_value == 0 || event == 0 || cpu_before == 0 || window == 0 ||
        transaction == 0 || payload == 0 || !bx_ntvdm_exception_event_v1_valid(event) ||
        ...
        event->fault_rip > UINT64_MAX - 4u || namespace_value->drive_index >= 26u ||
        !bx_ntvdm_cmd_boot_file_physical(cpu_before->ds,
            (uint16_t)cpu_before->edx, &address)) return 0;

    file_index = window->bytes[3] == 0x0cu ? 1u : 2u;
    guest_path = namespace_value->files[file_index].path;
    if (guest_path[0] != L'\\') return 0;
    bytes = wcslen(guest_path) + 3u; /* drive, colon, path, NUL */
    if (bytes > BX_NTVDM_CMD_BOOT_FILE_MAX_PATH) return 0;
    payload[0] = (uint8_t)('A' + namespace_value->drive_index);
    payload[1] = ':';
    for (index = 0u; index <= wcslen(guest_path); ++index) {
        if (guest_path[index] > 0x7fu) return 0;

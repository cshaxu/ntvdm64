    ...
    HANDLE *worker)
{
    ...
    *reservation=0;*task=0;*console=NULL;*shared_wow=FALSE;*worker=NULL;
    ...
    for (cursor=state->entries.Flink;cursor!=&state->entries;cursor=cursor->Flink) {
        ...
        /* The launcher registered this exact worker before resuming it.  Keep
         * that same-access duplicate for the BaseSrv worker watch: the worker
         * self-handle used by Connect is deliberately query/synchronize-only,
         * while the server owns selected-worker termination. */

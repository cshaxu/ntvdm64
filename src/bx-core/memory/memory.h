...
public:
 ...
 ~BX_MEM_C();
  ...
  // Copies only after every affected page has been proved to be ordinary
  // writable RAM. It never falls back to physical/device writes.
  BX_MEM_SMF bx_bool copy_to_ordinary_ram(bx_phy_address addr, Bit64u len,
                                          const Bit8u *data);

  // Copies only after every affected page has been proved to be ordinary
  // readable RAM. It never falls back to physical/device reads.
  BX_MEM_SMF bx_bool copy_from_ordinary_ram(bx_phy_address addr, Bit64u len,
                                            Bit8u *data);

  // Performs the same side-effect-free whole-span ordinary-RAM read check as
  // copy_from_ordinary_ram, without copying or retaining a host mapping.
  BX_MEM_SMF bx_bool ordinary_ram_readable(bx_phy_address addr, Bit64u len);

...
- The former generic mapping manager is removed. The sole Win32/x86 product
  uses the original direct process-local pointer/HANDLE carriers; guest-memory
  leasing remains a separate bounded-copy concern.
...
- `session.c` and `session.h`: `remove temporary generic route`. The previous
  ...
  with kernel-VDM monitor control. `ntvdm/monitor` now owns
...
- `session.c` and `session.h`: `small neutral extension`.  A bound session may
  ...
  a BOP selector, or an OpenNT service; `ntvdm/softpc` remains

...
/*
 * The original CCPU SAS writer calls these helpers when LIM is selected, but
 * the available NTVDM build profile excludes their source body. A dedicated
 * default-disabled reconstruction object may expose this unchanged block for
 * link-closure evidence only; normal NTVDM builds keep the historical guard.
 */
#if !defined(NTVDM) || defined(NTDOS64_CCPU_LIM_WRITEBACK_RECONSTRUCTION)
...
#endif	/* !NTVDM || NTDOS64_CCPU_LIM_WRITEBACK_RECONSTRUCTION */

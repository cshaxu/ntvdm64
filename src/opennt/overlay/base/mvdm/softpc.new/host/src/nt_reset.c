...
/*
 * The default-disabled NTDOS64 trace target stops here to observe the
 * original host lifecycle before console/UI or event-thread setup. This is
 * instrumentation only; it does not replace any historical host behavior.
 */
#ifdef NTDOS64_HOST_APPLINIT_TRACE
extern void ntdos64_host_applinit_trace_checkpoint(int stage);
#define NTDOS64_HOST_APPLINIT_TRACE_STAGE(stage) \
    ntdos64_host_applinit_trace_checkpoint(stage)
#else
#define NTDOS64_HOST_APPLINIT_TRACE_STAGE(stage) ((void)0)
#endif
...
// Check is for new console session
    ...
    if (bSwitchF == FALSE)
        ExitProcess (0);
    NTDOS64_HOST_APPLINIT_TRACE_STAGE(12);
	...
	StreamIoSwitchOn = FALSE;
	    ...
	    enable_stream_io();
    NTDOS64_HOST_APPLINIT_TRACE_STAGE(13);
    ...
    NTDOS64_HOST_APPLINIT_TRACE_STAGE(14);
    InitializeIcaLock();
    host_ica_lock();
    NTDOS64_HOST_APPLINIT_TRACE_STAGE(17);

    init_host_uis();    /* console setup */
    NTDOS64_HOST_APPLINIT_TRACE_STAGE(30);
    nt_start_event_thread();      /* Start event processing thread */
    NTDOS64_HOST_APPLINIT_TRACE_STAGE(31);
}

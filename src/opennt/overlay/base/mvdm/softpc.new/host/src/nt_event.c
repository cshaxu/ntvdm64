...
int PointerAttachedWindowed = FALSE;     /* So re-attached on FS switch */
...
/* Default-disabled host_applInit trace checkpoints; no event behavior changes. */
#ifdef NTDOS64_HOST_APPLINIT_TRACE
extern void ntdos64_host_applinit_trace_checkpoint(int stage);
#define NTDOS64_HOST_EVENT_TRACE_STAGE(stage) \
    ntdos64_host_applinit_trace_checkpoint(stage)
#else
#define NTDOS64_HOST_EVENT_TRACE_STAGE(stage) ((void)0)
#endif
...
{
    NTDOS64_HOST_EVENT_TRACE_STAGE(40);
    //
    ...
    NTDOS64_HOST_EVENT_TRACE_STAGE(41);
	...
	if(!SetConsoleCtrlHandler((PHANDLER_ROUTINE)CntrlHandler,TRUE))
	    DisplayErrorTerm(EHS_FUNC_FAILED,GetLastError(),__FILE__,__LINE__);
        NTDOS64_HOST_EVENT_TRACE_STAGE(42);
        ...
        if(!ThreadInfo.EventMgr.Handle)
            DisplayErrorTerm(EHS_FUNC_FAILED,GetLastError(),__FILE__,__LINE__);
        NTDOS64_HOST_EVENT_TRACE_STAGE(43);

        if(!(hConsoleWait = CreateEvent(NULL, FALSE, FALSE, NULL)))
            DisplayErrorTerm(EHS_FUNC_FAILED,GetLastError(),__FILE__,__LINE__);
        NTDOS64_HOST_EVENT_TRACE_STAGE(44);

        if(!(hConsoleWaitStall = CreateEvent(NULL, FALSE, FALSE, NULL)))
            DisplayErrorTerm(EHS_FUNC_FAILED,GetLastError(),__FILE__,__LINE__);
        NTDOS64_HOST_EVENT_TRACE_STAGE(45);

    if(!(hConsoleSuspend = CreateEvent(NULL, FALSE, FALSE, NULL)))
            DisplayErrorTerm(EHS_FUNC_FAILED,GetLastError(),__FILE__,__LINE__);
        NTDOS64_HOST_EVENT_TRACE_STAGE(46);


        InitQueue();
        NTDOS64_HOST_EVENT_TRACE_STAGE(47);
        check_malloc(key_history,MAX_KEY_EVENTS,KEY_EVENT_RECORD);
        NTDOS64_HOST_EVENT_TRACE_STAGE(48);
        InitKeyHistory();
        NTDOS64_HOST_EVENT_TRACE_STAGE(49);
        }

    NTDOS64_HOST_EVENT_TRACE_STAGE(50);
    return;
}
...
VOID ReturnBiosBufferKeys(VOID)
{
    ...
    while (BufferHead != BufferTail)  {
         ...
         InputRecord[i].Event.KeyEvent.uChar.AsciiChar = AsciiChar;

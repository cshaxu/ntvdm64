...
int wmain(int argc, wchar_t **argv)
{
    ...
    int has_include = 0, has_exclude = 0, has_mutation_mode = 0, has_tick_budget = 0,
        has_bop_observation = 0, has_generic_ud_observation = 0, has_first_fault_observation = 0;
    uint32_t mutation_mode = BX_NTVDM_ENGINE_MUTATION_MODE_V1_DIRECT;
    uint64_t instruction_tick_budget = UINT64_C(1000000);
    int validate_only = 0, observe_bop_sequence = 0, observe_generic_ud_sequence = 0, observe_first_fault = 0;
    int index = 1;
    ...
    if (validate_only) {
        wprintf(L"ntdos64-native: request include=%08x exclude=%08x mode=%u budget=%llu observe-bop-sequence=%u observe-ud-sequence=%u observe-first-fault=%u\n",
    ...
    if (observe_first_fault) bx_ntvdm_mantle_first_fault_observation_enable(1);
    if (!ntdos64_console_cancellation_v1_begin(&cancellation_event)) {
        ...
        if (observe_first_fault) bx_ntvdm_mantle_first_fault_observation_enable(0);
        return 1;
    }
...
usage:
    fwprintf(stderr, L"usage: ntdos64-native --byob-profile profile.json --byob-root directory [--mutation-mode direct|readonly] [--instruction-tick-budget positive-decimal] [--observe-bop-sequence] [--observe-ud-sequence] [--observe-first-fault] [--include-drives c,d] [--exclude-drives e] [--validate-only] target [args...]\n");
    return 2;
}

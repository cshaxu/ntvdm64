    ...
    int argument_count, wchar_t *const *arguments)
{
    ...
    for (index = 0; index < argument_count; ++index) {
        if (arguments[index] == 0 || (index != 0 &&
            (used >= BYOB_LAUNCH_DECLARATION_V1_TAIL_BYTES ||
    ...
    wchar_t *tail, uint32_t tail_capacity)
{
    ...
    if (value == 0 || kind == 0 || tail == 0 || value->version != BYOB_LAUNCH_DECLARATION_V1_VERSION ||

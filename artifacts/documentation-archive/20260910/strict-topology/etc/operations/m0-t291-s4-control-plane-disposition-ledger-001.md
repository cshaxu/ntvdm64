...
| --- | --- | --- | --- |
| CP-01 | `cmdexec.c`, `cmdmisc.c`, `config.c`, `wow32/wkman.c` → `GetNextVDMCommand(VDMINFO)`; BaseSrv/CSR notification, capture and re-entry counting | Partial but separated: adapter-monitor exports the original entry through one typed, session-bound command provider; app owns one immediate single-session producer. No wait/wake queue, WOW caller or broker notification is enabled. | S4 P2 removes the generic session operation route and proves the typed provider/unavailable path. S5 owns the source-shaped broker/wake and multi-caller recovery admission. |
...
| CP-03 | Monitor/SoftPC `NtVdmControl` service classes such as start, interrupt, feature, LDT and printer control | Explicit unavailable. They have kernel-VDM/monitor/device owners and may not use CP-01 or CP-02. | S4 P3 maps each reached service to monitor, firmware/device, DPMI or VDD owner. |

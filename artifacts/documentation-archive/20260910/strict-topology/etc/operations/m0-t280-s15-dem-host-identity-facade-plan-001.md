...
reverse-looks-up and releases opaque native values. It returns a fixed-width
surrogate, never a host pointer, and does not acquire guest memory, close
resources, select a DEM service, or interpret an `FFindId`.
...
publish/resolve and reverse-lookup identity, release/stale failure, distinct
session isolation, and disposal invalidation. The original `demsrch.c` body
is not modified in this S; that follow-on must record its local DIVERGENCE
markers separately.

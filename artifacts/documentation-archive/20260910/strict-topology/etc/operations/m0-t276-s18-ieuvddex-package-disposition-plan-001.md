...
Audit every selected `ieuvddex` path as one original IEU/VDD debugger-extension
package.
...
source-shaped debugger/monitor contracts, and assigns each reached external
family to `adapter-debugger`, `adapter-vdm-monitor`, the session mapping
manager, `adapter-softpc`, `adapter-bochs` or a separately named product
boundary.

...
3. Add the one-session callback transaction in the existing WOW adapter
   family.  P4 retains original `CBVDMFRAME` construction, scoped write/read,
   AX:DX return and task-stack restoration. P5 binds the original
   `SETVDMSTACK` / `host_simulate` / `VDMSTACK` interval to typed
   SoftPC/Bochs mechanics. This path is explicitly independent of the
   session command/control route: `GetNextVDMCommand` callers use that route,
   while `CallBack16` preserves original synchronous guest re-entry.

...
when the owner selects it and admits it to [the current status record](../../../states/CURRENT.md). The active
...
| 4 | Canonical original-package static and overlay closure | From the fixed file/interface selections, finish the selected `mvdm-host` union and each admitted non-MVDM original slice as exact static mirror/build manifests. For every non-byte-exact source, prove the true-subset or same-shaped-minimal-difference classification, README registration, and that any material mechanism is private to the matching `*-overlay` rather than an adapter or autonomous replacement. It proves package membership and finite link inputs only; no provider route or adapter body is enabled. | Final first-frontier file, package, interface, build and divergence rows, plus per-mirror/overlay ownership checks. |
...
or a per-BOP implementation backlog here; use `states/CURRENT.md` for the admitted

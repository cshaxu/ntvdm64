...
| --- | --- | --- | --- | --- |
| BX-TRACE-092 | 2026-08-23: T249 S1 proves existing selector-blind records cannot attribute a normal CPU write into the source-built low NTIO aperture; standing owner authorization covers the smallest necessary mechanical exception | Adopted `src/bx-core/memory/memory.cc` and `src/bx-core/cpu/cpu.h`; project-owned `src/bx-mantle/bx_ntvdm_physical_write_observation_v1.*`; diagnostic graph/fixture only | Default-off fixed-range observation is reached from both the existing physical-write slow path and the existing selector-blind `BX_NOTIFY_LIN_MEMORY_ACCESS` CPU fast path. The core forwards only physical address, write length, immediate input bytes and copied CPU position fields (CS selector/base, RIP, SS:SP); mantle independently range-matches and retains at most the first fixed-width event. | Registered before patch. No BOP, selector, DOS, OpenNT, NTIO, service, guest-address label, host capability, adapter callback, pointer retention, write filtering, memory handler alteration, CPU/register/flag mutation, stop request, device/firmware feature, default product behavior or unbounded trace is permitted. Disabled macro emits no call; configured non-overlap writes have identical memory effects. Acceptance requires disabled, invalid, non-overlap, overlap/first-event, reset and ordinary-RAM write-effect tests plus one bounded native checkpoint. Reject/remove if it grows beyond a caller-supplied fixed physical range or alters the original write result. |
| BX-MANTLE-093 | 2026-08-23: T253 S3 initially considered a core async-event hook, then rejected it after source audit proved existing `pc_system` timer callbacks run on the native CPU thread. This is project-authored mantle composition, not an adopted-Bochs intrusion. | Project-owned `src/bx-mantle` fixed physical-IRQ publication queue only; no `src/bx-core` file changes. | A host worker atomically publishes only a physical line `0..15`; the existing finite machine-stage timer drains it on the CPU thread and calls the existing mantle PIC request. Native PIC/CPU retain mask, cascade, acknowledgement and INTR behavior. | No BOP, selector, DOS, OpenNT, Redirector, vector-purpose, callback, guest address, handle, worker, host API, provider or CLI term enters bx-core or the physical-IRQ ABI. No direct vector injection, IVT access, CPU/register/flag mutation, altered PIC priority/mask behavior, device configuration or default behavior is allowed. The inactive/reset queue rejects or clears publications. Require producer, masked/deferred, normal PIC acknowledgement and lifecycle-reset tests. Reject any future need for a bx-core hook until separately registered. |
| BX-ABI-094 | 2026-08-23: T255 S1 proves original DPMI mode-transition and IRET bodies need a generic complete context result; the standing source-first authority permits the smallest extension of the existing #UD seam. | Adopted `src/bx-core/cpu/exception.cc`; project-owned fixed-width generic bridge header and result ABI only. | Versioned generic context-resume result: a mode target limited to unchanged/real/protected, complete 32-bit GPR/FLAGS masks and segment selector values. Core applies accepted values only through native `SetCR0`, register/flag setters and `writeEFlags`. Data/stack selectors use existing `load_seg_reg`; CS uses existing `jump_protected` after the native mode change because the upstream segment loader intentionally rejects protected-mode CS. No descriptor cache is copied or reimplemented. | Registered before patch. No BOP, selector, service, DPMI, DOS, OpenNT, LDT, guest address, host pointer, memory action, allocation policy, device, firmware, adapter callback, host capability or CLI term may enter bx-core. Existing v1 outcome and all real-mode behavior must remain bit-for-bit unchanged when no v2 callback accepts. S2 requires focused real→protected, protected→real, rejected invalid-mode and disabled/v1 compatibility fixtures. Reject if a descriptor cache is copied, an arbitrary CR0 value is accepted, a raw Bochs object crosses the boundary, or any provider-specific branch is added. |
| BX-MANTLE-095 | 2026-08-23: T256 S2 maps a no-fault, non-paged protected segment access composition; standing source-first authority permits a project-owned mantle action but no adopted core change. | Project-owned `src/bx-mantle/bx_ntvdm_protected_range_action_v1.{h,cc}` only; no `src/bx-core` or imported OpenNT file change. | A fixed-width synchronous record requests bounded copied read/write using the current native segment cache. While minimal-machine lifecycle is active, the mantle rejects every mode except protected paging-disabled; delegates segment checks to existing native `read_virtual_checks`/`write_virtual_checks`; forms address via `get_laddr32`; and delegates bytes to existing complete-span ordinary-RAM copied actions. | Registered before patch. No BOP, selector/service byte, DPMI, DOS, OpenNT, host API/capability, raw linear/physical/host pointer, Bochs object, descriptor cache/table change, page-table walk, `CR0`/register/FLAGS/interrupt mutation, device/ROM/handler access, callback, mapping allocation or product behavior is permitted. Reject zero/oversized/invalid records, inactive/real/V86/paging modes, non-ordinary/A20-wrapped spans and all check failures without guest fault delivery. Require focused inactive/real/paged/malformed/limit/A20/handler rejection, read/write and cross-page atomicity tests plus source scan. Reject if paging support, a debug walker, a core patch, an unbounded API or any provider term becomes necessary. |
| BX-MANTLE-096 | 2026-08-23: T256 S5 proves the runnable engine/stage is fixed at 1 MiB, while retained DPMI XMEM cannot obtain a safe backing interval without an explicit machine capacity. Standing authority admits the smallest project-owned copied capacity field before code. | Project-owned bx-mantle stage request/initialization, bx-vdm engine request and CLI parser only; no `src/bx-core` or imported OpenNT source change. | A fixed-width session `guest_memory_bytes` value in [1 MiB, 16 MiB], 64 KiB aligned, selects only the amount of native ordinary RAM constructed at stage entry. Default remains 1 MiB. Mantle receives no BOP, selector, DPMI, DOS, OpenNT, reservation, mapping, provider, host capability or pointer term. | Registered before patch. No allocator, reserved interval, host allocation/pointer, memory handler/ROM/device change, descriptor/LDT/page-table/CR0/register/flag mutation, direct core change or default capacity change is permitted. Require default, 2 MiB ordinary-RAM, malformed range, stage-reset and engine/CLI contract tests. Reject if this field begins to encode DPMI policy or if a low-memory shim is bulk-relaxed without individual audit. |
| BX-MANTLE-097 | 2026-08-23: T256 S7 proves capacity cannot safely imply allocation ownership; standing authority admits the minimum project-owned lifecycle record over an explicit configured ordinary-RAM interval. | Project-owned `src/bx-mantle/bx_ntvdm_ordinary_ram_reservation_v1.*`, stage/engine copied configuration and CLI parsing only; no `src/bx-core` or imported OpenNT source change. | A zero-default copied `[base, bytes]` interval is validated at stage admission. While the minimal machine is active, a bounded fixed table may first-fit allocate/release copied 32-bit addresses with opaque non-pointer IDs. It preflights the complete interval and every returned span with existing ordinary-RAM access predicates; reset clears configuration and every record. | Registered before patch. No BOP, selector, DPMI, DOS, OpenNT, xms, provider, host API/capability/allocation, raw host pointer, memory handler/ROM/device change, paging/LDT/CR0/register/flag mutation, automatic high-memory interval, adopted-core change or default behavior is permitted. Require zero/default rejection, aligned interval, malformed/out-of-capacity rejection, allocate/release/reuse, backing-RAM and reset tests. Reject if an opaque ID becomes a pointer or if any provider term enters mantle. |
| BX-MANTLE-091 | 2026-08-22: owner has standing queue authority for T244 S2, while requiring registered, minimum-necessary Bochs changes | Adopted Bochs 2.6 `src/bx-mantle/keyboard.{cc,h}` plus a mantle-private factory/lifecycle; no bx-core source | Sever exactly the upstream 8042 controller's plugin entry, GUI status items, paste path, mouse/CMOS registration and runtime parameter handlers so one native controller registers only ports `60h/64h`, uses the existing native timer primitive and raises ordinary IRQ1 through the existing PIC. | **Lifecycle repair admitted before patch, T248 S3.** Source inspection shows the prior headless destroy directly deleted an object whose original `init()` had registered four port handlers and a timer. The repair may add only a controller-local `fini()` that deactivates/unregisters that owned timer and unregisters the exact original byte handlers at `60h/64h`, then invokes it before delete. It may neither alter controller behavior while live nor add BOP/selector/DOS/OpenNT/SoftPC terms, host input, guest data, plugins, GUI, CMOS, mouse, PIT, video, firmware, adapter callback, host capability or bx-core changes. Acceptance requires focused repeated lifecycle/port-negative coverage and the T248 terminal-position regression. |
...
| BX-MANTLE-083 | 2026-08-17: T225 S23 P5 proves that the source-derived original INT 06 operation transcript needs synchronous native byte-port execution, while bx-vdm must not obtain a Bochs object/callback | Project-authored `src/bx-mantle/bx_ntvdm_port_action_v1.h/.cc` and only the existing minimal-machine lifecycle gate | Expose versioned copied byte port read/write requests; accept only while the established minimal machine owns initialized native port space **and the exact port is non-default registered**; delegate accepted requests exactly to existing `bx_devices.inp/outp`; return only copied success/value. | **T248 S3 lifecycle correction admitted before patch.** The wrapper previously reported an initialized but default/unmapped port as success, contradicting its focused inactive-port contract. It may compare only the existing port-array slot with the existing default read/write sentinel and reject it; core `inp/outp` retains original unmapped all-ones/ignored behavior. No selector/BOP/OpenNT/DOS term, device registration, firmware, callback, object pointer, generic device API, host capability or Bochs-source patch is permitted. The focused native lifecycle fixture must prove inactive rejection, native PIC dispatch, copied read result, default-slot rejection and post-cleanup rejection. |
...
| BX-IO-029 | 2026-08-11: coordinator selects the lowest-risk S6 retained-owner candidate | `src/bochs-core/iodev/devices.cc`; private `src/bochs-core-overlay/iodev/minimal_port_space.cc` | Source-object separation of the complete already registered empty-port lifecycle: initializer, default handlers, registration helpers and cleanup | T270/S1 re-homed the source object under the matching private overlay; no ABI, behavior, feature, lifecycle, or runtime change. |
| BX-IO-030 | 2026-08-11: S6 single-object observation proves the isolated empty-port initializer still imports only its original default-handler registration members from the rejected full device object | `src/bochs-core/iodev/devices.cc`; private `src/bochs-core-overlay/iodev/minimal_port_space.cc` | Source-object separation of the two original default I/O-handler registration members used exclusively by the isolated empty-port initializer | T270/S1 re-homed the source object under the matching private overlay; no ABI, behavior, feature, lifecycle, or runtime change. |
...
delete/reset a non-default handler. **T248 S3 accessor repair, admitted before
patch:** one boolean, mantle-private query may compare an exact port-array
slot to its existing default sentinel, so the typed port-action ABI can reject
an unmapped request without changing native `inp/outp` behavior.

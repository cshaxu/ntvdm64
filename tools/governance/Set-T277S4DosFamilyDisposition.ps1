...
function Family($symbol) {
    switch ($symbol) {
        'CMDInit' { return @('COMMAND-package-initialization','opennt-mvdm-host COMMAND package','original BOOL CMDInit(int,char**) call/return form','none','retain caller-visible BOOL failure; package body is not enabled','whole COMMAND package must close before enablement') }
        ...
        'nt_block_event_thread' { return @('COMMAND-session-event-lifecycle','session -> adapter-win32; adapter-vdd successor for original VDD hook','original void block call and BlockFlags ordering','session-owned event identity; no host HANDLE enters guest state','preserve source ordering; private VDD callback is a named later adapter-vdd branch','no CSRSS/NT4 event-thread recreation') }
        'nt_resume_event_thread' { return @('COMMAND-session-event-lifecycle','session -> adapter-win32; adapter-vdd successor for original VDD hook','original void resume call and stream-I/O restoration ordering','session-owned event identity; no host HANDLE enters guest state','preserve source ordering; private VDD callback is a named later adapter-vdd branch','no CSRSS/NT4 event-thread recreation') }
        'nt_std_handle_notification' { return @('COMMAND-session-event-lifecycle','session -> adapter-win32','original BOOL redirection notification, including stdoutRedirected state','session host_resource mapping instance owns any host standard-stream identity','record redirection state; X86GFX/full-screen branch is a named display successor','no process-global SetStdHandle mutation') }
        'cmdPushExitInConsoleBuffer' { return @('COMMAND-session-event-lifecycle','session -> adapter-win32','original void console-exit notification call','session-local completion notification; no console HANDLE enters guest state','non-WOW local notification may be recovered; WOW/console injection is an adapter-wow successor','no private console-group or CSRSS recreation') }
        'cmdPipeFileDataEOF' { return @('COMMAND-local-pipe-observation','opennt-mvdm-host COMMAND provider -> session -> adapter-win32','original BOOL(HANDLE,BOOL*) data/EOF query and timeout ordering','session host_resource mapping instance resolves the opaque host stream endpoint','retain TRUE/new-data-or-EOF and fEOF behavior; brokered pipe protocol belongs to adapter-redir','no guest-visible host HANDLE or new pipe protocol') }
        'cmdPipeFileEOF' { return @('COMMAND-local-pipe-observation','opennt-mvdm-host COMMAND provider -> session -> adapter-win32','original BOOL(HANDLE) EOF query and timeout ordering','session host_resource mapping instance resolves the opaque host stream endpoint','retain EOF result; brokered pipe protocol belongs to adapter-redir','no guest-visible host HANDLE or new pipe protocol') }
        'host_direct_access_error' { return @('top-level-direct-device-error','opennt-mvdm-host nt_error -> adapter-win32 -> session','original void reason-coded direct-access error call','no guest pointer or raw process-control state crosses the boundary','preserve reason and per-thread duplicate suppression; Ignore resumes caller path, Terminate requests controlled stop','WOW hard-error and old console placement remain named later branches') }
        'host_lpt_flush_initialize' { return @('host-device-state-cleanup','opennt-mvdm-host LPT provider -> adapter-win32','original void LPT flush-state initialization','session-owned host-resource state only','preserve original no-result initialization ordering; unavailable host port is explicit provider failure','no Bochs device or DOS semantic in adapter-win32') }
        ...
        'HostFdiskReset' { return @('host-device-state-cleanup','opennt-mvdm-host fixed-disk provider -> adapter-win32','original void fixed-disk reset call','session host_resource state only','preserve original reset ordering; unavailable host device remains explicit failure','no direct physical I/O or Bochs disk semantic in caller') }
        'HostFloppyReset' { return @('host-device-state-cleanup','opennt-mvdm-host floppy provider -> adapter-win32','original void floppy reset call','session host_resource state only','preserve original reset ordering; unavailable host device remains explicit failure','no direct physical I/O or Bochs floppy semantic in caller') }
        'HostTerminatePDB' { return @('host-device-state-cleanup','opennt-mvdm-host disk/floppy providers -> session','original void USHORT PDB cleanup call','PDB is a guest value; any host resource lookup uses session host_resource mapping only','preserve original cleanup ordering; missing owner record is a no-resource cleanup result','no raw host handle encoded in PDB') }
        default { throw "Unexpected DOS S4 symbol '$symbol'." }
    }
}

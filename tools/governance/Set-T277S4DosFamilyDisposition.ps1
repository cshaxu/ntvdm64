...
function Family($symbol) {
    switch ($symbol) {
        ...
        'cmdPipeFileDataEOF' { return @('COMMAND-local-pipe-observation','mvdm-host COMMAND provider -> session -> adapter-win32','original BOOL(HANDLE,BOOL*) data/EOF query and timeout ordering','session host_resource mapping instance resolves the opaque host stream endpoint','retain TRUE/new-data-or-EOF and fEOF behavior; brokered pipe protocol belongs to adapter-redir','no guest-visible host HANDLE or new pipe protocol') }
        'cmdPipeFileEOF' { return @('COMMAND-local-pipe-observation','mvdm-host COMMAND provider -> session -> adapter-win32','original BOOL(HANDLE) EOF query and timeout ordering','session host_resource mapping instance resolves the opaque host stream endpoint','retain EOF result; brokered pipe protocol belongs to adapter-redir','no guest-visible host HANDLE or new pipe protocol') }
        ...
        'host_lpt_flush_initialize' { return @('host-device-state-cleanup','mvdm-host LPT provider -> adapter-win32','original void LPT flush-state initialization','session-owned host-resource state only','preserve original no-result initialization ordering; unavailable host port is explicit provider failure','no Bochs device or DOS semantic in adapter-win32') }
        ...
        'HostFdiskReset' { return @('host-device-state-cleanup','mvdm-host fixed-disk provider -> adapter-win32','original void fixed-disk reset call','session host_resource state only','preserve original reset ordering; unavailable host device remains explicit failure','no direct physical I/O or Bochs disk semantic in caller') }
        'HostFloppyReset' { return @('host-device-state-cleanup','mvdm-host floppy provider -> adapter-win32','original void floppy reset call','session host_resource state only','preserve original reset ordering; unavailable host device remains explicit failure','no direct physical I/O or Bochs floppy semantic in caller') }

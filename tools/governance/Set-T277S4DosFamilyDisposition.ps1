...
function Family($symbol) {
    switch ($symbol) {
        ...
        'nt_block_event_thread' { return @('COMMAND-session-event-lifecycle','session -> adapter-mvdm-host-out/win32; adapter-mvdm-host-out/vdd successor for original VDD hook','original void block call and BlockFlags ordering','session-owned event identity; no host HANDLE enters guest state','preserve source ordering; private VDD callback is a named later adapter-mvdm-host-out/vdd branch','no CSRSS/NT4 event-thread recreation') }
        'nt_resume_event_thread' { return @('COMMAND-session-event-lifecycle','session -> adapter-mvdm-host-out/win32; adapter-mvdm-host-out/vdd successor for original VDD hook','original void resume call and stream-I/O restoration ordering','session-owned event identity; no host HANDLE enters guest state','preserve source ordering; private VDD callback is a named later adapter-mvdm-host-out/vdd branch','no CSRSS/NT4 event-thread recreation') }
        ...
        'cmdPushExitInConsoleBuffer' { return @('COMMAND-session-event-lifecycle','session -> adapter-mvdm-host-out/win32','original void console-exit notification call','session-local completion notification; no console HANDLE enters guest state','non-WOW local notification may be recovered; WOW/console injection is an adapter-mvdm-host-out/wow successor','no private console-group or CSRSS recreation') }
        'cmdPipeFileDataEOF' { return @('COMMAND-local-pipe-observation','mvdm-host COMMAND provider -> session -> adapter-mvdm-host-out/win32','original BOOL(HANDLE,BOOL*) data/EOF query and timeout ordering','session host_resource mapping instance resolves the opaque host stream endpoint','retain TRUE/new-data-or-EOF and fEOF behavior; brokered pipe protocol belongs to adapter-mvdm-host-out/redir','no guest-visible host HANDLE or new pipe protocol') }
        'cmdPipeFileEOF' { return @('COMMAND-local-pipe-observation','mvdm-host COMMAND provider -> session -> adapter-mvdm-host-out/win32','original BOOL(HANDLE) EOF query and timeout ordering','session host_resource mapping instance resolves the opaque host stream endpoint','retain EOF result; brokered pipe protocol belongs to adapter-mvdm-host-out/redir','no guest-visible host HANDLE or new pipe protocol') }
        ...
        'host_simulate' { return @('bounded-machine-run','adapter-mvdm-host-out/softpc -> adapter-bochs','original host_simulate call shape','S3 typed machine request; no pointer mapping','use S3 caller-return-path failure disposition','Bochs remains sole executor') }

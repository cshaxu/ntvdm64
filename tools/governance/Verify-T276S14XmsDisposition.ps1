...
if (@($rows | Where-Object { $_.source_path -eq 'xms.486/i386/xmsmem86.c' -and $_.provisional_disposition -eq 'architecture-specific direct-pointer exclusion' }).Count -ne 1) { throw 'The x86 direct-pointer routine must remain explicitly excluded.' }
...
Write-Output 'T276 S14 XMS disposition verification passed: 15 paths with direct-pointer and UMB gates.'

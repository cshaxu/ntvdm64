...
foreach ($provider in $active) {
    ...
    if ($provider.original_subdomain -in $overlayDomains) {
        ...
        $row.final_owner_or_link_boundary = 'mvdm-host original WOW32 provider with adapter-mvdm-host-out/wow overlay -> adapter-mvdm-host-out/win32, adapter-mvdm-host-out/softpc and adapter-mvdm-host-out/monitor as reached; session owns mapping instances'
        ...
        $row.named_adapter = 'adapter-mvdm-host-out/wow; adapter-mvdm-host-out/win32; adapter-mvdm-host-out/softpc; adapter-mvdm-host-out/monitor; session guest-memory, host-resource and completion-callback mapping managers'
    ...
    elseif ($provider.original_subdomain -in $bindingDomains) {
        ...
        $row.final_owner_or_link_boundary = 'mvdm-host original WOW32 provider -> adapter-mvdm-host-out/wow and adapter-mvdm-host-out/win32 public/same-shaped capability bindings; session owns identity mapping'
        $row.final_change_class = 'binding-only; preserve original provider dispatch, structures and failure order'
        $row.named_adapter = 'adapter-mvdm-host-out/wow; adapter-mvdm-host-out/win32; session host-resource mapping manager'

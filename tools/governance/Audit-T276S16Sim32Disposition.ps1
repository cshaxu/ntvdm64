...
$contracts = @{
    'sim32/sim32.c' = @('historical-wow-serial-monitor-transport', 'adapter-mvdm-host-out/wow; adapter-mvdm-host-out/monitor; session guest-memory mapping manager; adapter-mvdm-host-out/softpc; adapter-bochs', 'original packet transport provides Sim32 read/write/PSZ/free and wakeup forms through a private NT serial device and 16-bit SIM16 peer; preserve source/API evidence, but replace only its SIM32 public contract through the unique mapping manager when a source-proven caller is admitted', 'historical-product-shell exclusion; API-form recovery only')
    'sim32/sim32.h' = @('sim32-public-monitor-api', 'adapter-mvdm-host-out/monitor; session guest-memory mapping manager; adapter-mvdm-host-out/softpc; adapter-bochs; adapter-mvdm-host-out/wow', 'original Sim32Get/Set/PSZ/Free/Send declarations and convenience macros; same API form may use checked bounce leases, never raw guest pointers', 'declaration and API-form recovery prerequisite')
...
$result = foreach ($file in $files) {
    ...
    $family=if ($file.target_path -eq 'sim32/sim32.c') {'adapter-mvdm-host-out/wow (private serial/WOW peer); adapter-mvdm-host-out/monitor (SIM32 public API form); session guest-memory mapping manager (checked 32-bit guest address resolution); adapter-bochs (only machine memory/device mechanics)'} elseif ($file.target_path -eq 'sim32/sim32.h') {'adapter-mvdm-host-out/monitor; session guest-memory mapping manager; adapter-mvdm-host-out/softpc'} else {'none (build metadata)'}

...
$sourceContracts = @{
    ...
    'dos/dem/demgset.c'      = @('dem-drive-system-state', 'adapter-win32; adapter-softpc; session', 'original drive, date/time and system-state query/set provider')
    'dos/dem/demhndl.c'      = @('dem-handle-lifecycle', 'adapter-win32; adapter-softpc; session', 'original DOS-handle lifecycle provider; opaque host identity mapping remains session-owned')
    'dos/dem/demioctl.c'     = @('dem-device-ioctl', 'adapter-softpc; adapter-bochs; adapter-win32; session', 'original IOCTL/provider path; raw device and BIOS-style branches require machine-media ownership proof')
    'dos/dem/demlabel.c'     = @('dem-volume-label', 'adapter-win32; adapter-softpc; session', 'original volume-label provider and DOS error mapping')
    'dos/dem/demlock.c'      = @('dem-file-locking', 'adapter-win32; adapter-softpc; session', 'original DOS file-lock provider; public host locking binding must retain source failure behavior')
    'dos/dem/demmisc.c'      = @('dem-miscellaneous-state', 'adapter-softpc; adapter-win32; session', 'original miscellaneous DEM state and guest/host conversion helpers')
    'dos/dem/demmsg.c'       = @('dem-message-catalog', 'adapter-win32; session', 'original DEM message/resource and diagnostic helper path')
    'dos/dem/demsrch.c'      = @('dem-search-namespace', 'adapter-win32; adapter-softpc; session', 'original find-first/find-next namespace provider; guest DTA and host search state require declared mappings')
}

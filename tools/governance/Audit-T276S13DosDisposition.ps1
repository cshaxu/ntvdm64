...
$sourceContracts = @{
    'dos/command/cmd.c'      = @('command-service-entry', 'adapter-mvdm-host-in; adapter-softpc; adapter-mvdm-host-out/win32; session', 'original COMMAND service entry and result contract; preserve dispatch shape before individual services are admitted')
    'dos/command/cmdconf.c'  = @('command-bootstrap-configuration', 'adapter-softpc; adapter-mvdm-host-out/win32; session; mvdm-support', 'original CONFIG.SYS/AUTOEXEC bootstrap and configuration transfer; retain the source-owned ordering')
    ...
    'dos/command/cmdexec.c'  = @('command-child-lifecycle', 'session; adapter-mvdm-host-out/win32; adapter-softpc', 'original COMMAND child creation, wait and re-entry control flow; requires the session lifecycle contract, not a replacement command runner')
    ...
    'dos/command/cmdmisc.c'  = @('command-bootstrap-console', 'session; adapter-mvdm-host-out/win32; adapter-softpc; mvdm-support', 'original startup, console and command-information orchestration; it is a cross-provider composition point')
    ...
    'dos/command/cmdredir.c' = @('command-redirection-pipes', 'adapter-redir; adapter-mvdm-host-out/win32; session; adapter-softpc', 'original standard-stream redirection and pipe control flow; Redirector ownership is a prerequisite')
    'dos/dem/dem.c'          = @('dem-service-entry', 'adapter-mvdm-host-in; adapter-softpc; adapter-mvdm-host-out/win32; session', 'original DEM entry/configuration contract; retain source-owned service semantics')
    'dos/dem/demdasd.c'      = @('dem-raw-dasd', 'adapter-softpc; adapter-bochs; adapter-mvdm-host-out/win32; session', 'original absolute disk/floppy control path; machine-media ownership must be proved before enablement')
    ...
    'dos/dem/demdir.c'       = @('dem-directory-namespace', 'adapter-mvdm-host-out/win32; adapter-softpc; session', 'original directory/current-drive provider; public host path binding must preserve DOS-facing error order')
    ...
    'dos/dem/demerror.c'     = @('dem-hard-error-state', 'adapter-softpc; adapter-mvdm-host-out/win32; session', 'original hard-error state, guest register result and user-visible error ordering')
    'dos/dem/demfcb.c'       = @('dem-fcb-filesystem', 'adapter-mvdm-host-out/win32; adapter-softpc; session', 'original FCB filesystem provider; host handles and guest memory require their dedicated same-shaped adapters')
    'dos/dem/demfile.c'      = @('dem-handle-filesystem', 'adapter-mvdm-host-out/win32; adapter-softpc; adapter-redir; adapter-vdd; session', 'original DOS handle filesystem, pipe and VDD-aware provider; each external bridge remains separately owned')
    'dos/dem/demgset.c'      = @('dem-drive-system-state', 'adapter-mvdm-host-out/win32; adapter-softpc; session', 'original drive, date/time and system-state query/set provider')
    'dos/dem/demhndl.c'      = @('dem-handle-lifecycle', 'adapter-mvdm-host-out/win32; adapter-softpc; session', 'original DOS-handle lifecycle provider; opaque host identity mapping remains session-owned')
    'dos/dem/demioctl.c'     = @('dem-device-ioctl', 'adapter-softpc; adapter-bochs; adapter-mvdm-host-out/win32; session', 'original IOCTL/provider path; raw device and BIOS-style branches require machine-media ownership proof')
    'dos/dem/demlabel.c'     = @('dem-volume-label', 'adapter-mvdm-host-out/win32; adapter-softpc; session', 'original volume-label provider and DOS error mapping')
    'dos/dem/demlock.c'      = @('dem-file-locking', 'adapter-mvdm-host-out/win32; adapter-softpc; session', 'original DOS file-lock provider; public host locking binding must retain source failure behavior')
    'dos/dem/demmisc.c'      = @('dem-miscellaneous-state', 'adapter-softpc; adapter-mvdm-host-out/win32; session', 'original miscellaneous DEM state and guest/host conversion helpers')
    ...
    'dos/dem/demsrch.c'      = @('dem-search-namespace', 'adapter-mvdm-host-out/win32; adapter-softpc; session', 'original find-first/find-next namespace provider; guest DTA and host search state require declared mappings')
}
...
function Get-NonSourceContract {
    ...
    if ($Path -like 'dos/command/*') {
        return @('command-public-declaration', 'mvdm-host; adapter-mvdm-host-in; adapter-softpc; adapter-mvdm-host-out/win32; session', 'original COMMAND declaration/data surface; preserve exact source shape before provider enablement', 'declaration-only')
    }
    return @('dem-public-declaration', 'mvdm-host; adapter-mvdm-host-in; adapter-softpc; adapter-mvdm-host-out/win32; session', 'original DEM declaration/data surface; preserve exact source shape before provider enablement', 'declaration-only')
}
...
function Get-LexicalFamilies {
    ...
    if ($text -match '(?i)(softpc\.h|\bSim32|\bsas_|\bget[A-Z]|\bset[A-Z]|host_simulate)') { $families.Add('adapter-softpc (SoftPC/SAS/guest-state lexical surface)') }

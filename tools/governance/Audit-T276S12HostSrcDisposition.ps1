...
$groups = [ordered]@{
    ...
    'adapter-win32-host-capability' = @('fprt.c','nt_error.c','nt_keycd.c','nt_nls.c','nt_ntfun.c','nt_pif.c','nt_unix.c')
...
function Get-Contract {
    param([string]$Group)
    switch ($Group) {
        'opennt-bop-dispatch' { return @('adapter-mvdm-host-in; adapter-softpc; adapter-win32; specialist adapters', 'original BOP selector/host dispatch; preserve original routing shape, evaluate each reached historical boundary', 'adapter-bound review') }
        ...
        'cross-owner-startup-composition' { return @('app; session; adapter-softpc; adapter-win32; adapter-vdd; mvdm-host owner packages', 'original NTVDM startup/control composition reaches COMMAND, XMS, debugger/VDD, machine and host APIs; preserve it as a cross-owner composition nexus and do not misclassify it as a standalone monitor provider', 'composition-prerequisite review') }
        ...
        'cross-owner-launch-configuration' { return @('app; session; adapter-win32; adapter-softpc; adapter-vdm-monitor; mvdm-host owner packages', 'original launch/configuration composition crosses PIF, console, host environment, monitor and machine setup; retain exact mirror and do not classify it as a standalone Win32 provider', 'composition-prerequisite review') }
        'adapter-win32-host-capability' { return @('adapter-win32; session; app; adapter-debugger as individually proved', 'original host capability/NTDLL/Win32 source; preserve algorithm and error ordering while auditing each public-modern binding and its session/product owner', 'adapter-bound review') }
        'cross-owner-video-machine-initialization' { return @('adapter-vdm-monitor; adapter-softpc; adapter-bochs; adapter-win32', 'original video-memory and ROM-mapping initialization crosses monitor, machine and virtual-memory boundaries; retain exact mirror and do not classify it as a standalone Win32 provider', 'composition-prerequisite review') }
        ...
        'machine-device-bridge-review' { return @('adapter-softpc; adapter-bochs; adapter-win32 as individually proved', 'mixed original device bridge; determine Bochs-native replacement versus same-shaped host-device binding before enablement', 'device-bound review') }

...
foreach ($relative in @(
    'artifacts/documentation-archive/20260910/etc/operations/ledgers/m0-t280-s10-dem-ntioapi-abi-closure-plan-001.md',
    'artifacts/documentation-archive/20260910/etc/operations/ledgers/m0-t280-s10-dem-ntioapi-reached-abi-ledger.tsv',
    'artifacts/documentation-archive/20260910/etc/operations/ledgers/m0-t280-s10-dem-ntioapi-abi-evidence-001.md',
    'artifacts/documentation-archive/20260910/etc/operations/ledgers/m0-t280-s10-dem-ntioapi-implementation-plan-001.md',
    'src/mvdm-platform-abi/source/public/sdk/inc/ntioapi.h'
)) {
    if (-not (Test-Path -LiteralPath (Join-Path $root $relative) -PathType Leaf)) { throw "Missing T280 S10 artifact: $relative" }
}
$rows = Import-Csv -LiteralPath (Join-Path $root 'artifacts/documentation-archive/20260910/etc/operations/ledgers/m0-t280-s10-dem-ntioapi-reached-abi-ledger.tsv') -Delimiter "`t"
...
foreach ($token in @('NtQueryDirectoryFile','NtQueryVolumeInformationFile','NtOpenSymbolicLinkObject','FILE_BOTH_DIR_INFORMATION','FILE_FS_DEVICE_INFORMATION')) {
 ...
 $implementation = Get-Content -Raw -LiteralPath (Join-Path $root 'artifacts/documentation-archive/20260910/etc/operations/ledgers/m0-t280-s10-dem-ntioapi-implementation-plan-001.md')

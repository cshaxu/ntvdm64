...
const all = new Map();
for (const row of rootsRows) all.set(identity(row), { ...row, membership_origin: 'selected-mvdm-host-root' });
for (const row of admissionRows) {
  const key = identity(row);
  if (all.has(key)) throw new Error(`rebase identity already present in root closure: ${key}`);
  all.set(key, { ...row, membership_origin: row.rebase_admission });
}
const rows = [];
for (const row of [...all.values()].sort((left, right) => left.source_root.localeCompare(right.source_root) || left.source_path.localeCompare(right.source_path) || Number(left.source_line) - Number(right.source_line))) {
  ...
  const isRoot = row.membership_origin === 'selected-mvdm-host-root' && oldOrigin === 'mvdm-host-root';
  ...
  const admissionLegal = isRoot || allInbound.length > 0;
...
write('mvdm-zero-degree-legality-ledger.tsv', rows, columns);
write('mvdm-zero-degree-legality-exception-ledger.tsv', rows.filter((row) => row.legality !== 'structurally-valid' && row.legality !== 'structurally-valid-conditional-context-recorded'), columns);
const count = (predicate) => rows.filter(predicate).length;
console.log(`zero identities=${rows.length}; structural unconditional=${count((row) => row.legality === 'structurally-valid')}; structural conditional=${count((row) => row.legality === 'structurally-valid-conditional-context-recorded')}; exceptions=${count((row) => row.legality === 'requires-correction-or-variant-proof')}; roots=${count((row) => row.membership_origin === 'selected-mvdm-host-root')}; rebased=${count((row) => row.membership_origin !== 'selected-mvdm-host-root')}`);

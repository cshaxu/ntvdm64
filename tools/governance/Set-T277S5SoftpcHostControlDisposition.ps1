...
function Disposition($family) {
    switch ($family) {
        ...
        'COMMAND-package-entry' { return @('opennt-mvdm-host COMMAND package','original BOOL CMDInit(int,char**) call/return','none','retain original BOOL failure result; whole COMMAND package required','no startup product shell') }
        'session-stop-and-event' { return @('session -> adapter-win32; adapter-vdd successor where original hook is reached','original void event/notification/termination calls and source ordering','session-owned event/resource identity only','controlled stop never exits the surrounding app; VDD/CSRSS product branches remain explicit successors','no private event thread or process-global standard handles') }
        'BOP-dispatch-control' { return @('adapter-bop -> opennt-mvdm-host; adapter-vdd successor for MS_bop_5','original BOP entry shape and original dispatcher ordering','fixed-width CPU frame and checked guest-memory copy only','retain original unavailable/decline/controlled-stop outcome; no selector logic in Bochs','no second BOP dispatcher or trace-selected body enablement') }
        'machine-CPU-SAS-PIC-facade' { return @('adapter-softpc -> adapter-bochs','same-named historical CPU/SAS/PIC request form','session guest_memory lease for ranges; no raw pointer or parallel CPU state','typed mechanical failure is returned at the original caller boundary; Bochs remains sole executor','no CCPU/CVIDC/base device executor link') }
        'serial-host-endpoint' { return @('opennt-mvdm-host serial provider -> adapter-win32 and adapter-softpc -> adapter-bochs','original serial function parameters and BOOL/void result forms','session host_resource mapping owns endpoint identity; guest values stay fixed-width','preserve original result/timeout/error path; unavailable endpoint is explicit','no host HANDLE or COM executor crosses into guest/core') }
        'host-device-and-resource-endpoint' { return @('opennt-mvdm-host host provider -> adapter-win32; session owns resource records','original endpoint call shapes and sequencing','session host_resource mapping for host endpoint identity; no guest pointer','preserve original error/cleanup result; unsupported physical endpoint is explicit','no direct device I/O or Bochs policy in OpenNT caller') }
        'mouse-display-console-endpoint' { return @('opennt-mvdm-host UI provider -> adapter-win32; session owns UI/session state','original mouse/display/console call shapes','session host_resource mapping for window/console resources only','retain original status ordering; unsupported full-screen/old UI branch names a later monitor/WOW successor','no GUI or OpenNT semantics in Bochs') }
        'monitor-debugger-VDD-boundary' { return @('adapter-vdm-monitor, adapter-debugger or adapter-vdd as selected by original branch','same-shaped original monitor/debug/VDD call form','copied diagnostic/frame records or scoped guest_memory lease only','private NT4 monitor/VDD/debugger transport remains explicit unavailable/successor behavior','no kernel VDM, CSRSS or debugger body link') }
        default { throw "Unknown family $family" }
    }
}

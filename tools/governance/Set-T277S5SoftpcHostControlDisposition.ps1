...
function Disposition($family) {
    switch ($family) {
        ...
        'session-stop-and-event' { return @('session -> adapter-mvdm-host-out/win32; adapter-vdd successor where original hook is reached','original void event/notification/termination calls and source ordering','session-owned event/resource identity only','controlled stop never exits the surrounding app; VDD/CSRSS product branches remain explicit successors','no private event thread or process-global standard handles') }

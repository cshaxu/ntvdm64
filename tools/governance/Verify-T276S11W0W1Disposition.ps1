...
foreach ($row in $fileLedger) {
    ...
    if ($row.final_component_owner -notin @('mvdm-support', 'mvdm-tools')) { throw "Unexpected final owner for $($row.target_path)." }
...
if (@($fileLedger | Where-Object final_component_owner -eq 'mvdm-support').Count -ne 112) { throw 'Expected 112 support-owned W0/W1 paths.' }
if (@($fileLedger | Where-Object final_component_owner -eq 'mvdm-tools').Count -ne 28) { throw 'Expected 28 tool-owned W0/W1 paths.' }

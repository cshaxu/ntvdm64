...
$required = @(
    'src/ntvdm/monitor/include/monitor_context.h',
    'src/opennt-abi/host-compat/include/winbasep.h',
...
$source = Get-Content -LiteralPath (Join-Path $root 'src/ntvdm/monitor/source/vdm_control.c') -Raw
...
$carrier = Get-Content -LiteralPath (Join-Path $root 'src/opennt-abi/host-compat/include/winbasep.h') -Raw

...
const resolution = interfaces.map((item) => {
  ...
  return { canonical_interface_id: item.canonical_interface_id, callee_spelling: item.callee_spelling, raw_candidate_ids: item.raw_candidate_ids, raw_call_site_count: item.raw_call_site_count, resolution: matches.length ? (matches.length === 1 ? 'original-definition-found' : 'original-definition-variant-family') : terminal ? 'original-boundary-no-body-search' : 'no-original-c-cpp-definition-found-in-approved-trees', original_definition_ids: matches.map((match) => match.definition_id).join(';'), original_definition_identities: matches.map((match) => `${match.source_aliases}:${match.source_line}@${match.source_sha256}`).join(';'), basis: matches.length ? (fallbackSymbols.includes(item.callee_spelling) ? `Include-constrained package roots (${scopes.join(';')}) had no body; every approved-tree candidate is retained without selecting a provider.` : `Parsed only from original include-constrained package roots: ${scopes.join(';')}. Byte-identical cross-tree aliases are one identity.`) : terminal ? 'All original raw call sites terminate at a public/private/kernel/not-host-runtime boundary; T299 does not search beyond that boundary.' : `No C/C++ body definition was found in either approved tree after include-constrained roots (${scopes.join(';')}) and the discovery fallback.` };
});

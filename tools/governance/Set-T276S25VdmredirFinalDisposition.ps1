...
foreach ($row in $redir) {
    ...
    if ($row.file_kind -eq 'source') {
        ...
        $row.final_owner_or_link_boundary = 'mvdm-host original vdmredir provider -> adapter-mvdm-host-out/redir protocol boundary -> adapter-mvdm-host-out/win32/session; BOP consumers remain opennt-bop'
        $row.final_change_class = 'binding-only; retain original Redirector protocol ordering, dispatch and failure semantics'
        $row.named_adapter = 'adapter-mvdm-host-out/redir; adapter-mvdm-host-out/win32; session'
        ...
        $row.final_owner_or_link_boundary = 'mvdm-platform-abi Redirector protocol declaration supplied to the original provider and adapter-mvdm-host-out/redir binding'

...
$rows = @(
    ...
    [pscustomobject][ordered]@{
        interface_id='ADAPTER-IF-007'; family='win32-ntdll'; original_caller='dos/dem/{demdir,demfile,demgset,demsrch}.c'; original_declaration='inc/{oemuni,vdmredir}.h'; historical_dependency='NTVDM OEM path/file/search and NtVdmControl helpers'; observable_contract='original names, ANSI/OEM conversion order, Win32 last-error and explicit unsupported result'; old_candidate_scope='adapter-win32/facade/{opennt_dem_oem,opennt_dem_file_oem,opennt_dem_gset,opennt_dem_search,demfile_create_observation}.*'; source_first_disposition='recover only reached source-shaped facades in S4'; failure_or_boundary='public Win32 replacement only; `NtVdmControl` remains explicit unavailable unless monitor facade admits it'; successor='T275 S4'
    }
    [pscustomobject][ordered]@{
        interface_id='ADAPTER-IF-008'; family='win32-ntdll'; original_caller='dos/command/{cmdenv,cmdexec,cmdconf,cmdkeyb}.c'; original_declaration='inc/{cmdsvc,vdmtib}.h'; historical_dependency='NT RTL/environment/OEM/keyboard and BaseSrv command exchange'; observable_contract='original conversion, environment, current-directory, status and command-record ordering'; old_candidate_scope='adapter-win32/facade/{opennt_rtl_compat,opennt_command_oem,opennt_vdm_api}.*'; source_first_disposition='recover only local public-API/session-shaped calls in S4'; failure_or_boundary='BaseSrv/CSR broker is not silently emulated; external/multi-VDM paths remain broker disposition'; successor='T275 S4 and S6'
    }
    [pscustomobject][ordered]@{
        interface_id='ADAPTER-IF-009'; family='win32-ntdll'; original_caller='softpc.new/host/src/nt_error.c'; original_declaration='softpc.new/host/inc/error.h'; historical_dependency='NTVDM resource/error dialog product surface'; observable_contract='reason/reply ordering and VDM-session termination boundary'; old_candidate_scope='adapter-win32/facade/opennt_error_dialog_facade.*'; source_first_disposition='recover public-dialog subset in S4'; failure_or_boundary='no process termination and no CSRSS window placement'; successor='T275 S4'
    }
    [pscustomobject][ordered]@{

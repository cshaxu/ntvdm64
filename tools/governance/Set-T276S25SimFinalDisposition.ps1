...
foreach ($row in $sim) {
    ...
    switch ($row.source_path) {
        'sim32/sim32.h' {
            ...
            $row.final_owner_or_link_boundary = 'adapter-vdm-monitor same-shaped SIM32 public facade -> session guest-memory mapping manager -> adapter-softpc -> adapter-bochs'
        ...
        'sim16/incs.inc' {
            $row.final_disposition = 'binding-only'
            $row.final_owner_or_link_boundary = 'adapter-wow declaration evidence for a separately admitted complete WOW package'
            $row.final_change_class = 'binding-only; retain original 16-bit declaration/segment shape outside a current host graph'
            $row.named_adapter = 'adapter-wow; adapter-vdm-monitor; adapter-bochs'
        ...
        'sim16/sim16.asm' {
            ...
            $row.final_owner_or_link_boundary = 'retained historical WOW serial/monitor product-shell evidence; public API form is recovered by adapter-vdm-monitor or adapter-wow, not by importing this body'
            $row.final_change_class = 'none; byte-exact source mirror retained outside the final host graph'
            $row.named_adapter = 'adapter-vdm-monitor; adapter-wow; adapter-softpc; adapter-bochs'

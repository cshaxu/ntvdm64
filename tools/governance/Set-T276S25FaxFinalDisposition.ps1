...
foreach ($row in $fax) {
    ...
    if ($row.source_path -in $uiBodies) {
        ...
        $row.final_owner_or_link_boundary = 'mvdm-host original WOWFAXUI provider -> adapter-mvdm-host-out/wow -> adapter-mvdm-host-out/win32 and session copied broker records'
    ...
    elseif ($row.source_path -in $declarations) {
        ...
        $row.final_owner_or_link_boundary = 'mvdm-platform-abi original WOWFAX declaration surface supplied to the selected UI provider and adapter-mvdm-host-out/wow binding'
        $row.final_change_class = 'binding-only; preserve declaration and offset field shape'
        $row.named_adapter = 'adapter-mvdm-host-out/wow; adapter-mvdm-host-out/win32; session'

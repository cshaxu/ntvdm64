...
foreach ($row in $fax) {
    ...
    if ($row.source_path -in $uiBodies) {
        ...
        $row.final_owner_or_link_boundary = 'opennt-mvdm-host original WOWFAXUI provider -> adapter-wow -> adapter-win32 and session copied broker records'
    ...
    elseif ($row.source_path -in $declarations) {
        ...
        $row.final_owner_or_link_boundary = 'opennt-platform-abi original WOWFAX declaration surface supplied to the selected UI provider and adapter-wow binding'

...
$demRoot = Join-Path $root 'src/opennt-mvdm-host/dos/dem'
...
function Get-Disposition([string]$Header) {
    switch -Regex ($Header.ToLowerInvariant()) {
        '^(dem\.h|demmsg\.h|demdasd\.h)$' { return @('local-mirror', 'opennt-mvdm-host/dos/dem', 'byte-exact original DEM header') }
        '^(softpc\.h|mvdm\.h|vrnmpipe\.h|exterr\.h|dbgsvc\.h)$' { return @('support-mirror', 'opennt-mvdm-support/inc', 'byte-exact selected MVDM support header') }
        '^nt_vdd\.h$' { return @('platform-abi-mirror', 'opennt-platform-abi', 'byte-exact public SDK declaration mirror') }

...
$path = Join-Path (Resolve-Path -LiteralPath $RepositoryRoot).Path 'artifacts/documentation-archive/20260910/etc/operations/ledgers/m0-t276-s18-ieuvddex-file-disposition-ledger.tsv'
...
if (@($rows | Where-Object { $_.source_path -eq 'ieuvddex/ieuvddex.h' -and $_.required_boundary_owner -like 'adapter-mvdm-host-out/debugger*' }).Count -ne 1) { throw 'IEUVDDEx declarations must be assigned to adapter-mvdm-host-out/debugger.' }

...
$families = @(
    [pscustomobject][ordered]@{ interface_id = 'DOS-IF-01'; interface_family = 'BOP ABI transport'; owner = 'adapter-bop'; source_paths = Get-PathsForOwner 'adapter-bop'; evidence_basis = 'literal cmdsvc.h/dossvc.h/BOP lexical surface'; original_contract = 'OpenNT DEM/COMMAND dispatch remains source-owned; adapter transports fixed-width ingress/completion only'; disposition = 'existing adapter boundary prerequisite' },

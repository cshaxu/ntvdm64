...
$families = @(
    ...
    [pscustomobject][ordered]@{ interface_id = 'DOS-IF-02'; interface_family = 'SoftPC/SAS guest-state facade'; owner = 'adapter-mvdm-host-out/softpc'; source_paths = Get-PathsForOwner 'adapter-mvdm-host-out/softpc'; evidence_basis = 'literal softpc.h, Sim32, SAS or register-access lexical surface'; original_contract = 'retain historical CCPU/SAS call shapes and bind only through adapter-mvdm-host-out/softpc to adapter-bochs'; disposition = 'existing adapter boundary prerequisite' },
    [pscustomobject][ordered]@{ interface_id = 'DOS-IF-03'; interface_family = 'public Win32/NTDLL host capability'; owner = 'adapter-mvdm-host-out/win32'; source_paths = Get-PathsForOwner 'adapter-mvdm-host-out/win32'; evidence_basis = 'literal Win32/NTDLL header or API lexical surface'; original_contract = 'retain original provider/error ordering while mapping unavailable historical API through same-shaped public-modern facade'; disposition = 'existing adapter boundary prerequisite' },
    ...
    [pscustomobject][ordered]@{ interface_id = 'DOS-IF-05'; interface_family = 'redirector and standard-stream pipe bridge'; owner = 'adapter-mvdm-host-out/redir'; source_paths = Get-PathsForOwner 'adapter-mvdm-host-out/redir'; evidence_basis = 'literal vrnmpipe/pipe/redirector lexical surface'; original_contract = 'COMMAND/DEM keep original pipe/redirector control flow; specialist adapter owns host bridge mechanics'; disposition = 'specialist adapter prerequisite' },
    [pscustomobject][ordered]@{ interface_id = 'DOS-IF-06'; interface_family = 'VDD product callback boundary'; owner = 'adapter-mvdm-host-out/vdd'; source_paths = Get-PathsForOwner 'adapter-mvdm-host-out/vdd'; evidence_basis = 'literal nt_vdd/VDD lexical surface'; original_contract = 'VDD-specific behavior remains behind adapter-mvdm-host-out/vdd and cannot become generic DEM filesystem behavior'; disposition = 'specialist adapter prerequisite' },

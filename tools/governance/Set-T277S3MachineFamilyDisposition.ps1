...
function Family($row) {
    switch ($row.symbol) {
        ...
        'xmsEnableA20Wrapping' { return @('XMS-A20-and-state-byte','adapter-softpc -> adapter-bochs -> session guest_memory','same-shaped A20 toggle plus checked epoch-scoped Himem state-byte lease','lease-unavailable behavior requires caller/return-path proof','no XMS allocator in adapter') }
        'xmsDisableA20Wrapping' { return @('XMS-A20-and-state-byte','adapter-softpc -> adapter-bochs -> session guest_memory','same-shaped A20 toggle plus checked epoch-scoped Himem state-byte lease','lease-unavailable behavior requires caller/return-path proof','no XMS allocator in adapter') }
        'sas_overwrite_memory' { return @('XMS-range-overwrite-notification','adapter-softpc -> adapter-bochs -> session guest_memory','checked guest range mutation notification; no raw host PVOID','range-error behavior requires caller/return-path proof','no pointer identity crosses adapter') }
        'host_simulate' { return @('DPMI-bounded-machine-run','adapter-softpc -> adapter-bochs','same-shaped bounded run/stop/resume request','stop/failure result requires original caller-return-path proof','Bochs remains sole executor') }
        'EnableEmulatorIretHooks' { return @('DPMI-IRET-hook-control','adapter-vdm-monitor -> adapter-softpc','same-shaped per-bound-session IRET observation enable','unavailable behavior requires original caller-return-path proof','no kernel VDM hook') }
        'DisableEmulatorIretHooks' { return @('DPMI-IRET-hook-control','adapter-vdm-monitor -> adapter-softpc','same-shaped per-bound-session IRET observation disable','unavailable behavior requires original caller-return-path proof','no kernel VDM hook') }
        'switch_to_real_mode' { return @('DPMI-real-mode-transition','adapter-softpc -> adapter-bochs','typed mode-transition request preserving original entry/result contract','failure behavior requires original caller-return-path proof','no BIOS or DPMI policy in Bochs') }
        default { return @('DPMI-atomic-CPU-frame','adapter-softpc -> adapter-bochs','atomic typed CPU state read/commit preserving get/set ordering','failure behavior requires original caller-return-path proof','requires later source-form ABI detail before implementation') }
    }
}

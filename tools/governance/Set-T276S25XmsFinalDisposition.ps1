...
foreach ($row in $xms) {
    ...
    switch ($row.source_path) {
        'xms.486/xms.c' {
            ...
            $row.final_owner_or_link_boundary = 'mvdm-host original XMS initialization/range provider -> adapter-mvdm-host-out/softpc -> adapter-bochs; session owns instance state'
            $row.final_change_class = 'binding-only; preserve original suballocator and initialization sequence'
            $row.named_adapter = 'adapter-mvdm-host-out/softpc; adapter-bochs; session guest-memory mapping manager'
        ...
        'xms.486/xmsa20.c' {
            $row.final_disposition = 'adapter-backed'
            $row.final_owner_or_link_boundary = 'mvdm-host original A20 service -> adapter-mvdm-host-out/softpc -> adapter-bochs'
            $row.final_change_class = 'binding-only; preserve original AX/BL result control flow'
            $row.named_adapter = 'adapter-mvdm-host-out/softpc; adapter-bochs'
        ...
        'xms.486/xmsblock.c' {
            $row.final_disposition = 'adapter-backed'
            $row.final_owner_or_link_boundary = 'mvdm-host original XMS block provider -> adapter-mvdm-host-out/softpc -> adapter-bochs; suballoc remains original support'
        ...
        'xms.486/xmsdisp.c' {
            $row.final_disposition = 'adapter-backed'
            $row.final_owner_or_link_boundary = 'mvdm-host original XMS dispatch -> adapter-mvdm-host-in ingress/completion and adapter-mvdm-host-out/softpc machine facade'
            $row.final_change_class = 'binding-only; preserve selector meanings and unsupported-service failure route'
            $row.named_adapter = 'adapter-mvdm-host-in; adapter-mvdm-host-out/softpc'
        ...
        'xms.486/xmsmisc.c' {
            $row.final_disposition = 'adapter-backed'
            $row.final_owner_or_link_boundary = 'mvdm-host original system information/INT 15 provider -> adapter-mvdm-host-out/softpc -> adapter-bochs'
            $row.final_change_class = 'binding-only; preserve original system-page and interrupt-hook control flow'
            $row.named_adapter = 'adapter-mvdm-host-out/softpc; adapter-bochs; session'
        ...
        'xms.486/xmsumb.c' {
            $row.final_disposition = 'adapter-backed'
            $row.final_owner_or_link_boundary = 'mvdm-host original UMB provider -> adapter-mvdm-host-out/softpc -> adapter-bochs; session owns the durable bounded UMB range contract'
        ...
        'xms.486/xms.h' {
            ...
            $row.named_adapter = 'adapter-mvdm-host-in; adapter-mvdm-host-out/softpc; session'
        ...
        'xms.486/i386/xmsmem86.c' {
            ...
            $row.final_owner_or_link_boundary = 'retained original x86 direct-address backend evidence; same-shaped contracts are supplied by adapter-mvdm-host-out/softpc -> adapter-bochs'

...
function Disposition($family){switch($family){
 ...
 'Redirector-guest-address-lease'{return @('adapter-mvdm-host-out/redir -> adapter-mvdm-host-out/softpc -> session guest_memory','original segment:offset:size conversion contract','scoped session guest_memory lease or copied buffer; never return a durable raw PVOID','invalid span/expired lease yields original source-derived error path','no Redirector-private pointer mapper')}
 'Redirector-debug-output'{return @('adapter-mvdm-host-out/redir -> adapter-mvdm-host-out/debugger','original variadic debug-output call shape','formatted copied diagnostic record only','debug output unavailable is non-functional diagnostic omission, never protocol success','no debugger product body link')}
 'WOW-bounded-machine-run'{return @('adapter-mvdm-host-out/softpc -> adapter-bochs','original host_simulate ordering around saved/restored guest IP','S3 typed bounded run only; no raw CPU pointer','return/stop result follows WOW caller path; no CCPU loop','Bochs remains sole executor')}
 'WOW-debugger-notification'{return @('adapter-mvdm-host-out/wow -> adapter-mvdm-host-out/debugger','original debugger notification parameters and ordering','copied fixed-width session diagnostic record; no raw VP/PVOID/HANDLE','debugger absent follows original no-debug path','no NT4 remote-debug product shell')}
 'WOW-media-debug'{return @('adapter-mvdm-host-out/wow -> adapter-mvdm-host-out/debugger','original VOID MCI debug output/level helper forms','copied UTF-8/ANSI diagnostic data only','debug facility absent is explicit no-debug behavior','no multimedia product enablement')}
 'WOW-16-to-32-dispatch'{return @('adapter-mvdm-host-out/wow -> adapter-mvdm-host-out/softpc -> session guest_memory','original W32Dispatch entry and WOW thunk ordering','scoped guest_memory leases and 32-bit compatibility object values only','unavailable thunk returns original WOW failure; no direct host API substitution','no raw pointer/handle or table reconstruction outside original WOW package')}
 'WOW-serial-endpoint'{return @('adapter-mvdm-host-out/wow -> adapter-mvdm-host-out/win32; session host_resource mapping','original BYTE/HANDLE COM shadow/query forms','session host_resource maps native endpoint identity to old 32-bit compatible object values','missing endpoint preserves original invalid/no-port result','no native HANDLE crosses old WOW ABI')}
 default{throw "Unknown family $family"}}}

...
$rows=foreach($f in $files){$p=$f.target_path;$role='';$boundary='';$summary='';$disp='';
 ...
 elseif($p -in $selected486 -or $p -in $selectedShared){$role='historical-dosx-guest-provider-body';$boundary='mvdm-guest-dos; adapter-mvdm-host-in; adapter-softpc; adapter-vdm-monitor; adapter-bochs; session mapping manager';$summary='selected by original DOSX 486 object list; 16-bit DPMI provider/interrupt-reflector guest body, retained as a whole guest-product recovery unit';$disp='guest-provider package prerequisite; no host composition'}
 elseif($p -match '^dpmi/(dxboot|dxfunc|dxint31|dxintr|dxmain|dxstrt|dxutil|dxhpbios|dxvcpi|dxvcpibt|ntintr32)\.asm$'){$role='historical-dosx-alternate-or-gated-guest-body';$boundary='mvdm-guest-dos; adapter-mvdm-host-in; adapter-softpc; adapter-vdm-monitor; adapter-bochs';$summary='original alternate, optional or non-selected DOSX guest body; preserve source and build-gate evidence without host composition';$disp='guest-profile-gated source evidence'}
 elseif($p -match '^dpmi/.*\.(inc|mas)$'){$role='historical-dosx-guest-declaration-or-macro';$boundary='mvdm-guest-dos; adapter-mvdm-host-in; adapter-softpc; adapter-vdm-monitor; adapter-bochs';$summary='original DOSX guest interface/macro/layout evidence; retain with the guest package and do not translate it into a host ABI';$disp='guest-declaration prerequisite'}

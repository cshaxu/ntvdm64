...
$path = Join-Path $root 'artifacts/documentation-archive/20260910/etc/operations/ledgers/m0-t276-s12-softpc-file-disposition-ledger.tsv'
...
if (@($rows | Where-Object final_component_owner -eq 'mvdm-softpc-firmware').Count -ne 58) { throw 'Expected 58 separately mirrored firmware-input rows.' }
foreach ($row in $rows) {

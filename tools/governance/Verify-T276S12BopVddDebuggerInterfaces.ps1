...
$path = Join-Path $root 'artifacts/documentation-archive/20260910/etc/operations/ledgers/m0-t276-s12-bop-vdd-debugger-interface-ledger.tsv'
...
if (@($rows | Where-Object source_path -eq 'softpc.new/host/src/nt_bop.c' | Where-Object single_recovery_owner -notmatch 'adapter-mvdm-host-in').Count -eq 0) { throw 'BOP dispatcher review must name mechanical adapter-mvdm-host-in boundary.' }
if (@($rows | Where-Object source_path -eq 'softpc.new/host/src/nt_vdd.c' | Where-Object single_recovery_owner -match 'adapter-mvdm-host-out/vdd').Count -eq 0) { throw 'VDD review must name adapter-mvdm-host-out/vdd boundary.' }

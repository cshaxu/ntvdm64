...
foreach ($relative in @(
    'src/adapter-win32/include/ntioapi.h',
    'src/adapter-win32/source/ntioapi_facade.c',
    'tests/adapter-win32/t280_s11_ntioapi_facade_fixture.c',
...
$header = Get-Content -Raw -LiteralPath (Join-Path $root 'src/adapter-win32/include/ntioapi.h')
$source = Get-Content -Raw -LiteralPath (Join-Path $root 'src/adapter-win32/source/ntioapi_facade.c')

...
foreach ($relative in @(
    ...
    'src/adapter-win32/README.md',
    'tests/adapter-win32/t280_s9_dem_declaration_fixture.c',
...
$facade = Get-Content -LiteralPath (Join-Path $root 'src/adapter-win32/include/nt.h') -Raw
...
if ($facade -match '#include <ntioapi\.h>') { throw 'T280 S9 imported the unbound ntioapi surface into adapter-win32.' }
$fixture = Get-Content -LiteralPath (Join-Path $root 'tests/adapter-win32/t280_s9_dem_declaration_fixture.c') -Raw

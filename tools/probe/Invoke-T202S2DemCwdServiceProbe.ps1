...
$sources = @('bx_ntvdm_mutation_profile_v1.c','bx_ntvdm_mutation_overlay_v1.c',
    ...
    'bx_ntvdm_dem_cwd_service_v2.c') |
    ForEach-Object { Join-Path $root ('src\bx-vdm\' + $_) }
$sources += Join-Path $root 'tests\bx-vdm\bx_ntvdm_dem_cwd_service_v2_test.c'
...
if ($LASTEXITCODE -ne 0) { throw "T202 S2 DEM CWD service compilation failed: $LASTEXITCODE" }
& $exe 2>&1 | Tee-Object -FilePath (Join-Path $build 'run.log')
$runExit = $LASTEXITCODE
[ordered]@{ schema='ntdos64.t202.s2.dem-cwd-service.v2'; architecture='x64'; runtimeLibrary='/MT'; hostIo=$true; guestExecution=$false; runExitCode=$runExit; passed=($runExit -eq 0) } | ConvertTo-Json | Set-Content -LiteralPath (Join-Path $build 't202-s2-dem-cwd-service.json') -Encoding utf8
if ($runExit -ne 0) { throw "T202 S2 DEM CWD service fixture failed: $runExit" }
Write-Host "T202 S2 DEM CWD service probe passed: $build"

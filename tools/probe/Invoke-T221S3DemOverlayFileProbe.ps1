...
if ($LASTEXITCODE -ne 0) { throw "T221 S3 DEM Overlay file compilation failed: $LASTEXITCODE" }
& $exe 2>&1 | Tee-Object -FilePath (Join-Path $build 'run.log')
$runExit = $LASTEXITCODE
[ordered]@{ schema='ntdos64.t221.s3.dem-overlay-file.v1'; architecture='x64'; runtimeLibrary='/MT'; hostIo=$false; guestExecution=$false; runExitCode=$runExit; passed=($runExit -eq 0) } | ConvertTo-Json | Set-Content -LiteralPath (Join-Path $build 't221-s3-dem-overlay-file.json') -Encoding utf8
if ($runExit -ne 0) { throw "T221 S3 DEM Overlay file probe failed: $runExit" }
Write-Host "T221 S3 DEM Overlay file probe passed: $build"

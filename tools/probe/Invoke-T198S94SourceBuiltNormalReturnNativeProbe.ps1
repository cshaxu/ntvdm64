...
& (Join-Path $repository 'tools\probe\Invoke-T198S93SourceBuiltNormalReturnFixturePrepare.ps1') -RepositoryRoot $repository -BuildRoot $prepared
...
$includes = @('src', 'refs\bochs', 'refs\bochs\instrument\stubs', 'src\bx-core', 'src\bx-core\cpu', 'src\bx-mantle', 'src\bx-vdm', 'src\cli', 'tests\bx-vdm') | ForEach-Object { '/I "' + (Join-Path $repository $_) + '"' }
...
if ($responseLines -notcontains 'user32.lib') { $responseLines += 'user32.lib' }

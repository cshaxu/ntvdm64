...
if ([string]::IsNullOrWhiteSpace($BuildRoot)) {
    $BuildRoot = Join-Path $repository 'artifacts\build\t199-s18-top-level-facade-r1'
}
...
$relatives = @(
    ...
    'tests\bx-vdm\bx_ntvdm_top_level_package_facade_v1_test.c'
)
...
$exe = Join-Path $build 't199-s18-top-level-facade.exe'
$compile = 'call "' + $vsDevCmd + '" -arch=x64 -host_arch=x64 >nul && cl.exe /nologo /TC /std:c11 /W4 /WX /MT /DWIN32 /I "' +
...
if ($LASTEXITCODE -ne 0) { throw "MSVC x64 top-level facade compilation failed: $LASTEXITCODE" }
& $exe 2>&1 | Tee-Object -LiteralPath (Join-Path $build 'run.log')
$runExit = $LASTEXITCODE
[ordered]@{ schema = 'ntdos64.t199.s18.top-level-facade.x64-probe.v1'; architecture = 'x64'; compiler = 'MSVC cl.exe via VsDevCmd'; runtime = '/MT'; sourceClosure = $relatives; runExitCode = $runExit; expectedRunExitCode = 0; passed = ($runExit -eq 0) } |
    ConvertTo-Json -Depth 3 | Set-Content -LiteralPath (Join-Path $build 't199-s18-top-level-facade.json') -Encoding utf8
if ($runExit -ne 0) { throw "MSVC x64 top-level facade fixture failed: $runExit" }
Write-Host "Built and ran MSVC x64 top-level facade fixture: $exe"

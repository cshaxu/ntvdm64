...
$sources += Join-Path $root 'tests\bx-vdm\bx_ntvdm_dem_local_file_backend_v1_test.c'
...
if ($LASTEXITCODE -ne 0) { throw "T202 S4 DEM local-file backend compilation failed: $LASTEXITCODE" }
& $exe 2>&1 | Tee-Object -FilePath (Join-Path $build 'run.log')
$runExit = $LASTEXITCODE
[ordered]@{ schema='ntdos64.t202.s4.dem-local-file-backend.v1'; architecture='x64'; runtimeLibrary='/MT'; hostIo=$true; guestExecution=$false; runExitCode=$runExit; passed=($runExit -eq 0) } | ConvertTo-Json | Set-Content -LiteralPath (Join-Path $build 't202-s4-dem-local-file-backend.json') -Encoding utf8
if ($runExit -ne 0) { throw "T202 S4 DEM local-file backend probe failed: $runExit" }
Write-Host "T202 S4 DEM local-file backend probe passed: $build"

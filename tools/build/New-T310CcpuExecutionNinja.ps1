...
New-Item -ItemType Directory -Force $build, (Join-Path $build 'obj/ccpu'), (Join-Path $build 'obj/host'), (Join-Path $build 'obj/test') | Out-Null
...
$graph.Add('build ccpu-bounded-execution.exe: link ' + (($testObj + @('original-ccpu-execution.lib', 'original-softpc-s5-host.lib')) -join ' '))

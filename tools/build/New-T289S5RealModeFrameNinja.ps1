...
$build = Join-Path $root ("build/M0-T289/S5/realmode-frame-{0}" -f $Architecture)
...
$fixture = 'tests/adapter-bochs/t289_s5_realmode_frame_fixture.cc'
...
New-Item -ItemType Directory -Force $build, (Join-Path $build 'obj/core'), (Join-Path $build 'obj/adapter'), (Join-Path $build 'obj/fixture'), (Join-Path $build 'lib'), (Join-Path $build 'bin') | Out-Null
$environment = Join-Path $build 'msvc-mt.cmd'
@('@echo off', 'set "MVDM_T289_CALLER_CWD=%CD%"', 'if defined VSCMD_VER goto ready', ('call "' + $vs + '" -arch=' + $Architecture + ' -host_arch=x64 >nul'), 'if errorlevel 1 exit /b %errorlevel%', ':ready', 'cd /d "%MVDM_T289_CALLER_CWD%"', '%*') | Set-Content -LiteralPath $environment -Encoding ascii
$manifest = [ordered]@{ schema = 'm0.t289.s5.real-mode-frame.v1'; architecture = $Architecture; runtimeLibrary = '/MT'; coreSources = @($core + $overlay | ForEach-Object { [ordered]@{ path = $_; sha256 = Sha256 (Join-Path $root $_) } }); adapterSources = @($adapter | ForEach-Object { [ordered]@{ path = $_; sha256 = Sha256 (Join-Path $root $_) } }); fixture = [ordered]@{ path = $fixture; sha256 = Sha256 (Join-Path $root $fixture) }; forbiddenInputs = @('src.old', 'DPMI provider source', 'BOP ingress', 'prebuilt Bochs archive') }
$manifest | ConvertTo-Json -Depth 6 | Set-Content -LiteralPath (Join-Path $build 'source-manifest.json') -Encoding utf8
$includes = @('src', 'src/bochs-core', 'src/bochs-core/cpu', 'src/bochs-core/iodev', 'src/bochs-core/instrument/stubs', 'src/adapter-bochs') | ForEach-Object { '/I "' + (NinjaPath (Join-Path $root $_)) + '"' }
...
function AddLibrary([string]$name, [string[]]$sources) {
    ...
    $library = 'lib/' + $name + '.lib'; [IO.File]::WriteAllLines((Join-Path $build ($library + '.rsp')), $objects, [Text.UTF8Encoding]::new($false)); $graph.Add('build ' + $library + ': lib ' + ($objects -join ' ')); return $library
}
$coreLibrary = AddLibrary 'bochs-core' @($core + $overlay)
$adapterLibrary = AddLibrary 'adapter-bochs' $adapter
$fixtureObject = 'obj/fixture/' + (ObjectName $fixture); $graph.Add('build ' + $fixtureObject + ': cxx ' + (NinjaPath (Join-Path $root $fixture)))
$output = 'bin/t289-s5-realmode-frame-fixture.exe'; [IO.File]::WriteAllLines((Join-Path $build ($output + '.rsp')), @($fixtureObject, $adapterLibrary, $coreLibrary, 'kernel32.lib', 'user32.lib'), [Text.UTF8Encoding]::new($false)); $graph.Add('build ' + $output + ': link ' + $fixtureObject + ' ' + $adapterLibrary + ' ' + $coreLibrary); $graph.Add('default ' + $output)
[IO.File]::WriteAllLines((Join-Path $build 'build.ninja'), $graph, [Text.UTF8Encoding]::new($false))
Write-Output "Generated T289 S5 real-mode frame graph: $build"

...
$build = Join-Path $root ("build/M0-T310/S8/p2-ica-lock/{0}" -f $Architecture)
...
$graph.Add('build obj/nt_eoi.obj: cc ' + (NinjaPath (Join-Path $root 'src/mvdm-host/softpc.new/host/src/nt_eoi.c')))
$graph.Add('build obj/mvdm_ica_eoi_bridge.obj: cc ' + (NinjaPath (Join-Path $root 'src/mvdm-host-overlay/softpc.new/host/src/mvdm_ica_eoi_bridge.c')))
$graph.Add('build obj/softpc_ica_lock_fixture.obj: cc ' + (NinjaPath (Join-Path $root 'tests/mvdm-host/softpc_ica_lock_fixture.c')))
$graph.Add('build softpc-ica-lock-fixture.exe: link obj/nt_eoi.obj obj/mvdm_ica_eoi_bridge.obj obj/softpc_ica_lock_fixture.obj')
$graph.Add('build verify: run softpc-ica-lock-fixture.exe')
...
Write-Host "Generated T310 S8 P2 original ICA-lock graph: $build"

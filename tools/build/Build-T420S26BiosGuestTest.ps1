...
$source = Join-Path $repository 'tests\observation\bios_capability.asm'
$output = Join-Path $build 'BIOSTST.COM'
& $nasm.Source -f bin -o $output $source
if ($LASTEXITCODE -ne 0) { throw "NASM failed for S26 BIOS fixture: $LASTEXITCODE" }
[ordered]@{
    schema = 'm0.t420.s26.bios-guest.v1'
    source = 'tests/observation/bios_capability.asm'
    output = 'BIOSTST.COM'
    sha256 = (Get-FileHash -LiteralPath $output -Algorithm SHA256).Hash.ToLowerInvariant()
} | ConvertTo-Json | Set-Content -LiteralPath (Join-Path $build 'manifest.json') -Encoding utf8
Write-Host "S26 BIOS guest fixture completed: $output"

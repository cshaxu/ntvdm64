...
if (Test-Path -LiteralPath $output) { throw "Refusing to overwrite S46 guest EXEC lifecycle fixture root: $output" }
$items = @(
...
foreach ($item in $items) {
    ...
    if ((Get-Item -LiteralPath $destination).Length -ne $item.bytes -or (Get-Sha256 $destination) -ne $item.sha256) { throw "Copied S46 identity mismatch for $($item.role)" }
}
$profile = [ordered]@{
    schema='ntdos64-byob-profile-v8'; profile='nt4-en-us-cli-stream-v8'; architecture='x86'; locale='en-US'; compatibility_group='t225-s46-guest-exec-lifecycle'
    ...
    features=@(); owner_note='T225 S46 source-built guest parent/child/return fixture; TARGET.EXE is a byte-identical profile alias for source-built SHARE.EXE.'
...
[ordered]@{schema='ntdos64.t225.s46.guest-exec-fixture-inputs.v1';classification='primary-original-toolchain-source-built';sourceRoot=$root;inputs=$items;targetAlias=[ordered]@{source_role='share';source_name='SHARE.EXE';profile_name='TARGET.EXE';content_changed=$false};allowedUse=@('T225 S46 bounded Direct/Readonly guest EXEC lifecycle observation');forbiddenUse=@('reference fallback','default runtime discovery','release packaging','BOP semantic expansion','Overlay selection','Virtual selection')} | ConvertTo-Json -Depth 7 | ForEach-Object { [IO.File]::WriteAllText((Join-Path $output 'guest-exec-fixture-manifest.json'), $_ + [Environment]::NewLine, [Text.UTF8Encoding]::new($false)) }
Write-Host "Staged source-built T225 S46 guest EXEC fixture inputs: $output"
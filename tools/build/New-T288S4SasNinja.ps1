...
$build = Join-Path $root ("build/M0-T288/S4/{0}" -f $Architecture)
...
$sources = @('src/adapter-mvdm-host-out/softpc/mvdm_sas.c', 'tests/adapter-mvdm-host-out/softpc/sas_memory_fixture.c')
...
$graph.Add('build bin/sas-memory-fixture.exe: link ' + ($objects -join ' '))
$graph.Add('default bin/sas-memory-fixture.exe')
$graph | Set-Content -LiteralPath (Join-Path $build 'build.ninja') -Encoding ascii
Write-Output ("Generated T288 S4 {0} SAS graph: {1}" -f $Architecture, $build)

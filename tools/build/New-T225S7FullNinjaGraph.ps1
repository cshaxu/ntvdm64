...
$graph.Add('')
$graph.Add('rule lib')
$graph.Add('  command = cmd.exe /d /s /c call ' + $envNinja + ' lib.exe /nologo /OUT:$build_root/$out @$rspfile')
$graph.Add('  rspfile = $out.rsp')
$graph.Add('  rspfile_content = $in')
$graph.Add('  description = LIB $out')
$graph.Add('')
$graph.Add('rule link')
$graph.Add('  command = cmd.exe /d /s /c call ' + $envNinja + ' link.exe /nologo /OPT:REF /OUT:$build_root/$out @$rspfile')
$graph.Add('  rspfile = $out.rsp')

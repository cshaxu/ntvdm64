...
$source = Join-Path $repository 'tests\observation\system_capability.asm'
$output = Join-Path $build 'SYSTST.COM'
& $nasm.Source -f bin -o $output $source
if ($LASTEXITCODE -ne 0) { throw "NASM failed for S24 system fixture: $LASTEXITCODE" }
[ordered]@{
    schema = 'm0.t420.s24.system-guest.v1'
    source = 'tests/observation/system_capability.asm'
    output = 'SYSTST.COM'
    sha256 = (Get-FileHash -LiteralPath $output -Algorithm SHA256).Hash.ToLowerInvariant()
} | ConvertTo-Json | Set-Content -LiteralPath (Join-Path $build 'manifest.json') -Encoding utf8
Write-Host "S24 system guest fixture completed: $output"

...
$build = Join-Path $root ("build/M0-T287/S5/{0}" -f $Architecture)
...
$cSources = @(
    ...
    'src/mvdm-host/xms.486/xmsa20.c',
    'src/mvdm-host/xms.486/xmsumb.c',
    'src/mvdm-host/xms.486/xmsmisc.c'
)
$cxxSources = @(
...
$fixture = 'tests/adapter-bochs/t287_s5_xms_umb_ivt_fixture.cc'
foreach ($path in $cSources + $cxxSources + $fixture) {
    if (!(Test-Path -LiteralPath (Join-Path $root $path) -PathType Leaf) -or
        $path -match '(^|/)src\.old(/|$)') { throw "Invalid S5 input: $path" }
}
...
$manifest = [ordered]@{
    schema = 'm0.t287.s5.xms-umb-ivt.v1'; architecture = $Architecture; runtimeLibrary = '/MT'
...
$graph.Add('ninja_required_version = 1.10'); $graph.Add('build_root = ' + (NinjaPath $build))
...
$library = 'lib/original-xms-umb-ivt.lib'
...
$output = 'bin/t287-s5-xms-umb-ivt-fixture.exe'
...
$graph.Add('build ' + $output + ': link ' + $fixtureObject + ' ' + $library + ' ' + ($bridgeObjects -join ' ') + ' ' + (NinjaPath $bochs))
$graph.Add('default ' + $output)
[IO.File]::WriteAllLines((Join-Path $build 'build.ninja'), $graph, [Text.UTF8Encoding]::new($false))
Write-Host "Generated T287 S5 $Architecture XMS UMB/IVT graph: $build"

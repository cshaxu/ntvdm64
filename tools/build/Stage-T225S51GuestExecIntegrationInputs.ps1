...
if (Test-Path -LiteralPath $output) { throw "Refusing to overwrite S51 guest integration fixture root: $output" }
$items = @(
...
[ordered]@{schema='ntdos64.t225.s51.guest-exec-integration-inputs.v1';classification='primary-original-toolchain-source-built';sourceRoot=$root;inputs=$items;targetAlias=[ordered]@{source_role='share';source_name='SHARE.EXE';profile_name='TARGET.EXE';content_changed=$false};allowedUse=@('T225 S51 bounded Direct/Readonly post-handoff integration observation');forbiddenUse=@('reference fallback','default runtime discovery','release packaging','BOP semantic expansion','Overlay selection','Virtual selection')} | ConvertTo-Json -Depth 7 | ForEach-Object { [IO.File]::WriteAllText((Join-Path $output 'guest-exec-lifecycle-ledger-inputs-manifest.json'), $_ + [Environment]::NewLine, [Text.UTF8Encoding]::new($false)) }
Write-Host "Staged source-built T225 S51 guest integration fixture inputs: $output"
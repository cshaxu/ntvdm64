...
$engineHash = Copy-Verified $engine (Join-Path $bundle 'ntdos64-bochs.exe')
...
$optionHash = Copy-Verified $option (Join-Path $bundle 'ROM\ntdos64-post-handoff.rom')
$manifest = @(
...
[IO.File]::WriteAllText((Join-Path $bundle 'ntdos64-bochs-engine-bundle-v1.txt'), $manifest + "`n", [Text.UTF8Encoding]::new($false))

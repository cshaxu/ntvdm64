...
if ($baseManifest.adapterSources.Count -ne 67 -or $baseManifest.cliSources.Count -ne 5 -or
    (Hash (Join-Path $base 'ntdos64-t182-finite-sequence.exe')) -ne
...
$record = [ordered]@{
    schema = 'ntdos64.t187.s3.passive-listener-engine-derivative.v1'
    baseRoot = $base
    baseEngineSha256 = Hash (Join-Path $base 'ntdos64-t182-finite-sequence.exe')
    buildRoot = $build
    bochsObjectTargets = @('cpu\exception.o')
    finalTargets = @('ntdos64-t187-passive-listener.exe','ntdos64-t187-passive-listener.map')
...
$makePath = Join-Path $build 'ntdos64-t187-passive-listener.mak'
...
$record.permittedBuild = 'nmake /f ntdos64-t187-passive-listener.mak ntdos64-t187-passive-listener.exe'

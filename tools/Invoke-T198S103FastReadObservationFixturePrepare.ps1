...
foreach ($path in @($vsDevCmd, $config, $bridge,
    (Join-Path $repository 'tools\Invoke-T198S93SourceBuiltNormalReturnFixturePrepare.ps1'))) {
...
$includes = @('src', 'src\bochs', 'src\bochs\instrument\stubs', 'src\bx-core', 'src\bx-core\cpu', 'src\bx-mantle', 'src\bx-vdm', 'src\cli') |

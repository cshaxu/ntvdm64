...
foreach ($path in @($vs, $baseline, $inputComposition,
    ...
    if (-not (Test-Path -LiteralPath $path)) { throw "Missing T214 S2 input: $path" }
}
...
$includes = @('src','src\cli','src\bochs','src\bochs\instrument\stubs','src\bx-core','src\bx-core\cpu','src\bx-mantle','src\bx-vdm') | ForEach-Object { '/I "' + (Join-Path $root $_) + '"' }
...
if ($LASTEXITCODE -ne 0) { throw "T214 S2 current-object compile failed: $LASTEXITCODE" }
& (Join-Path $root 'tools\Invoke-T200S27NativeCliProbe.ps1') `
...
$expected = 'terminal=2 detail=40 lifecycle=3 presentation=3 cancellation=0'
if (-not $base.passed -or $runLog -notmatch [regex]::Escape($expected)) {
    throw "T214 S2 native diagnostic did not produce expected namespace-initialize rejection: $expected"
}
$record = [ordered]@{
    schema='ntdos64.t214.s2.composition-admission-diagnostic.v1'
    architecture='x64'
    runtimeLibrary='/MT'
    rebuiltCurrentObjects=@($sources.Source)
    runExitCode=3
    rejectionStage=40
    rejectionStageName='namespace-initialize'
    reachedGuestCpu=$false
    resetAfterRejection='composition runtime reset is called before the rejected result is returned'
    passed=$true
}
$record | ConvertTo-Json -Depth 4 | Set-Content -LiteralPath (Join-Path $build 't214-s2-composition-admission-diagnostic.json') -Encoding utf8
Write-Host "T214 S2 composition admission diagnostic passed: $build"

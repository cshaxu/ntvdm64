...
cd /d "$build"
nmake.exe /f runner-native-container.mak runner-native-container.exe > "$logPath" 2>&1
:record
set "APP_EXITCODE=%ERRORLEVEL%"
> "$exitTemp" echo %RUNNER_EXITCODE%
...
$record = [ordered]@{
    schema = 'runner.t95-s6.intact-native-detached-observation.v1'
    buildRoot = $build
    ...
    command = 'nmake /f runner-native-container.mak runner-native-container.exe'
    retryPolicy = 'none'
}

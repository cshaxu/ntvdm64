...
$baseEngine = Join-Path $base 'runner-t188-copied-state.exe'
...
$baseMake = Join-Path $base 'runner-t188-copied-state.mak'
...
$make = @(
    ...
    'runner-t190-command-bootstrap.exe: cpu\exception.o adapter\bx_ntvdm_adapter_runtime.obj adapter\bx_ntvdm_cmd_comspec_bootstrap_service.obj',
    "`t$linkLine"
)
$makePath = Join-Path $build 'runner-t190-command-bootstrap.mak'
...
$record = [ordered]@{
    schema='runner.t190.s3.command-bootstrap-derivative.v1'
    ...
    permittedBuild='nmake /f runner-t190-command-bootstrap.mak runner-t190-command-bootstrap.exe'
}

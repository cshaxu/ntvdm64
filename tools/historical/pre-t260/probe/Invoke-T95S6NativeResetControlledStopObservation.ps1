...
param(
    ...
    [string]$BuildRoot = '',
    [string]$EvidenceRoot = '',
    [string]$BinaryName = 'runner-native-container.exe',
...
setlocal
set "RUNNER_UD_TEST="
set "RUNNER_ADAPTER_BRIDGE="
set "RUNNER_ADAPTER_PROFILE="
set "RUNNER_ADAPTER_ROOT="
cd /d "$evidence"
...
$negativeTerms = @('RUNNER_ADAPTER_PROFILE=', 'RUNNER_ADAPTER_ROOT=', '--byob', 'ntio=', 'ntdos=', 'command=')
$commandHasForbiddenTerm = @($negativeTerms | Where-Object { $command -match [regex]::Escape($_) }).Count -ne 0
$record = [ordered]@{
    schema = 'runner.t95-s6.native-reset-controlled-stop-observation.v1'
    buildRoot = $build
    ...
    removedEnvironmentVariables = @('RUNNER_UD_TEST', 'RUNNER_ADAPTER_BRIDGE', 'RUNNER_ADAPTER_PROFILE', 'RUNNER_ADAPTER_ROOT')
}

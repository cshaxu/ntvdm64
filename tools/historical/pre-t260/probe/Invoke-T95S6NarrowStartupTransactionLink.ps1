...
$shim = Join-Path $build 'runner-native-startup-transaction.mak'
...
if ($record.schema -ne 'runner.t95-s6.narrow-startup-transaction-build-root.v1') {
...
cd /d "$build"
nmake.exe /f runner-native-startup-transaction.mak runner-native-startup-transaction.exe > "$log" 2>&1
:record
set "RUNNER_EXITCODE=!ERRORLEVEL!"
> "$exitRecord" echo !RUNNER_EXITCODE!

...
$sources = @('src\cli\runner_engine_worker_v1.c', 'src\bx-vdm\bx_ntvdm_engine_contract_v1.c', 'tests\runner\runner_engine_worker_v1_test.c') | ForEach-Object { Join-Path $root $_ }
...
$command = 'call "' + $vs + '" -arch=x64 -host_arch=x64 >nul && cl.exe /nologo /TC /std:c11 /W4 /WX /MT /I "' + (Join-Path $root 'src') + '" /I "' + (Join-Path $root 'src\cli') + '" /I "' + (Join-Path $root 'src\bx-machine') + '" /Fe:"' + $exe + '" "' + ($sources -join '" "') + '"'
...
$record = [ordered]@{schema='runner.t212.s6.cli-cancellation.v1';architecture='x64';runtimeLibrary='/MT';sourceClosure=@('src/cli/runner_engine_worker_v1.[hc]','src/bx-machine/bx_ntvdm_engine_contract_v1.[hc]','tests/runner/runner_engine_worker_v1_test.c');engineHandleTransfer=$false;guestExecution=$false;runExitCode=$run;expectedRunExitCode=0;passed=($run -eq 0)}

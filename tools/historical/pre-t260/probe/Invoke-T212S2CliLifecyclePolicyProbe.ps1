...
$sources = @(
    'src\cli\runner_lifecycle_v1.c',
    'src\bx-vdm\bx_ntvdm_engine_contract_v1.c',
    'tests\runner\runner_lifecycle_v1_test.c'
...
$command = 'call "' + $vs + '" -arch=x64 -host_arch=x64 >nul && cl.exe /nologo /TC /std:c11 /W4 /WX /MT /I "' +
    (Join-Path $root 'src\cli') + '" /I "' + (Join-Path $root 'src\bx-machine') +
    '" /Fe:"' + $exe + '" "' + ($sources -join '" "') + '"'
...
$record = [ordered]@{
    schema = 'runner.t212.s2.cli-lifecycle-policy.v1'
    architecture = 'x64'
    runtimeLibrary = '/MT'
    sourceClosure = @('src/cli/runner_lifecycle_v1.[hc]', 'src/bx-machine/bx_ntvdm_engine_contract_v1.c', 'tests/runner/runner_lifecycle_v1_test.c')

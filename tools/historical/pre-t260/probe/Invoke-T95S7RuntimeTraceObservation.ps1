...
param(
    ...
    [string]$BuildRoot = '',
    [string]$BinaryName = 'runner-s7-runtime-trace.exe',
...
$profile=[ordered]@{schema='runner-byob-profile-v4';profile='nt4-en-us-command-smoke-v4';architecture='x86';locale='en-US';compatibility_group='t97-pathname-runtime-trace';components=@($inputs|%{[ordered]@{role=$_.role;file_name=$_.name;required=$true;bytes=$_.bytes;sha256=$_.sha.ToLowerInvariant();version=$null}});features=@();owner_note=$null;guest_command_placement=@{path='\COMMAND.COM';drive_index=2};guest_target_placement=@{path='\TARGET.COM';drive_index=2};guest_boot_files=@{config=@{path='\CONFIG.SYS';materialization='minimal-comment-v1'};autoexec=@{path='\AUTOEXEC.BAT';materialization='empty-v1'}};guest_search_metadata=@{command=@{attributes=32;dos_time=1;dos_date=33};target=@{attributes=32;dos_time=2;dos_date=34};config=@{attributes=32;dos_time=3;dos_date=35};autoexec=@{attributes=32;dos_time=4;dos_date=36}};host_drive_inventory=@{types=$driveTypes};machine_startup_plan=$true;machine_startup_entry='ntio-v0'}
$optionRomLine = ''
if ($DeferredStartupPlan) {
    ...
    $optionRom[511]=0xf2
    [IO.File]::WriteAllBytes((Join-Path $evidence 'ROM\runner-post-handoff.rom'), $optionRom)
    if ((Hash (Join-Path $evidence 'ROM\runner-post-handoff.rom')) -ne 'E4CA4AE5190A15ED741D1DFED5A58533A2B6926A40302D839FB714F9FC673FFB') { throw 'Option-ROM identity mismatch' }
    $profile.machine_startup_handoff=[ordered]@{exception_vector=6;instruction_bytes='0f0b';cs=51840;ip=3}
    $optionRomLine="optromimage1: file=ROM/runner-post-handoff.rom, address=0xca800`r`n"
}

...
$binary = Join-Path $build 'runner-native-startup-transaction.exe'
...
$environmentChanges = [ordered]@{
    ...
    RUNNER_ADAPTER_PROFILE = $profilePath
    RUNNER_ADAPTER_ROOT = (Join-Path $evidence 'byob')
}
...
$record = [ordered]@{
    schema = 'runner.t95-s6.real-ntio-startup-transaction-observation.v2'
    buildRoot = $build; evidenceRoot = $evidence
    command = 'runner-native-startup-transaction.exe -q -f real-ntio-startup.bochsrc -benchmark 1 -log bochs.log'
    ...
    # Passive catalogue observations deliberately use the "runner adapter bop
    ...
    bopObservationCount = [regex]::Matches($log, 'runner adapter bop observed cpu=').Count
    broadInterceptorObserved = $log -match 'runner #UD test interceptor|runner broad (?:#UD )?interceptor'

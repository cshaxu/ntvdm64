...
$shim = Join-Path $build 'runner-native-adapter-observation.mak'
...
if ($rootRecord.schema -ne 'runner.t95-s6.narrow-companion-build-root.v1') { throw 'Unsupported companion build-root record.' }
...
cd /d "$build"
nmake.exe /f runner-native-adapter-observation.mak runner-native-adapter-observation.exe > "$log" 2>&1
:record
set "RUNNER_EXITCODE=%ERRORLEVEL%"
> "$exitCodePath" echo %RUNNER_EXITCODE%
...
$output = Join-Path $build 'runner-native-adapter-observation.exe'
$map = Join-Path $build 'runner-native-adapter-observation.map'
$record = [ordered]@{
    schema = 'runner.t95-s6.narrow-companion-link-observation.v1'
    buildRoot = $build

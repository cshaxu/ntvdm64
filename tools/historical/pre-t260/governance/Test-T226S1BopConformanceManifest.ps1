...
$temp = Join-Path ([IO.Path]::GetTempPath()) ('runner-t226-s1-' + [guid]::NewGuid().ToString('N') + '.json')
try {
    ...
    if ($actual.schema -ne 'runner.t226.s1.bop-local-conformance-manifest.v1') { throw 'Unexpected conformance schema' }

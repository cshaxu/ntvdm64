...
$projectionTool = Join-Path $repository 'tools\Project-BochsConfig.ps1'
...
$source = Join-Path $repository 'src\bx-mantle\bx_ntvdm_minimal_sim.cc'
$command = 'call "' + $devCmd + '" -arch=x86 -host_arch=x64 >nul && cl.exe /nologo /c /std:c++14 /EHsc /MT /DWIN32 /I "' + $repository + '\src" /I "' + $repository + '\src\bochs" /I "' + $repository + '\src\bochs\instrument\stubs" /FI "' + $config + '" /Fo"' + $object + '" "' + $source + '"'
...
$record = [ordered]@{ schema = 'ntdos64.t197.s6.minimal-sim-object.v1'; source = 'src/bx-mantle/bx_ntvdm_minimal_sim.cc'; configuration = 'tools/t197-s6-cpu5-mantle-config-projection.json'; architecture = 'x86'; compiler = 'MSVC cl.exe via VsDevCmd'; defines = @('WIN32'); includeRoots = @('src', 'src/bochs', 'src/bochs/instrument/stubs'); forbiddenInputs = @('main.cc', 'config.cc', 'gui/siminterface.cc', 'bochs.exe', 'device archives'); object = 'bx_ntvdm_minimal_sim.obj' }

...
for(const symbol of ['_OpenNtBaseGetBinaryTypeW@8','_BaseIsDosApplication'])
    assert(map.split(/\r?\n/).some(line=>line.includes(symbol)&&line.includes('client.obj')),`Wrong provider: ${symbol}`);
const run=spawnSync(exe,[],{cwd:build,windowsHide:true,encoding:'utf8',timeout:10000});
fs.writeFileSync(path.join(build,'result.json'),JSON.stringify({hashes,status:run.status,stdout:run.stdout,stderr:run.stderr,error:run.error?.message},null,2));

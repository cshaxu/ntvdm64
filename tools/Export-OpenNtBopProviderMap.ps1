...
$tables = [ordered]@{
    BOP_DOS = Read-DispatchTable 'upstream\opennt\base\mvdm\dos\dem\demdisp.c' '^\s*PFNSVC\s+apfnSVC\s*\[\]\s*='
    BOP_CMD = Read-DispatchTable 'upstream\opennt\base\mvdm\dos\command\cmddisp.c' '^\s*PFNSVC\s+apfnSVCCmd\s*\[\]\s*='
    BOP_XMS = Read-DispatchTable 'upstream\opennt\base\mvdm\xms.486\xmsdisp.c' '^\s*PFNSVC\s+apfnXMSSvc\s*\[\]\s*='
    BOP_DPMI = Read-DispatchTable 'upstream\opennt\base\mvdm\dpmi32\dpmi32.c' '^\s*VOID\s*\(\*DpmiDispatchTable\['
    BOP_REDIR = Read-DispatchTable 'upstream\opennt\base\mvdm\vdmredir\vrdisp.c' '^\s*VOID\s*\(\*VrDispatchTable\[\]\)\(VOID\)\s*='
}
...
foreach ($family in $inventory.service_families) {
    ...
    foreach ($entry in $family.entries) {
        ...
        $mapRows += [ordered]@{
            ...
            handler_source = if ($isDebugger) { 'upstream/opennt/base/mvdm/dbg/dbg.c:1361' } elseif ($null -eq $handler) { $null } else { "$($handler.source):$($handler.line)" }
...
$record = [ordered]@{
    ...
    debugger = [ordered]@{ selector_name = 'BOP_DEBUGGER'; original_entry = 'MS_bop_6 -> DBGDispatch'; source = 'upstream/opennt/base/mvdm/dbg/dbg.c:1361'; host_binding = $bindings.BOP_DEBUGGER; current_disposition = 'mapped-not-enabled' }

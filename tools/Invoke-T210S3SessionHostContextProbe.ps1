...
$exe = Join-Path $build 't210-s3-session-host-context.exe'
...
$sources += Join-Path $root 'tests\bx-vdm\bx_ntvdm_session_host_context_v1_test.c'
...
if ($LASTEXITCODE -ne 0) { throw "T210 S3 session host-context compilation failed: $LASTEXITCODE" }
& $exe 2>&1 | Tee-Object -FilePath (Join-Path $build 'run.log')
$runExit = $LASTEXITCODE
[ordered]@{ schema='ntdos64.t210.s3.session-host-context.v1'; architecture='x64'; runtimeLibrary='/MT'; hostIo=$true; guestExecution=$false; runExitCode=$runExit; passed=($runExit -eq 0) } | ConvertTo-Json | Set-Content -LiteralPath (Join-Path $build 't210-s3-session-host-context.json') -Encoding utf8
if ($runExit -ne 0) { throw "T210 S3 session host-context fixture failed: $runExit" }
Write-Host "T210 S3 session host-context probe passed: $build"

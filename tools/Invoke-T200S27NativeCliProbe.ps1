...
foreach ($name in @('target-selection.obj','engine-contract.obj','engine-run.obj')) { $lines += '"' + (Join-Path $build $name) + '"' }
...
@{schema='ntdos64.t200.s27.native-cli.v1';architecture='x64';runtimeLibrary='/MT';environmentTransport=$false;legacyBochsShell=$false;runExitCode=$runExit;expectedRunExitCode=0;passed=($runExit -eq 0)} | ConvertTo-Json | Set-Content -LiteralPath (Join-Path $build 't200-s27-native-cli.json') -Encoding utf8
if ($runExit -ne 0) { throw "S27 native CLI failed: $runExit" }
